...
//import com.f1j.swing.BookModelImpl ;
...
public class JRptList {
    ...
    private void initialize() {
        ...
        if (m_SumX_Caption.equalsIgnoreCase("")) {
            m_SumX_Caption = ""+bundle.getString("waitForTran")+"  ";
        }

        if (m_SumH_Caption.equalsIgnoreCase("")) {
            m_SumH_Caption = ""+bundle.getString("waitForTran")+"  ";
        }
...
////////////////////////////////////////////////////////////////
     ...
     * @param cDestCol 转换列
     * @param cStartRow 开始行
     * @param cChangeSQL 转换的SQL
     * @throws Exception
     */
    public void Change_Col_Code(String cDestCol, String cStartRow,
                                String cChangeSQL,
                                String cType, String cMethodName) throws
            Exception {
        System.out.println("=====开始列转换======");
        int i, iMax;
        String tSQL = "", tStr = "";
        int t_Sheet;
//        int t_Row_Count;
     ...
     * 转换行代码
     * @param cDestRow 目标行
     * @param cStartCol 开始的列
     * @param cChangeSQL 转化的SQL
     * @throws Exception 异常
     */
...
//        int t_Row_Count;
     ...
     * @throws Exception
     */
    private boolean CheckZero(int cX, int cY, int cEX, int cEY) throws
            Exception {
        int t_Sheet;
        int i, j;
        String tData = "";
    ...
    public void Compute_Sum() throws Exception {
        ...
        while (true) {
            ...
            if (t_Str.startsWith("#")) {
                ...
                if (!m_FV.getText(t_Cur_Row - 1, 0).trim().
                    equalsIgnoreCase("ROW_COL_FUNCTION")) {
                    break;
                }
                //解释行函数
                Put_Row_Col_FunctionData(m_FV.getText(t_Cur_Row - 1, 1),
                                         m_FV.getText(t_Cur_Row - 1, 2),
                                         m_FV.getText(t_Cur_Row - 1, 3));
            }
            t_Cur_Row += 1;
        }
        ...
        while (true) {
            ...
            if (t_Str.startsWith("#")) {
                ...
                if (!m_FV.getText(t_Cur_Row - 1, 0).trim().
                    equalsIgnoreCase("CHANGE_ROW_CODE")) {
                    break;
                }
                //转换行代码
                Change_Row_Code(m_FV.getText(t_Cur_Row - 1, 1),
                                m_FV.getText(t_Cur_Row - 1, 3),
                                m_FV.getText(t_Cur_Row - 1, 2));
            }
            t_Cur_Row += 1;
        ...
        while (true) {
            ...
            if (t_Str.startsWith("#")) {
                ...
                if (!m_FV.getText(t_Cur_Row - 1, 0).trim().
                    equalsIgnoreCase("CHANGE_COL_CODE")) {
                    break;
                }

                //转换列代码
                Change_Col_Code(m_FV.getText(t_Cur_Row - 1, 1),
                                m_FV.getText(t_Cur_Row - 1, 3),
                                m_FV.getText(t_Cur_Row - 1, 2),
                                m_FV.getText(t_Cur_Row - 1, 4),
        ...
        /*********************************************************************
         ...
         '            If UCase(Trim(.TextRC(t_Cur_Row, 1))) <> "UNITE_ROW" Then Exit Do
           '            Unite_Row .TextRC(t_Cur_Row, 2) '"+bundle.getString("waitForTran")+"
           '        End If
           '        t_Cur_Row += 1
           '    Loop
           '    ''''''''''''''''''"+bundle.getString("waitForTran")+"
         ...
         '            If UCase(Trim(.TextRC(t_Cur_Row, 1))) <> "UNITE_COL" Then Exit Do
           '            Unite_Col .TextRC(t_Cur_Row, 2) '"+bundle.getString("waitForTran")+"
           '        End If
           '        t_Cur_Row += 1
           '    Loop
         **************************************************************/
     ...
     * @throws Exception
     */
    public void Copy_Page_End(String c_Type, int c_Row, int c_Col) throws
            Exception {
        int t_Sheet;
        String t_Str = "";
        ...
        if (!c_Type.trim().equalsIgnoreCase("")) {
            ...
            if (!t_Str.trim().equalsIgnoreCase("")) {
                ...
                Copy_Sheet_Data(t_Rage, m_FV, R_Format, t_Rage1, m_FV, P_End,
                                0);
     ...
     * @throws Exception
     */
    public void Copy_Page_Head(String c_Type, int c_Row, int c_Col) throws
            Exception {
     ...
     * @throws Exception
     */
    public void Copy_Report_End(String c_Type, int c_Row, int c_Col) throws
            Exception {
        int t_Sheet;
        String t_Str = "";
        CRage t_Rage = new CRage();
        ...
        if (!c_Type.trim().equalsIgnoreCase("")) {
            ...
            if (!t_Str.trim().equalsIgnoreCase("")) {
                ...
                Copy_Sheet_Data(t_Rage, m_FV, R_Format, t_Rage1, m_FV, R_End,
                                0);
     ...
     * @throws Exception
     */
    public void Copy_Report_Head(String c_Type, int c_Row, int c_Col) throws
            Exception {
        int t_Sheet;
        String t_Str = "";
        CRage t_Rage = new CRage();
     ...
     * @param c_SurRage 指定区域
     * @param c_SurFV 源Sheet
     * @param c_SurFVIndex 源Sheet的编号
     * @param c_DestRage 目标区域
     * @param c_DestFV 目标Sheet
     * @param c_DestIndex 目标Sheet的编号
     * @param c_NeedRowHeight 保持行高度
     * @throws Exception
     */
    public void Copy_Sheet_Data(CRage c_SurRage, BookModelImpl c_SurFV,
                                int c_SurFVIndex, CRage c_DestRage,
                                BookModelImpl c_DestFV, int c_DestIndex,
                                int c_NeedRowHeight) throws Exception {
        int t_DstR2, t_DstC2;
        ...
        System.out.println("源Sheet:" + c_SurFVIndex);
        System.out.println("目标Sheet:" + c_DestIndex);
        System.out.println("源范围:" + (c_SurRage.m_Top - 1) + "," +
                           (c_SurRage.m_Left - 1)
                           + "," + (c_SurRage.m_Buttom - 1) + "," +
                           (c_SurRage.m_Right - 1));
        System.out.println("目标范围:" + (c_DestRage.m_Top - 1) + "," +
                           (c_DestRage.m_Left - 1)
        ...
        if (c_SurRage.m_Buttom > 0 && c_SurRage.m_Right > 0) {
            c_DestFV.copyRange(c_DestIndex, c_DestRage.m_Top - 1,
                               c_DestRage.m_Left - 1,
                               t_DstR2 - 1, t_DstC2 - 1, c_SurFV, c_SurFVIndex,
                               c_SurRage.m_Top - 1, c_SurRage.m_Left - 1,
...
//            m_DestFV.editPaste();
            m_DestFV.copyRange( -1, 0, -1, m_FV.getLastCol(), m_FV,
                               m_StartDisplayRow - 1, m_StartDisplayCol - 1, -1,
                               -1);
//            m_DestFV.copyAll(m_FV);

            for (i = 1; i <= m_FV.getLastCol() + 1; i++) {
                m_DestFV.setColWidth(i + m_StartDisplayCol - 2,
                                     m_FV.getColWidth(i - 1));
            }

            for (i = 1; i <= m_FV.getLastRow() + 1; i++) {
                m_DestFV.setRowHeight(i + m_StartDisplayRow - 2,
                                      m_FV.getRowHeight(i - 1));
            }
            //拷贝冻结列
...
//        int t_FldCount;
    ...
    /**
     * 执行函数，其中c_FunName是函数名，c_FunParameter是函数的参数
     * RowSum("2,3")表示将2，3列求和，放到第4列
     * @param c_FunName 函数名
     * @param c_FunParameter 函数的参数
     * @param c_Row_Number  行号
     * @return 字符串
     * @throws Exception
     */
...
//        int pcrRight, pcrTop, pcrBottom;
     ...
     * @param c_FunParameter 参数
     * @param c_Row 行号
     * @return 字符串
     * @throws Exception
     */
    private String Get_ExecSql(String c_FunParameter, int c_Row) throws
            Exception {
        SSRS tSSRS = new SSRS();
...
//        int i, i_Max;
    ...
    /**
     * 得到c_P参数的积,如果需要指定精度，则最后一个参数指定精度
     * JD1表示精确到小数点1位，JD2表示精确到2位....
     * 如果包含非数字字段，则该字段为0
     * @param c_P 传入数组
     * @return 计算结果
     */
...
//        String t_Format;
    ...
    /**
     * 本过程的功能是把系统定义文件中sheet=1的变量全部替换
     * 系统定义文件中sheet=1的变量包括两种类型，固定变量和不定变量
     * @param c_FileName 文件名
     * @throws Exception
     */
    private void Get_Ord_Val(String c_FileName) throws Exception {
        ...
        for (i = 0; i <= m_SysFBV.getLastRow(); i++) {
            ...
            if (m_SysFBV.getText(i, 1).equalsIgnoreCase("2")) {
                //替换sql语句返回值的变量
                t_Str = Get_Sql_Val(m_SysFBV.getText(i, 0),
                                    m_SysFBV.getText(i, 2),
            ...
            for (j = 1; j <= (t_Name.length - 1) / 2; j++) {
                ...
                if (!new File(getConfigFilePath() + "system_define.vts").
                    ...
                    for (i = 0; i <= t_F1Val.getLastRow(); i++) {
                        //查找变量
                        if (t_F1Val.getText(i,
                                            0).equalsIgnoreCase(t_ValName)) {
                            //取得sql语句
                    ...
                    t_n_sql = StrTool.replace(t_n_sql, "?" + t_ValName + "?",
                                              Get_Sql_Val(t_VName, t_Sql, t_DSN,
                            c_FileName));
...
//        String t_Value;
    ...
    /**
     * 隐藏列,row:s_x:s_y:e_x:e_y:含义为按照row相同进行隐藏，
     * 校验数据从s_x:s_y:e_x:e_y区域内
     * @param cControlString 传入参数
     * @throws Exception
     */
    ...
    void Hide_Row(String cControlString) throws Exception {
        ...
        while (true) {
            ...
            if (tSameStr.trim().equalsIgnoreCase(m_FV.getText(tCol - 1,
                    tCurRow - 1).trim())) {
                tCurRow += 1;
            } else {
     ...
     * 打印报表
     * @param FileName 文件名
     * @param pageContext 从JSP中传入的上下文
     * @throws Exception
     */
    public void Prt_RptList(PageContext pageContext, String FileName) throws
            Exception {
    ...
    public void Prt_RptList(String configPath, String templatePath,
                            String generatedPath, String urlPrefix,
                            String appPath,
                            String FileName) throws
            Exception {
    ...
    public void Prt_RptList1() throws Exception {
        ...
        //按指定格式生成文件
        m_FV.write(tOutFileName,
                   new com.f1j.ss.WriteParams(Constants.eFileCurrentFormat));

        if (StrTool.isUnicodeString(mWebReportWebDirectory.trim())) {
            mWebReportWebDirectory = StrTool.unicodeToGBK(
                    mWebReportWebDirectory.trim());
        }
        ...
        if (mNeedExcel) {
            ...
            tOutFileName = mWebReportDiskDirectory.trim() + tOutFileName1;
            m_FV.write(tOutFileName,
                       new com.f1j.ss.WriteParams(Constants.eFileExcel97));
            if (StrTool.isUnicodeString(tOutFileName1.trim())) {
                tOutFileName1 = StrTool.unicodeToGBK(tOutFileName1.trim());
            }
            ...
            if (StrTool.isUnicodeString(mExcelOutWebReportURL.trim())) {
                mExcelOutWebReportURL = StrTool.unicodeToGBK(
                        mExcelOutWebReportURL.trim());
            }
        }
    }
    ...
    public void AddPicture() throws Exception {
        ...
        for (int i = 0; i < m_FV.getLastRow(); i++) {
            ...
            if (m_FV.getText(i, 0).equalsIgnoreCase("picture")) {
                ...
                if (gr == null) {
                    DEBUG_OUT(""+bundle.getString("waitForTran")+"");
                } else {
                    ...
                    m_FV.addPicture(pos.getX1(), pos.getY1(), pos.getX2(),
                                    pos.getY2(),
     ...
     * @param c_DSN DSN
     * @param c_Key 键值
     * @param c_FldCount 字段个数
     * @param c_T8 转制格式参数
     * @throws Exception
     */

    public void Put_Data(int c_SRow, int c_SCol, String c_SQL, String c_DSN,
                         String c_Key, int c_FldCount, String c_T8) throws
            Exception {
        //Debug.Print c_SQL
        if (!c_Key.equalsIgnoreCase("")) {
            if (!c_T8.equalsIgnoreCase("")) {
                ...
                Put_Data_By_Key(c_SRow, c_SCol, c_SQL, c_DSN, c_Key,
                                c_FldCount);
            }
        } else {
...
// sangood Code
     ...
     * 分组数据，并保存
     * @param c_Group 分组列
     * @param c_Src_F1 源Sheet
     * @param c_Dst_F1 目标Sheet
     * @param c_Count 行数
     * @throws Exception
     */

    public void Order0(String[] c_Group, BookModelImpl c_Src_F1,
                       BookModelImpl c_Dst_F1,
    ...
    public void Order1(String[] c_Group, BookModelImpl c_Src_F1,
        ...
        for (int m = 1; m <= c_Group.length - 1; m++) {
            ...
            //移动到目的地
            c_Dst_F1.copyRange(0, 0, c_Dst_F1.getLastRow(), c_Dst_F1.getLastCol(),
                               c_Dst_F1,
                               0, 0, c_Dst_F1.getLastRow(), c_Dst_F1.getLastCol());

            //接着在源中删除之
            c_Src_F1.clearRange(0, 0, c_Dst_F1.getLastRow(),
                                c_Dst_F1.getLastCol(), Constants.eClearValues);
            ...
            while (i <= t_Count) {
                for (int j = 1; j <= c_Group.length - 1; j++) {
                    if (c_Src_F1.getText(i - 1,
                        ...
                        equalsIgnoreCase("")) {
                        c_Src_F1.deleteRange(i - 1, 0, i - 1,
                                             c_Src_F1.getLastCol(),
                                             Constants.eShiftRows);
                        t_Count--;
                        i--;
                ...
                while (j <= t_Count) {
                    ...
                    if (!t_Base.equalsIgnoreCase("")
                        ...
                        t_Comp = c_Src_F1.getText(j - 1,
                                                  Integer.parseInt(c_Group[m -
                                1]) - 1).trim();
                        break;
                    }
                    j++;
                    if (!c_Src_F1.getText(j - 1,
                                          Integer.parseInt(c_Group[m - 1]) - 1).
                        trim().equalsIgnoreCase(t_Comp)) {
                        break;
                    }
                } //全部搜索完毕，更新分组标志
    ...
    /**
     * 读取数据，放到R_Data中,
     * c_Key表示该查询语句要按某些列为健值来填写数据格式必须为1,2:1,2:3,4
     * 其中":"前面的1,2表示对应到现在已经有的数据，后面的1,2表示对应到查询结果的1，2字段,
     * 最后的3,4表示要填写3，4两个字段
     * @param c_SRow 源行
     * @param c_SCol 源列
     * @param c_SQL SQL
     * @param c_DSN DSN
     * @param c_Key 表示该查询语句要按某些列为健值来填写,数据格式必须为1,2:1,2:3,4
     * @param c_FldCount 字段数
     * @return 字符串
     * @throws Exception
     */
    public String Put_Data_By_Key(int c_SRow, int c_SCol, String c_SQL,
                                  String c_DSN, String c_Key, int c_FldCount) throws
            Exception {
        //在填写数据前会标记该列是否已经填写过
...
//        int t_FldCount;
        ...
        if (!(t_KeyCount == t_FldKey.size() - 1)) {
            DEBUG_OUT(
                    "Put_Data_By_Key()"+bundle.getString("waitForTran")+"");
            return "";
        }
                ...
                if (t_HavePut.get(i).equals(Boolean.FALSE)) {
                    for (j = 0; j <= t_KeyCount; j++) {
                        if (m_FV.getText(i - 1,
                                ...
                                toString()) - 1).trim().equalsIgnoreCase(
                                        JRptUtility.ChgValue(t_Rs.
                                GetText(rowcount + 1,
                                        Integer.parseInt(t_FldKey.get(j).
                                toString()) +
                                        1)).trim())) {
                            break;
                        }
            ...
            if (t_PP_Row != 0) {
                for (int i = 0; i <= t_FldDisplayCount; i++) {
                    t_Str = JRptUtility.ChgValue(t_Rs.GetText(rowcount + 1,
                            Integer.parseInt(t_FldIndex.
                                             get(i).toString()) + 1));

                    if (JRptUtility.IsNumeric(t_Str)) {
                        m_FV.setNumber(t_PP_Row - 1, t_Col + i - 1,
     ...
     * @param c_SQL SQL
     * @param c_DSN DSN
     * @param c_Key 表示该查询语句要按某些列为健值来填写数据格式必须为1,2:1,2:3,4
     * @param c_FldCount 字段数
     * @param c_T8 转换格式参数
     * @throws Exception
     */
    public void Put_Data_By_Key1(int c_SRow, int c_SCol, String c_SQL,
                                 String c_DSN, String c_Key, int c_FldCount,
                                 String c_T8) throws Exception {
...
//        int t_FldCount;
                ...
                if (t_HavePut.get(i).equals(Boolean.FALSE)) {
                    ...
                    if (j == t_KeyCount + 1) {
                        ...
                        for (j = 1; j <= m_FV.getLastCol() + 1; j++) {
                            t_F1v.setText(t_Next_Line - 1, j - 1,
                                          m_FV.getText(i - 1, j - 1));
                        }

                        for (j = 0; j <= t_FldDisplayCount; j++) {
                            t_Str = JRptUtility.ChgValue(t_Rs.GetText(rowcount +
                                    1,
     ...
     * @throws Exception
     */
    public String Put_Data_NOKey(int c_SRow, int c_SCol, String c_SQL,
...
//            tMaxRow = t_RowCount;
     ...
     * @throws Exception
     */
    public String Put_Row_Data_NOKey(int c_SRow, int c_SCol, String c_SQL,
                                     String c_DSN, int c_FldCount) throws
            Exception {
        SSRS t_Rs = new SSRS();
        int t_Row, t_Col;
        int t_FldCount;
     ...
     * @throws Exception
     */
    public String Put_Row_Data_NOKey1(int c_SRow, int c_SCol, String c_SQL,
                                      String c_DSN, int c_FldCount) throws
            Exception {
        SSRS t_Rs = new SSRS();
        int t_Row, t_Col;
...
//        BookModelImpl  t_Flv;
    ...
    /**
     * 解释合计函数,如SUM_H(3:4),表示对第三列进行合计，放在第4列
     * SUM_H(3),表示对第三列进行合计，放在第3列
     * @param c_Row 行
     * @param c_Text 值
     * @throws Exception
     */
    public void Put_Sum_H(int c_Row, String c_Text) throws Exception {
        ...
        if (t_ColStr.equalsIgnoreCase("")) {
            ...
            m_FV.setText(t_RowCount + 1 - 1, Integer.parseInt(t_ColStr) - 1,
                         bf.toString());
        }
        m_FV.setSheet(t_Sheet);
    }
...
//        int t_CurRow;
            ...
            if (t_Group.equalsIgnoreCase("")) {
                ...
                if (t_Group.equalsIgnoreCase(t_Str)) {
                    ...
                    for (j = 0; j <= t_GroupColCount; j++) {
                        ...
                        m_FV.clearRange(i - 1, (int) t_GroupCol[j] - 1,
    ...
    public void Raise_Report() throws Exception {
        ...
        System.out.println("数据行数" + t_Data_Count);
    ...
    public void Read_Data() throws Exception {
        ...
        while (true) {
            ...
            if (!t_Str.substring(0, 1).equalsIgnoreCase("#")) {
                ...
                Put_Data(conDataStart, t_Col, t_Sql, t_DSN, t_Key, t_FldCount,
                         t_T8);
            } //end if
            t_Cur_Row += 1;
            DEBUG_OUT(""+bundle.getString("waitForTran")+"");
        ...
        while (true) {
            ...
            if (!t_Str.substring(0, 1).equalsIgnoreCase("#")) {
                ...
                if (t_Sql.substring(0, 1).equalsIgnoreCase("!")
                    ...
                    t_Str = ReplaceValSQL(m_FV.getText(t_Cur_Row - 1, 2).trim(),
                                          m_Vals);
...
//        m_FV.setSheet(R_Format);
        ...
        if (!f.exists()) {
            m_ErrString = ""+bundle.getString("waitForTran")+"";
            return 0;
        }

//        m_FV.setSheet(R_Format);
        m_FV.read(c_FileName);
        System.out.println("读入数据摸班后的sheet个数" + m_FV.getNumSheets());
        InitSheets();
        m_FV.setSheet(R_Define);
...
//        String t_ValValue;
        ...
        while (true) {
            ...
            t_Return = StrTool.replace(StrTool.unicodeToGBK(t_Return),
                    ...
                    "?"),
                                       StrTool.unicodeToGBK(Get_Sys_Val_Value(
                                               t_ValName, c_Index)));
    ...
    /**
     * 本函数的功能是替换形如?row:col?的变量
     * 此时已经从数据库里取出数据存在指定的sheet,用row:col指定其值，并返回
     * @param c_SQL SQL
     * @return 字符串
     * @throws Exception
     */
        ...
        if (t_Val.indexOf(":") > 0) {
            ...
            while (!t_Val.equalsIgnoreCase("")) {
                ...
                if (t_Row_Col.length < 1) {
                    ...
                    //取指定坐标的值
                    t_Value = m_FV.getText(Integer.parseInt(t_Row_Col[0])
                                           - 1,
                    ...
                    c_SQL = StrTool.replace(c_SQL, "?" + t_Val + "?",
                                            t_Value);
                    //取下一个 ?XXXX? 字符串
                    t_Val = "";
                    t_Val = JRptUtility.Get_Str(c_SQL, 2, "?").trim();
    ...
    /**
     * 函数功能是把一个在"system_define.vts"中定义的函数，
     * 用具体的参数替换其定义的sql语句，然后返回
     * @param c_Func 函数
     * @return 字符串
     * @throws Exception
     */
    private String ReplaceFunc(String c_Func) throws Exception {
        ...
        t_FuncName = JRptUtility.Get_Str(c_Func, 2, "!").trim().
        ...
        for (i = 1; i <= j; i++) {
            ...
            if (((String[]) (c_Vals.get(i)))[1] == null ||
                ...
                while (t_I <= t_Start) {
                    ...
                    if (t_StrArray[t_I].indexOf(((String[]) c_Vals.get(i))[
                                                0]) >= 0) {
                ...
                if (((String[]) (c_Vals.get(i)))[1] == null ||
                    ...
                    while (t_I <= t_Start) {
                        ...
                        if (t_StrArray[t_I].indexOf(((String[]) c_Vals.get(i))[
                                0]) >= 0) {
                            if (t_StrArray[t_I - 1].equalsIgnoreCase(",")) {
                                for (t_End = t_I - 1; t_End <= t_Start - 2;
                                             t_End++) {
                                    t_StrArray[t_End] = t_StrArray[t_End + 2];
                                }
                                t_Start -= 2;
                                ...
                                if (t_StrArray[t_I + 1].equalsIgnoreCase(",")) {
                                    for (t_End = t_I; t_End <= t_Start - 2;
                                                 t_End++) {
                                        t_StrArray[t_End] = t_StrArray[t_End +
                                                2];
                                    }
                                    t_Start -= 2;
                                    t_I -= 1;
                    ...
                    while (t_I <= t_Start) {
                        ...
                        if (t_StrArray[t_I].indexOf(((String[]) c_Vals.get(i))[
                            ...
                            if (t_StrArray[t_I - 1].equalsIgnoreCase("(")) {
                                ...
                                if (t_StrArray[t_I + 1].equalsIgnoreCase(")")) {
                                    if (t_StrArray[t_I -
                                        2].equalsIgnoreCase("NOT")) {

                                        for (t_End = t_I - 2;
                                                t_End <= t_Start - 4; t_End++) {
                                            t_StrArray[t_End] = t_StrArray[
                                                    t_End + 4];
                                        }
                                        for (t_End = t_Start - 3;
                                                t_End <= t_Start; t_End++) {
                                    ...
                                    for (t_End = t_Start - 1; t_End <= t_Start;
                                                 t_End++) {
                                        t_StrArray[t_End] = "";
                                    }
                                    t_Start -= 2;
                            ...
                            } else if (t_StrArray[t_I -
                                       1].equalsIgnoreCase("NOT")) {
                                for (t_End = t_I - 2; t_End <= t_Start - 3;
                                             t_End++) {
                                    t_StrArray[t_End] = t_StrArray[t_End + 3];
                                }
                                for (t_End = t_Start - 2; t_End <= t_Start;
                                             t_End++) {
                                    t_StrArray[t_End] = "";
                                }
                                t_Start -= 3;
                                ...
                                if (!t_StrArray[t_I + 1].equalsIgnoreCase("AND")
                                    ...
                                    &&
                                    !t_StrArray[t_I + 1].equalsIgnoreCase("OR")) {
                                    for (t_End = t_I; t_End <= t_Start - 1;
                                                 t_End++) {
                                        t_StrArray[t_End] = t_StrArray[t_End +
                                                1];
                                    }
                                    for (t_End = t_Start; t_End <= t_Start;
                                                 t_End++) {
                                        t_StrArray[t_End] = "";
                                    }
                                    t_Start -= 1;
...
//                t_Str = t_Str.replaceAll( "?" + t_Find + "?", t_Replace );
    ...
    /**
     * 设置报表指定的区域的格式并合并单元格
     * @param cDestArea 目的区域
     * @param cRefArea 参考区域
     * @throws Exception
     */
    public void Report_Merge(String cDestArea, String cRefArea) throws
            Exception {
        int t_Sheet;
        int tStartRow, tStartCol, tEndRow, tEndCol;
        int tRefRow, tRefCol;
     ...
     * @param cRefArea 参考区域
     * @param cDestArea 目标区域
     * @throws Exception
     */
    public void Set_Report_Text(String cRefArea, String cDestArea) throws
            Exception {
        int t_Sheet;
        int tStartRow, tStartCol, tEndRow, tEndCol;
        int tRefRow, tRefCol;
...
//        int iMax,jMax;
     ...
     * 在报表生成时设置单个单元格的值
     * @param cRefArea 参考区域
     * @param cDataValue 数据值
     * @throws Exception
     */
    public void Set_Report_Text1(String cRefArea, String cDataValue) throws
            Exception {
        //在报表生成时设置单个值
...
//        int i;
        ...
        for (t_Cur_Row = 1; t_Cur_Row <= t_Row_Count; t_Cur_Row++) {
            ...
            if (!t_Str.startsWith("#")) {
                if (t_Str.equalsIgnoreCase("REPORT_MERGE")) {
                    //合并的报表区域
                    Report_Merge(m_FV.getText(t_Cur_Row - 1, 1),
                                 m_FV.getText(t_Cur_Row - 1, 2));
                }
            }
            ...
            if (!t_Str.startsWith("#")) {
                if (t_Str.equalsIgnoreCase("SET_REPORT_TEXT")) {
                    if (m_FV.getText(t_Cur_Row - 1, 2).indexOf(":") < 0) {
                        //如果没有包含":"，则表示取单值
                        //设置报表数据
                        Set_Report_Text1(m_FV.getText(t_Cur_Row - 1, 1),
                                         m_FV.getText(t_Cur_Row - 1, 2));
                    } else {
                        //设置报表数据
                        Set_Report_Text(m_FV.getText(t_Cur_Row - 1, 1),
                                        m_FV.getText(t_Cur_Row - 1, 2));
                    }
                }
            }
    ...
    private int TransferColPos(String cPosString, BookModelImpl cFV) {
        ...
        tStr = StrTool.replace(tStr, "curcol",
                               String.valueOf(this.m_Data_Col).trim());
        tStr = StrTool.replace(tStr, "maxcol",
                               String.valueOf(cFV.getLastCol() + 1).trim());
    ...
    private int TransferRowPos(String cPosString, BookModelImpl cFV) {
        ...
        tStr = StrTool.replace(tStr, "currow",
                               String.valueOf(this.m_Data_Row).trim());

        tStr = StrTool.replace(tStr, "maxrow",
                               String.valueOf(cFV.getLastRow() + 1).trim());
        returnvalue = (int) JRptUtility.GetValue(tStr);
        if (returnvalue == -1) {
    ...
    /**
     * 合并"行"单元格
     * 根据指定的坐标范围,合并指定列的顺序排列的数据相同的行
     * @param cControlString 字符
     * @throws Exception
     */
...
//        int tStartCol,tEndCol;
        ...
        while (true) {
            ...
            if (tSameStr.trim().equalsIgnoreCase(m_FV.getText(tCurRow - 1,
                    tCol - 1).trim())) {
                ...
                if (tStartRow != tCurRow - 1) {
                    ...
                    m_FV.setSelection(tStartRow - 1, tCol - 1, tCurRow - 1 - 1,
                                      tCol - 1);
                    tCF.setMergeCells(true);
                    tCF.setHorizontalAlignment(Format.
        ...
        if (tRow == -1) {
            for (int z = 1; z <= m_Data_Row; z++) {
                ...
                while (true) {
                    ...
                    if (tSameStr.trim().equalsIgnoreCase(m_FV.getText(z - 1,
                            tCurCol - 1).trim())) {
                        ...
                        if (tStartCol != tCurCol - 1) {
                            ...
                            tCF.setHorizontalAlignment(Format.
                                    eHorizontalAlignmentCenter);
                            tCF.setVerticalAlignment(Format.
...
//        int i;
    ...
    public void Write_Sum_X_To_Data(int c_Row, int c_ComputeCol, double c_Data,
                                    String c_Type, int cGroupCol) throws
            Exception {
        ...
        if (c_Type.trim().equalsIgnoreCase("B")) {
            ...
            m_FV.insertRange(t_Row - 1 + 1, 0, t_Row - 1 + 1, 0,
                             Constants.eShiftRows);
            //设置小计信息
            m_FV.setText(t_Row - 1 + 1, c_ComputeCol - 1,
                         m_SumX_Caption + String.valueOf(c_Data));
        }
        // 2. 如果描述信息为"BN"
        ...
        // 3. 如果描述信息为"R+列号"
            ...
            if (JRptUtility.IsNumeric(t_ColStr)) {
                ...
                m_ErrString = "Write_Sum_X_To_Data: RX "+bundle.getString("waitForTran")+"";
                return;
            }
            //设置指定行列的数据
...
//            t_Rpt.Prt_RptList("d:\\LIS\\ui\\WEB\\Config\\","D:\\lis\\ui\\f1print\\template\\","d:\\LIS\\ui\\web\\Generated\\","", "","BankDayCheck");
