 ...
 * <p>Title: </p>
 * <p>Description: </p>
 * <p>Copyright: Copyright (c) 2004.11.2</p>
...
public class BarCode
{
    ...
    private static String _BarCodeOffset=null;
    ...
    public void generatedImage()
    {
        ...
        { //显示相应文本在条形码下方
            ...
            //添加条形码垂直方向偏移量 wellhi 2007-11-09
            g.drawString(this.codeStr.toUpperCase(), imageWidth / 2 - xFont / 2,
                         h_Start + yFont - 3 + getBarCodeOffset() + this.barHeight);
            System.out.println("BarCode--fonM.getAscent():" + fonM.getAscent());
            System.out.println("BarCode--fonM.getDescent():" + fonM.getDescent());
            System.out.println("BarCode--h_Start:" + h_Start);
            System.out.println("BarCode--barHeight:" + barHeight);
            System.out.println("BarCode--getBarCodeOffset:" + getBarCodeOffset());
        }
    }
    ...
    /**
     * 从数据库中获取条形码垂直偏移量设置，出错或者没有设置就返回0
     * @author wellhi
     * @return int
     */
    private static int getBarCodeOffset()
    {
    	int ret=0;
    	//如果缓存中有值就直接取
    	if(_BarCodeOffset!=null)
    	{
    		ret=Integer.parseInt(_BarCodeOffset, 10);
    		return ret;
    	}
    	ExeSQL nExeSQL=new ExeSQL();
    	String sql="select sysvarvalue from ldsysvar where sysvar='BarCodeOffset'";
    	String BarCodeOffset =nExeSQL.getOneValue(sql);
    	
    	if(!(BarCodeOffset==null || BarCodeOffset.equals("")))
    	{
    		//放入缓存变量
    		_BarCodeOffset=BarCodeOffset;
    		ret=Integer.parseInt(BarCodeOffset, 10);
    	}
    	return ret;
    }
    //测试方法
