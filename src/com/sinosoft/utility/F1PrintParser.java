...
public class F1PrintParser
{
	...
	// 页面信息设置类 by wellhi 2008-03-27
	private F1ViewInfo _F1ViewInfo = new F1ViewInfo();
	...
	protected boolean parse() throws Exception
	{
		...
		m_bmOutput.read(strTemplateFile, new com.f1j.ss.ReadParams());
		controlDisplay(m_bmOutput, m_fp);

		int nMaxRow = bmTemplate.getMaxRow();
		...
		for (nRow = 0; nRow < nMaxRow - 1; nRow++)
		{
			...
			for (nCol = 0; nCol < nMaxCol - 1; nCol = bmTemplate.getSelEndCol() + 1)
			{
				...
				{
					// nNewMaxCol = nNewMaxCol < nNewCol + 1 ? nNewCol + 1 :
					// nNewMaxCol;
					...
					if (strSubText.equals(FLAG_MULTI_RECORD))
					{
						...
						while (m_fp.next())
						{
							if (nListLen > nMaxListLength)
							{
								m_bmOutput.insertRange(nCurrentRow, nNewCol,
										nCurrentRow, bmTemplate.getSelEndCol(),
						...
						if (nListLen > 0)
						{
							m_bmOutput.copyRange(nNewRow, nNewCol, nNewRow
									+ nListLen - 1, bmTemplate.getSelEndCol(),
									bmTemplate, nRow, nCol, nRow, bmTemplate
											.getSelEndCol(),
						...
						if (strSubText.equals(FLAG_ACRMULTI_RECORD))
						{
							strText = strText.substring(2);
							nChildPos = strText.lastIndexOf("/");
							strXPath = "/DATASET"
									+ strText.substring(0, nChildPos);
							strNodeName = strText.substring(nChildPos + 1);
							strListVal = m_fp.getNodeListValue(strXPath,
									strNodeName);
							...
							for (int k = 0; k < strListVal.length; k++)
							{
								...
								if (k > 0 && nNewRow == 0)
								{
									m_bmOutput.insertRange(nNewRow,
											nCurrentCol, nNewRow, nCurrentCol,
											Constants.eShiftRows);
								}
								if (strText.length() > 0)
								{
								...
								// m_bmOutput.setRowHeight(nCurrentRow,
								// nRowHeight);
								m_bmOutput.setSelection(nNewRow, nCurrentCol,
										nNewRow, nCurrentCol);
								m_bmOutput.setCellFormat(cellFormat);
							}
							...
							if (strSubText.equals(FLAG_PAGE_BREAK))
							{
								m_bmOutput.addRowPageBreak(nNewRow);

							}
							else
								if (strSubText.equals(FLAG_FULL_ROW))
								{
									// 一次解析一行的数据
									strXPath = "/DATASET"
											+ strText.substring(2);
									...
									while (m_fp.next())
									{
										if (nListLen > nMaxListLength)
										{
											m_bmOutput.insertRange(nCurrentRow,
													nNewCol, nCurrentRow,
													nNewCol + nColCount,
													Constants.eShiftRows);
										}
										...
										for (nColIndex = 0; nColIndex < nColCount; nColIndex++)
										{
											...
											bmTemplate.setSelection(nRow, nCol
													+ nColIndex, nRow, nCol
													+ nColIndex);
											cellFormat = bmTemplate
													.getCellFormat();
											cellFormat.useAllFormats();

											setValue(m_bmOutput, cellFormat,
													nCurrentRow, nNewCol
															+ nColIndex,
													m_fp.getString(nColIndex));

											m_bmOutput.setSelection(
													nCurrentRow, nNewCol
															+ nColIndex,
													nCurrentRow, nNewCol
															+ nColIndex);
									...
									if (strSubText.equals(FLAG_DATA_LIST))
									{
										// 覆盖行而不插入行
										strText = strText.substring(2);
										nChildPos = strText.lastIndexOf("/");
										strXPath = "/DATASET"
												+ strText.substring(0,
														nChildPos);
										strNodeName = strText
												.substring(nChildPos + 1);

										m_fp.query(strXPath);
										int nColIndex = m_fp
												.getColIndex(strNodeName);
										...
										while (m_fp.next())
										{
											...
											// m_bmOutput.setRowHeight(nCurrentRow,
											// nRowHeight);
											// m_bmOutput.setSelection(nCurrentRow,
											// nNewCol,
											// nCurrentRow, nNewCol);
											// m_bmOutput.setCellFormat(cellFormat);
											//
										...
										if (nListLen > 0)
										{
											m_bmOutput.copyRange(nNewRow,
													nNewCol, nNewRow + nListLen
															- 1, bmTemplate
															.getSelEndCol(),
													bmTemplate, nRow, nCol,
													nRow, bmTemplate
															.getSelEndCol(),
													Constants.eCopyFormats);
										}
										...
										if (strSubText.equals(FLAG_GROUP_SUM))
										{ // 分类汇总
											strText = strText.substring(2);
											String[] strItems = strText
													.split(",");

											String strFuncID = strItems[0]; // 计算函数编号
											int nGroupColIndex = findColIndex(
													bmTemplate, nRow,
													strItems[1]); // 分类类的列序号
											if (nGroupColIndex == -1)
											{
												System.out
														.println(""+bundle.getString("Cannotfind")+"["
																+ strItems[1]
											...
											{
												nSumColIndex[nIndex] = findColIndex(
														bmTemplate, nRow,
														strItems[nIndex + 2]);
											...
											{
												if (!m_bmOutput.getText(
														nCurrentRow,
														nGroupColIndex).equals(
														""))
												{
													strOldGroupColValue = m_bmOutput
															.getText(
																	nCurrentRow,
																	nGroupColIndex); // 保存上一个分类列的值
													break;
												}
												nCurrentRow--;
											}
											...
											while (nCurrentRow >= nNewRow)
											{
												...
												{
													if (strGroupColValue
															.equals(strOldGroupColValue))
													{
														getGroupSum(m_bmOutput,
																nCurrentRow,
																nSumColIndex,
																dSumColValue,
																true);
											...
											// 最后一个边界条件的检查
											insertRowWithFormat(bmTemplate,
													nRow, m_bmOutput,
													nInsertedRow, nCol);
											m_bmOutput.setText(nInsertedRow,
													nGroupColIndex, ""+bundle.getString("Summary")+"");
											for (nIndex = 0; nIndex < nSumColIndex.length; nIndex++)
											{
												m_bmOutput
														.setText(
											...
											// nInsertRowIndex = nCurrentRow +
											// 1;
											// strOldGroupColValue =
											// strGroupColValue;
											// getGroupSum(m_bmOutput,
											// nCurrentRow, nSumColIndex,
											// dSumColValue, false);
											//
											// nListLength++;
											nMaxListLength++;
										...
										else
										{
											...
											m_bmOutput.setSelection(nNewRow,
													nNewCol, nNewRow, nNewCol);
											m_bmOutput
													.setCellFormat(cellFormat);
											// m_bmOutput.setRowHeight(nNewRow,
											// nRowHeight);
											// if (nMaxListLength < 1) {
											// nMaxListLength = 1;
											// }
		...
		m_log.info("00 00 00 "+bundle.getString("logInfo")+":[" + m_fp.getTemplate() + "];数据行数为:["
				+ nNewRow + "]");
	...
	/**
	 * 设置页眉页脚等ViewInfo by wellhi 2008-03-27
	 * 
	 * @author wellhi
	 * @param 用F1ViewInfo传递设置变量
	 */
	public void setViewInfo(F1ViewInfo nF1ViewInfo)
	{
		_F1ViewInfo = nF1ViewInfo;
	}
	...
	private void setViewInfo()
	{
		...
		m_bmOutput.setAllowSelections(_F1ViewInfo.getAllowSelections()); // 打印显示界面不能选中
		m_bmOutput.setAllowObjectSelections(_F1ViewInfo
				.getAllowObjectSelections()); // 打印显示界面不能选中
		m_bmOutput.setShowGridLines(_F1ViewInfo.getShowGridLines()); // 去掉显示中的网格
		m_bmOutput.setShowColHeading(_F1ViewInfo.getShowColHeading()); // 去掉显示的列头
		m_bmOutput.setShowRowHeading(_F1ViewInfo.getShowRowHeading()); // 去掉显示的行头
		m_bmOutput.setShowEditBar(_F1ViewInfo.getShowEditBar()); // 去掉显示的bar
		m_bmOutput.setShowEditBarCellRef(_F1ViewInfo.getShowEditBarCellRef()); // 去掉显示的bar
		// m_bmOutput.setShowTabs(Constants.eTabsOff); // 去掉显示的sheet
		...
		m_bmOutput.setPrintGridLines(_F1ViewInfo.getPrintGridLines()); // 在打印时去掉网格
		m_bmOutput.setPrintHeader(_F1ViewInfo.getPrintHeader()); // 在打印时去掉头标题
		m_bmOutput.setPrintFooter(_F1ViewInfo.getPrintFooter()); // 在打印时去掉页码
	}
		...
		for (nRow = 0; nRow < bmTemplate.getMaxRow(); nRow++)
		{
			...
			if (strText != null && strText.length() > 1)
			{
				...
				if (strSubText.equals(FLAG_DISPLAY_START))
				{
					...
					if (strSubText.equals(FLAG_DISPLAY_END))
					{
						bNeedDelete = false;
						...
						if (strSubText.equals(FLAG_MODEL_END))
						{
							break; // When end of model is found, jump out of
							// loop
	...
	public static String QueryComVTS(String strDefVtsName, String strComCode)
	{
		...
		String Sql2 = "";
		String strIter = "";
		if (strComCode.equals("86"))
		{ // if(strComCode=="86"),如果管理机构代码是86
		...
		{ // 如果管理机构代码不是86
			...
			Sql1 = "select VtsFileName from ManageComVTS where"
					+ " DefVtsName='" + strDefVtsName + "' and ComCode='"
					+ strComCode + "'";
			ssrs1 = es.execSQL(Sql1);

			if ((ssrs1.getMaxRow() == 1) && (!ssrs1.GetText(1, 1).equals("")))
			{ // 如果查询有结果
			...
			{ // 如果查询没有结果
				// 则去查询该管理机构对应的上级管理机构
				Sql2 = "select UPCOMCODE from ldcom" + " where COMCODE='"
						+ strComCode + "'";
				ssrs2 = es.execSQL(Sql2);
