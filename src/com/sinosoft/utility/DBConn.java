...
public class DBConn implements java.sql.Connection
{
    ...
    public boolean createConnection()
    {
        ...
        try
        {
            if (con != null)
            {
                if (!con.isClosed())
                {
                    try
                    {
                        ...
                        stmt.execute("select top 1 * from sys.objects");
                        stmt.close();
                        return true;
                    }
                    catch (SQLException e)
                    {
                        System.out.println("DBConn Exception1: "+e.getMessage());
                        e.printStackTrace();
                        ...
                        {
                            System.out.println("DBConn Exception2: "+ex.getMessage());
                            e.printStackTrace();
                        }
                        finally
                        {
            ...
            else if (JUrl.getDBType().equalsIgnoreCase("SQLSERVER2005"))
            {
                dbType = 6;
            }
            ...
            switch (dbType)
            {
                ...
                case 3:
                    ...
                    // SQLSERVER
                	//20150303 tianzf TdsDriver驱动websphere不识别
