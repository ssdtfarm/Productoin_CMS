...
public class JimiImage
{
     ...
     * @param    String : sDestImage, 目标 GIF 图像文件存放路径
     * @param    int    : nTransparentIndex, 使某种颜色透明的调色板颜色值, 或 -1 不透明
     * @param    boolean: isInterlaced, 是否采用隔行扫描处理方式
     * @param    int    : nFrameDelay, 每一桢的显示延迟时间, 以毫秒为单位
     * @param    boolean: isUsingLocalPalette, 每一桢是否使用本地系统调色板颜色
     * @param    int    : nLoopNumbers, 动画循环显示次数, 或 -1 永远循环
     * @return   boolean
     */
    //public boolean convertToGIF(String sSourceImage, String sDestImage, int nTransparentIndex, boolean isInterlaced, int nFrameDelay, boolean isUsingLocalPalette, int nLoopNumbers)
    //{
    //    if (sSourceImage == null || sSourceImage.trim().equals(""))
    //    {
    //        System.out.println("\t@> JimiImage.convertToGIF() : 要转换的源图像文件路径不能为空！");
    //        return false;
    //    }
    //
    ...
    //    {
    //        System.out.println("\t@> JimiImage.convertToGIF() : 要转换的源图像文件路径不存在！");
    //        return false;
    //    }
    //    else if (!tSourceImageFile.canRead())
    //    {
    //        System.out.println("\t@> JimiImage.convertToGIF() : 要转换的源图像文件路径不可读！");
    //        return false;
    //    }
    //    else if (!tSourceImageFile.isFile())
    //    {
    //        System.out.println("\t@> JimiImage.convertToGIF() : 要转换的源图像路径不是一个有效的文件名！");
    //        return false;
    //    }
    ...
    //    {
    //        System.out.println("\t@> JimiImage.convertToGIF() : 转换图像格式发生异常！");
     ...
     * 转换图像格式为 PNG
     * @param    String : sSourceImage, 其它格式的源图像文件路径
     * @param    String : sDestImage, 目标 PNG 图像文件存放路径
     * @param    String : sCompression, 压缩比, none, default, fast, max
     * @return   boolean
     */
    public boolean convertToPNG(String sSourceImage, String sDestImage, String sCompression)
    {
        if (sSourceImage == null || sSourceImage.trim().equals(""))
        {
            System.out.println("\t@> JimiImage.convertToPNG() : 要转换的源图像文件路径不能为空！");
            return false;
        }
        ...
        if (!tSourceImageFile.exists())
        {
            System.out.println("\t@> JimiImage.convertToPNG() : 要转换的源图像文件路径不存在！");
            return false;
        }
        else if (!tSourceImageFile.canRead())
        {
            System.out.println("\t@> JimiImage.convertToPNG() : 要转换的源图像文件路径不可读！");
            return false;
        }
        else if (!tSourceImageFile.isFile())
        {
            System.out.println("\t@> JimiImage.convertToPNG() : 要转换的源图像路径不是一个有效的文件名！");
            return false;
        }
        tSourceImageFile = null;
        ...
        {
            System.out.println("\t@> JimiImage.convertToPNG() : 转换图像格式发生异常！");
            je.printStackTrace();
            return false;
        }
        catch (Exception ex)
