...
private static Logger logger = Logger.getLogger(LDSysVarSchema.class);
	...
	*/
	...
	{
		if(aSysVar!=null && aSysVar.length()>20)
			throw new IllegalArgumentException("SysvarSysVar值"+aSysVar+"的长度"+aSysVar.length()+"大于最大值20");
		SysVar = aSysVar;
	}
	...
	{
		if(aSysVarType!=null && aSysVarType.length()>3)
			throw new IllegalArgumentException("SysvartypeSysVarType值"+aSysVarType+"的长度"+aSysVarType.length()+"大于最大值3");
	...
	{
		if(aSysVarValue!=null && aSysVarValue.length()>500)
			throw new IllegalArgumentException("SysvarvalueSysVarValue值"+aSysVarValue+"的长度"+aSysVarValue.length()+"大于最大值500");
