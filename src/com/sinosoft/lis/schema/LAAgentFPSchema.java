...
import com.sinosoft.lis.db.LAAgentFPDB;

/*
 * <p>ClassName: LAAgentFPSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: LAAgentFP_Setting
 */
public class LAAgentFPSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LAAgentFPSchema.class);
	// @Field
	/** Agentcode */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LAAgentFPSchema cloned = (LAAgentFPSchema)super.clone();
	...
	/**
	* 使用另外一个 LAAgentFPSchema 对象给 Schema 赋值
	* @param: aLAAgentFPSchema LAAgentFPSchema
	**/
	public void setSchema(LAAgentFPSchema aLAAgentFPSchema)
	{
		this.AgentCode = aLAAgentFPSchema.getAgentCode();
		this.PlanType = aLAAgentFPSchema.getPlanType();
		this.FinancePackage = aLAAgentFPSchema.getFinancePackage();
		this.FinanceStartMonth = aLAAgentFPSchema.getFinanceStartMonth();
		this.MonthlyInstallment = aLAAgentFPSchema.getMonthlyInstallment();
		this.FinanceEndMonth = aLAAgentFPSchema.getFinanceEndMonth();
		this.FPAI = aLAAgentFPSchema.getFPAI();
		this.CBPaidMonth = aLAAgentFPSchema.getCBPaidMonth();
		this.CBInstallmentMonth = aLAAgentFPSchema.getCBInstallmentMonth();
		this.ChallengeBonusAmount = aLAAgentFPSchema.getChallengeBonusAmount();
		this.AchieveRewardRate = aLAAgentFPSchema.getAchieveRewardRate();
		this.AchieveRewardAmount = aLAAgentFPSchema.getAchieveRewardAmount();
		this.MegaFixedBonusReq = aLAAgentFPSchema.getMegaFixedBonusReq();
		this.MegaFixedBonus = aLAAgentFPSchema.getMegaFixedBonus();
		this.MegaExtraBonusReq = aLAAgentFPSchema.getMegaExtraBonusReq();
		this.MegaExtraBonusRate = aLAAgentFPSchema.getMegaExtraBonusRate();
		this.MegaAwardReq = aLAAgentFPSchema.getMegaAwardReq();
		this.MegaAward = aLAAgentFPSchema.getMegaAward();
		this.Operator = aLAAgentFPSchema.getOperator();
		this.MakeDate = fDate.getDate( aLAAgentFPSchema.getMakeDate());
		this.MakeTime = aLAAgentFPSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLAAgentFPSchema.getModifyDate());
		this.ModifyTime = aLAAgentFPSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LAAgentFP表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LAAgentFPSchema";
	...
	public LAAgentFPSchema getSchema()
	{
		LAAgentFPSchema aLAAgentFPSchema = new LAAgentFPSchema();
		aLAAgentFPSchema.setSchema(this);
		return aLAAgentFPSchema;
	}

	public LAAgentFPDB getDB()
	{
		LAAgentFPDB aDBOper = new LAAgentFPDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLAAgentFP>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LAAgentFPSchema other = (LAAgentFPSchema)otherObject;
		return
