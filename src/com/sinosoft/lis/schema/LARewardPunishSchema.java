...
{
private static Logger logger = Logger.getLogger(LARewardPunishSchema.class);
	// @Field
	/** Agentcode */
	...
	**/
	public void setSchema(LARewardPunishSchema aLARewardPunishSchema)
	{
		...
		this.Amount = aLARewardPunishSchema.getAmount();
		this.DoneDate = fDate.getDate( aLARewardPunishSchema.getDoneDate());
		this.DoneFlag = aLARewardPunishSchema.getDoneFlag();
		this.Remarks = aLARewardPunishSchema.getRemarks();
		this.AwardTitle = aLARewardPunishSchema.getAwardTitle();
		this.AwardSource = aLARewardPunishSchema.getAwardSource();
		this.AwardReason = aLARewardPunishSchema.getAwardReason();
		this.AwardType = aLARewardPunishSchema.getAwardType();
		this.AwardQuarter = aLARewardPunishSchema.getAwardQuarter();
		this.AwardYear = aLARewardPunishSchema.getAwardYear();
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LARewardPunish表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
