...
private static Logger logger = Logger.getLogger(LDMaxNoSchema.class);
	...
	*/
	...
	{
		if(aNoType!=null && aNoType.length()>15)
			throw new IllegalArgumentException("NotypeNoType值"+aNoType+"的长度"+aNoType.length()+"大于最大值15");
		NoType = aNoType;
	}
	...
	{
		if(aNoLimit!=null && aNoLimit.length()>20)
			throw new IllegalArgumentException("NolimitNoLimit值"+aNoLimit+"的长度"+aNoLimit.length()+"大于最大值20");
		NoLimit = aNoLimit;
	}
	...
	public void setMaxNo(double aMaxNo)
	{
		MaxNo = aMaxNo;
	}
	public void setMaxNo(String aMaxNo)
	{
		...
		{
			Double tDouble = new Double(aMaxNo);
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			MaxNo = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,3,SysConst.PACKAGESPILTER))).doubleValue();
		}
