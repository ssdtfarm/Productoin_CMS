...
{
private static Logger logger = Logger.getLogger(LADimissionSchema.class);
	// @Field
	/** Agentcode */
	...
	**/
	public void setSchema(LADimissionSchema aLADimissionSchema)
	{
		...
		this.ManageCom = aLADimissionSchema.getManageCom();
		this.Branchtype = aLADimissionSchema.getBranchtype();
		this.TerminationNum = aLADimissionSchema.getTerminationNum();
		this.Reason = aLADimissionSchema.getReason();
		this.OutWorkDate = fDate.getDate( aLADimissionSchema.getOutWorkDate());
		this.EndFlag = aLADimissionSchema.getEndFlag();
		this.Remarks = aLADimissionSchema.getRemarks();
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LADimission表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
