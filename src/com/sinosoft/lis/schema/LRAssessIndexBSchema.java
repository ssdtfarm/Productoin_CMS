...
import com.sinosoft.lis.db.LRAssessIndexBDB;

/*
 * <p>ClassName: LRAssessIndexBSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRAssessIndexBSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LRAssessIndexBSchema.class);
	// @Field
	/** Edorno */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LRAssessIndexBSchema cloned = (LRAssessIndexBSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LRAssessIndexBSchema 对象给 Schema 赋值
	* @param: aLRAssessIndexBSchema LRAssessIndexBSchema
	**/
	public void setSchema(LRAssessIndexBSchema aLRAssessIndexBSchema)
	{
		this.EdorNo = aLRAssessIndexBSchema.getEdorNo();
		this.BaseCode = aLRAssessIndexBSchema.getBaseCode();
		this.AgentGrade = aLRAssessIndexBSchema.getAgentGrade();
		this.IndexCode = aLRAssessIndexBSchema.getIndexCode();
		this.IndexName = aLRAssessIndexBSchema.getIndexName();
		this.IndexSet = aLRAssessIndexBSchema.getIndexSet();
		this.MainIndexFlag = aLRAssessIndexBSchema.getMainIndexFlag();
		this.IndexType = aLRAssessIndexBSchema.getIndexType();
		this.IndexSerise = aLRAssessIndexBSchema.getIndexSerise();
		this.CalType = aLRAssessIndexBSchema.getCalType();
		this.CalCode = aLRAssessIndexBSchema.getCalCode();
		this.DataType = aLRAssessIndexBSchema.getDataType();
		this.ResultType = aLRAssessIndexBSchema.getResultType();
		this.CalPrpty = aLRAssessIndexBSchema.getCalPrpty();
		this.DefaultValue = aLRAssessIndexBSchema.getDefaultValue();
		this.Description = aLRAssessIndexBSchema.getDescription();
		this.AllSet = aLRAssessIndexBSchema.getAllSet();
		this.CalSql = aLRAssessIndexBSchema.getCalSql();
		this.ITableName = aLRAssessIndexBSchema.getITableName();
		this.IColName = aLRAssessIndexBSchema.getIColName();
		this.BranchType = aLRAssessIndexBSchema.getBranchType();
		this.BranchType2 = aLRAssessIndexBSchema.getBranchType2();
		this.Json = aLRAssessIndexBSchema.getJson();
		this.SqlTemp = aLRAssessIndexBSchema.getSqlTemp();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LRAssessIndexB表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LRAssessIndexBSchema";
	...
	public LRAssessIndexBSchema getSchema()
	{
		LRAssessIndexBSchema aLRAssessIndexBSchema = new LRAssessIndexBSchema();
		aLRAssessIndexBSchema.setSchema(this);
		return aLRAssessIndexBSchema;
	}

	public LRAssessIndexBDB getDB()
	{
		LRAssessIndexBDB aDBOper = new LRAssessIndexBDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLRAssessIndexB>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			IndexCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			IndexName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			IndexSet = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			MainIndexFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			IndexType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			IndexSerise = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			CalType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			CalCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			DataType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			ResultType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			CalPrpty = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			DefaultValue = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			Description = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, SysConst.PACKAGESPILTER );
			AllSet = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 17, SysConst.PACKAGESPILTER );
			CalSql = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18, SysConst.PACKAGESPILTER );
			ITableName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 19, SysConst.PACKAGESPILTER );
			IColName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			...
			Json = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 23, SysConst.PACKAGESPILTER );
			SqlTemp = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 24, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LRAssessIndexBSchema other = (LRAssessIndexBSchema)otherObject;
		return
