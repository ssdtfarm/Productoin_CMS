...
private static Logger logger = Logger.getLogger(LDUserLogSchema.class);
	...
	*/
	...
	{
		if(aLogNo!=null && aLogNo.length()>10)
			throw new IllegalArgumentException("LognoLogNo值"+aLogNo+"的长度"+aLogNo.length()+"大于最大值10");
		LogNo = aLogNo;
	}
	...
	{
		if(aLogType!=null && aLogType.length()>2)
			throw new IllegalArgumentException("LogtypeLogType值"+aLogType+"的长度"+aLogType.length()+"大于最大值2");
		LogType = aLogType;
	}
	...
	{
		if(aLogContent!=null && aLogContent.length()>150)
			throw new IllegalArgumentException("LogcontentLogContent值"+aLogContent+"的长度"+aLogContent.length()+"大于最大值150");
	...
	{
		if(aCientIP!=null && aCientIP.length()>15)
			throw new IllegalArgumentException("CientipCientIP值"+aCientIP+"的长度"+aCientIP.length()+"大于最大值15");
		CientIP = aCientIP;
	}
	...
	**/
	public void setSchema(LDUserLogSchema aLDUserLogSchema)
	{
		...
		this.ServerAddress = aLDUserLogSchema.getServerAddress();
	}
