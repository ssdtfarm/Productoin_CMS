...
import com.sinosoft.lis.db.LAAgentAdjustDB;

/*
 * <p>ClassName: LAAgentAdjustSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: LAAgentSUA
 */
public class LAAgentAdjustSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LAAgentAdjustSchema.class);
	// @Field
	/** 加扣款序号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LAAgentAdjustSchema cloned = (LAAgentAdjustSchema)super.clone();
	...
	*/
	...
	public void setReason(String aReason)
	{
		if(aReason!=null && aReason.length()>500)
			throw new IllegalArgumentException("原因Reason值"+aReason+"的长度"+aReason.length()+"大于最大值500");
		Reason = aReason;
	}
	/**
	* Operator
	...
	*/
	...
	/**
	* 使用另外一个 LAAgentAdjustSchema 对象给 Schema 赋值
	* @param: aLAAgentAdjustSchema LAAgentAdjustSchema
	**/
	public void setSchema(LAAgentAdjustSchema aLAAgentAdjustSchema)
	{
		this.AdjustSN = aLAAgentAdjustSchema.getAdjustSN();
		this.AgentCode = aLAAgentAdjustSchema.getAgentCode();
		this.AdjustDate = fDate.getDate( aLAAgentAdjustSchema.getAdjustDate());
		this.AdjustMonth = aLAAgentAdjustSchema.getAdjustMonth();
		this.SubjectCode = aLAAgentAdjustSchema.getSubjectCode();
		this.Amount = aLAAgentAdjustSchema.getAmount();
		this.PaymentOrganisationType = aLAAgentAdjustSchema.getPaymentOrganisationType();
		this.PaymentOrganisation = aLAAgentAdjustSchema.getPaymentOrganisation();
		this.Remark = aLAAgentAdjustSchema.getRemark();
		this.CalType = aLAAgentAdjustSchema.getCalType();
		this.PaymentState = aLAAgentAdjustSchema.getPaymentState();
		this.ApprovalFlag = aLAAgentAdjustSchema.getApprovalFlag();
		this.ApprovalOperator = aLAAgentAdjustSchema.getApprovalOperator();
		this.ApprovalDate = fDate.getDate( aLAAgentAdjustSchema.getApprovalDate());
		this.Reason = aLAAgentAdjustSchema.getReason();
		this.Operator = aLAAgentAdjustSchema.getOperator();
		this.MakeDate = fDate.getDate( aLAAgentAdjustSchema.getMakeDate());
		this.MakeTime = aLAAgentAdjustSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLAAgentAdjustSchema.getModifyDate());
		this.ModifyTime = aLAAgentAdjustSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LAAgentAdjust表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LAAgentAdjustSchema";
	...
	public LAAgentAdjustSchema getSchema()
	{
		LAAgentAdjustSchema aLAAgentAdjustSchema = new LAAgentAdjustSchema();
		aLAAgentAdjustSchema.setSchema(this);
		return aLAAgentAdjustSchema;
	}

	public LAAgentAdjustDB getDB()
	{
		LAAgentAdjustDB aDBOper = new LAAgentAdjustDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLAAgentAdjust>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LAAgentAdjustSchema other = (LAAgentAdjustSchema)otherObject;
		return
