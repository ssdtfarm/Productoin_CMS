...
/*
 * <p>ClassName: LRAssessIndexPSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRAssessIndexPSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LRAssessIndexPSchema.class);
	// @Field
	/** Basecode */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LRAssessIndexPSchema cloned = (LRAssessIndexPSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LRAssessIndexPSchema 对象给 Schema 赋值
	* @param: aLRAssessIndexPSchema LRAssessIndexPSchema
	**/
	public void setSchema(LRAssessIndexPSchema aLRAssessIndexPSchema)
	{
		this.BaseCode = aLRAssessIndexPSchema.getBaseCode();
		this.AgentGrade = aLRAssessIndexPSchema.getAgentGrade();
		this.IndexCode = aLRAssessIndexPSchema.getIndexCode();
		this.IndexName = aLRAssessIndexPSchema.getIndexName();
		this.IndexSet = aLRAssessIndexPSchema.getIndexSet();
		this.MainIndexFlag = aLRAssessIndexPSchema.getMainIndexFlag();
		this.IndexType = aLRAssessIndexPSchema.getIndexType();
		this.IndexSerise = aLRAssessIndexPSchema.getIndexSerise();
		this.CalType = aLRAssessIndexPSchema.getCalType();
		this.CalCode = aLRAssessIndexPSchema.getCalCode();
		this.DataType = aLRAssessIndexPSchema.getDataType();
		this.ResultType = aLRAssessIndexPSchema.getResultType();
		this.CalPrpty = aLRAssessIndexPSchema.getCalPrpty();
		this.DefaultValue = aLRAssessIndexPSchema.getDefaultValue();
		this.Description = aLRAssessIndexPSchema.getDescription();
		this.AllSet = aLRAssessIndexPSchema.getAllSet();
		this.CalSql = aLRAssessIndexPSchema.getCalSql();
		this.SqlTemp = aLRAssessIndexPSchema.getSqlTemp();
		this.Json = aLRAssessIndexPSchema.getJson();
		this.BranchType = aLRAssessIndexPSchema.getBranchType();
		this.BranchType2 = aLRAssessIndexPSchema.getBranchType2();
		this.ITableName = aLRAssessIndexPSchema.getITableName();
		this.IColName = aLRAssessIndexPSchema.getIColName();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LRAssessIndexP表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LRAssessIndexPSchema";
	...
	public LRAssessIndexPSchema getSchema()
	{
		LRAssessIndexPSchema aLRAssessIndexPSchema = new LRAssessIndexPSchema();
		aLRAssessIndexPSchema.setSchema(this);
		return aLRAssessIndexPSchema;
	}

	public LRAssessIndexPDB getDB()
	{
		LRAssessIndexPDB aDBOper = new LRAssessIndexPDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLRAssessIndexP描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(IColName));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLRAssessIndexP>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			SqlTemp = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18, SysConst.PACKAGESPILTER );
			Json = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 19, SysConst.PACKAGESPILTER );
			...
			ITableName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22, SysConst.PACKAGESPILTER );
			IColName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 23, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LRAssessIndexPSchema other = (LRAssessIndexPSchema)otherObject;
		return
