...
//        System.out.println("---Start Save---");
        ...
        {
            // @@错误处理
            CError.buildErr(this, ""+bundle.getString("connFaild")+"");
            return false;
        }
...
//                            System.out.println("执行SQL语句:" + tSQL);
                            ...
                            {
                              CError.buildErr(this, ""+bundle.getString("Dataprocessingfailed")+":\n"
...
//                            System.out.println("执行SQL语句:" + tSQL);
                            if (!tExeSQL.execUpdateSQL(tSQL))
                            {
                                CError.buildErr(this, ""+bundle.getString("deleteFaild")+":\n"
                                    + tExeSQL.mErrors.getFirstError());
                                conn.rollback();
                                conn.close();
                                return false;
...
//                            System.out.println("执行SQL语句:" + tSQL);
                            if (!tExeSQL.execUpdateSQL(tSQL))
                            {
                                CError.buildErr(this,
                                                ""+bundle.getString("DatasubmitFaild")+"" +
                        ...
                        if (action.equals("SELECT"))                  //加入select方法，如果查出则表示有重复操作并报错退出
                        {
                            ...
                            {
                                CError.buildErr(this, ""+bundle.getString("ErrorMsg")+"");
                                conn.rollback();
                                conn.close();
                                return false;
                            }
                    ...
                    if (action.equals("INSERT"))
                    {
                        ...
                        if (!b.booleanValue())
                        {
                          CErrors nCErrors = (CErrors)getProperty(DBObject, "mErrors");
                          CError.buildErr(this, ""+bundle.getString("DatasubmitFaild")+":\n"
                                          + nCErrors.getFirstError());
                            try
                            {
                                conn.rollback();
                            }
