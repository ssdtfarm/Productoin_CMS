...
//            System.out.println("StringBuffer : " + tSql.toString());
            ...
            String tSql = "select * from LDMenuGrp where MenuGrpCode in ( select MenuGrpCode from LDUserToMenuGrp where UserCode = ?) "; 
            List paraList =new ArrayList(); 
            paraList.add(mLDUserSchema.getUserCode());
            mLDMenuGrpSet = tLDMenuGrpDB.executeQuery(tSql,PubFun.getFormatBV(paraList)); 
...
//                    "select * from ldmenugrp ");
            tLDMenuGrpDB = new LDMenuGrpDB();
            mLDMenuGrpSet = tLDMenuGrpDB.query();
        }
...
//            System.out.println("StringBuffer : " + tSql.toString());
            ...
            List paraList =new ArrayList(); 
            paraList.add(mLDUserSchema.getUserCode()); 
            mLDMenuGrpSet  = tLDMenuGrpDB.executeQuery(sqlStr,PubFun.getFormatBV(paraList)); 
        }
        else if (mLDMenuGrpSchema != null)
        {
            //通过菜单组编码查询菜单集合
...
//            System.out.println(tLDMenuGrpSet.size());
            ...
            LDMenuGrpSet tLDMenuGrpSet  = tLDMenuGrpDB.executeQuery(sqlStr,PubFun.getFormatBV(paraList)); 
        }
        else
        {
            //默认返回全部节点
