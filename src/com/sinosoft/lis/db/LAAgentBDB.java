...
public class LAAgentBDB extends LAAgentBSchema
{
	...
	**/
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LAAgentB WHERE  AgentCode = ? AND Operator1 = ? AND MakeDate1 = ? AND MakeTime1 = ?");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				...
				pstmt.setString(70, this.getOperator1());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(71, 91);
			} else {
				pstmt.setDate(71, Date.valueOf(this.getMakeDate1()));
			}
			if(this.getMakeTime1() == null || this.getMakeTime1().equals("null")) {
				pstmt.setNull(72, 12);
			} else {
				pstmt.setString(72, this.getMakeTime1());
			}
			if(this.getLastJobNature() == null || this.getLastJobNature().equals("null")) {
				pstmt.setNull(73, 12);
			} else {
				pstmt.setString(73, this.getLastJobNature());
			}
			// set where condition
			if(this.getAgentCode() == null || this.getAgentCode().equals("null")) {
				pstmt.setNull(74, 12);
			} else {
				pstmt.setString(74, this.getAgentCode());
			}
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				pstmt.setNull(75, 12);
			} else {
				pstmt.setString(75, this.getOperator1());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(76, 91);
			} else {
				pstmt.setDate(76, Date.valueOf(this.getMakeDate1()));
			}
			if(this.getMakeTime1() == null || this.getMakeTime1().equals("null")) {
				pstmt.setNull(77, 12);
			} else {
				pstmt.setString(77, this.getMakeTime1());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LAAgentB VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getAgentCode() == null || this.getAgentCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getAgentCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAgentB WHERE  AgentCode = ? AND Operator1 = ? AND MakeDate1 = ? AND MakeTime1 = ?", 
	...
	public LAAgentBSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAAgentBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAAgentB is querying for all data!");
			 }
			 mSql.append("select * from LAAgentB ");
	...
	public LAAgentBSet executeQuery(String sql, List bv)
	{
	...
	public LAAgentBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAAgentBSet aLAAgentBSet = new LAAgentBSet();
