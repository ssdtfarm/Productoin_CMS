...
/*
 * <p>ClassName: LAFinanceDataDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAFinanceDataDB extends LAFinanceDataSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAFinanceDataDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAFinanceDataSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAFinanceDataDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getDCFlag() == null || this.getDCFlag().equals("null")) {
				...
				pstmt.setString(8, this.getDCFlag());
			}
			pstmt.setDouble(9, this.getAmount());
			if(this.getB1() == null || this.getB1().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getB1());
			}
			if(this.getB2() == null || this.getB2().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getB2());
			}
			if(this.getB3() == null || this.getB3().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getB3());
			}
			...
			if(this.getTransDate() == null || this.getTransDate().equals("null")) {
				...
				pstmt.setDate(19, Date.valueOf(this.getTransDate()));
			}
			if(this.getManageCom() == null || this.getManageCom().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
			...
			if(this.getReportCode() == null || this.getReportCode().equals("null")) {
				...
				pstmt.setString(21, this.getReportCode());
			}
			if(this.getTitleCode() == null || this.getTitleCode().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getTitleCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAFinanceData WHERE  IndexCalNo = ? AND TransDate = ? AND ManageCom = ? AND ReportCode = ? AND TitleCode = ?", 
	...
	public LAFinanceDataSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAFinanceDataSet aLAFinanceDataSet = new LAFinanceDataSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAFinanceDataSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAFinanceData is querying for all data!");
			 }
			 mSql.append("select * from LAFinanceData ");
			...
			{
				i++;
				LAFinanceDataSchema s1 = new LAFinanceDataSchema();
				s1.setSchema(rs,i);
				aLAFinanceDataSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAFinanceDataSet;
	}

	public LAFinanceDataSet executeQuery(String sql, List bv)
	{
	...
	public LAFinanceDataSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAFinanceDataSet aLAFinanceDataSet = new LAFinanceDataSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAFinanceData");
			LAFinanceDataSchema aSchema = this.getSchema();
	...
	public LAFinanceDataSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAFinanceDataSet aLAFinanceDataSet = new LAFinanceDataSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAFinanceData " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAFinanceDataSet
 */
public LAFinanceDataSet getData()
{
    int tCount = 0;
    LAFinanceDataSet tLAFinanceDataSet = new LAFinanceDataSet();
    LAFinanceDataSchema tLAFinanceDataSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAFinanceDataDB";
    ...
    {
        tCount = 1;
        tLAFinanceDataSchema = new LAFinanceDataSchema();
        tLAFinanceDataSchema.setSchema(mResultSet, 1);
        tLAFinanceDataSet.add(tLAFinanceDataSchema);
