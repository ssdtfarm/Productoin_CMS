...
/*
 * <p>ClassName: LARelationshipBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LARelationshipBDB extends LARelationshipBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LARelationshipBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LARelationshipBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LARelationshipBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRelationID() == null || this.getRelationID().equals("null")) {
				...
				pstmt.setString(2, this.getRelationID());
			}
			...
			if(this.getIAgentCode() == null || this.getIAgentCode().equals("null")) {
				...
				pstmt.setString(8, this.getIAgentCode());
			}
			if(this.getIAgentGroup() == null || this.getIAgentGroup().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getIAgentGroup());
			}
			if(this.getIAgentGrade() == null || this.getIAgentGrade().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getIAgentGrade());
			}
			pstmt.setInt(11, this.getDeep());
			...
			if(this.getStartDate() == null || this.getStartDate().equals("null")) {
				...
				pstmt.setDate(13, Date.valueOf(this.getStartDate()));
			}
			if(this.getEndDate() == null || this.getEndDate().equals("null")) {
				pstmt.setNull(14, 91);
			} else {
				pstmt.setDate(14, Date.valueOf(this.getEndDate()));
			}
			if(this.getPBreakDate() == null || this.getPBreakDate().equals("null")) {
				pstmt.setNull(15, 91);
			} else {
				pstmt.setDate(15, Date.valueOf(this.getPBreakDate()));
			}
			pstmt.setInt(16, this.getINo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LARelationshipB WHERE  EdorNo = ?", 
	...
	public LARelationshipBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARelationshipBSet aLARelationshipBSet = new LARelationshipBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LARelationshipBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LARelationshipB is querying for all data!");
			 }
			 mSql.append("select * from LARelationshipB ");
			...
			{
				i++;
				LARelationshipBSchema s1 = new LARelationshipBSchema();
				s1.setSchema(rs,i);
				aLARelationshipBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLARelationshipBSet;
	}

	public LARelationshipBSet executeQuery(String sql, List bv)
	{
	...
	public LARelationshipBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARelationshipBSet aLARelationshipBSet = new LARelationshipBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LARelationshipB");
			LARelationshipBSchema aSchema = this.getSchema();
	...
	public LARelationshipBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LARelationshipBSet aLARelationshipBSet = new LARelationshipBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LARelationshipB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LARelationshipBSet
 */
public LARelationshipBSet getData()
{
    int tCount = 0;
    LARelationshipBSet tLARelationshipBSet = new LARelationshipBSet();
    LARelationshipBSchema tLARelationshipBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LARelationshipBDB";
    ...
    {
        tCount = 1;
        tLARelationshipBSchema = new LARelationshipBSchema();
        tLARelationshipBSchema.setSchema(mResultSet, 1);
        tLARelationshipBSet.add(tLARelationshipBSchema);
