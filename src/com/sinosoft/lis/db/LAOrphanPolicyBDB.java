...
public class LAOrphanPolicyBDB extends LAOrphanPolicyBSchema
{
	...
	**/
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				...
				pstmt.setDate(11, Date.valueOf(this.getMakeDate1()));
			}
			if(this.getMakeTime1() == null || this.getMakeTime1().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getMakeTime1());
			}
			// set where condition
			if(this.getMainPolNo() == null || this.getMainPolNo().equals("null")) {
				pstmt.setNull(13, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAOrphanPolicyB WHERE  MainPolNo = ? AND Operator1 = ? AND MakeDate1 = ? AND MakeTime1 = ?", 
	...
	public LAOrphanPolicyBSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAOrphanPolicyBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAOrphanPolicyB is querying for all data!");
			 }
			 mSql.append("select * from LAOrphanPolicyB ");
	...
	public LAOrphanPolicyBSet executeQuery(String sql, List bv)
	{
	...
	public LAOrphanPolicyBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAOrphanPolicyBSet aLAOrphanPolicyBSet = new LAOrphanPolicyBSet();
