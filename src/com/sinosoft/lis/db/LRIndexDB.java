...
/*
 * <p>ClassName: LRIndexDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRIndexDB extends LRIndexSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRIndexDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRIndexSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getWageCode() == null || this.getWageCode().equals("null")) {
				...
				pstmt.setString(1, this.getWageCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getWageName() == null || this.getWageName().equals("null")) {
				...
				pstmt.setString(2, this.getWageName());
			}
			...
			if(this.getWageType() == null || this.getWageType().equals("null")) {
				...
				pstmt.setString(5, this.getWageType());
			}
			...
			if(this.getBranchType2() == null || this.getBranchType2().equals("null")) {
				...
				pstmt.setString(8, this.getBranchType2());
			}
			...
			if(this.getWageCode() == null || this.getWageCode().equals("null")) {
				...
				pstmt.setString(14, this.getWageCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRIndex WHERE  WageCode = ?", 
	...
	public LRIndexSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexSet aLRIndexSet = new LRIndexSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRIndexSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRIndex is querying for all data!");
			 }
			 mSql.append("select * from LRIndex ");
			...
			{
				i++;
				LRIndexSchema s1 = new LRIndexSchema();
				s1.setSchema(rs,i);
				aLRIndexSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LRIndexSet executeQuery(String sql, List bv)
	{
	...
	public LRIndexSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexSet aLRIndexSet = new LRIndexSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRIndex");
			LRIndexSchema aSchema = this.getSchema();
	...
	public LRIndexSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRIndexSet aLRIndexSet = new LRIndexSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRIndex " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LRIndexSet
 */
public LRIndexSet getData()
{
    int tCount = 0;
    LRIndexSet tLRIndexSet = new LRIndexSet();
    LRIndexSchema tLRIndexSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRIndexDB";
    ...
    {
        tCount = 1;
        tLRIndexSchema = new LRIndexSchema();
        tLRIndexSchema.setSchema(mResultSet, 1);
        tLRIndexSet.add(tLRIndexSchema);
