...
/*
 * <p>ClassName: LRAssessIndexPDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRAssessIndexPDB extends LRAssessIndexPSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRAssessIndexPDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRAssessIndexPSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LRAssessIndexPDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getMainIndexFlag() == null || this.getMainIndexFlag().equals("null")) {
				...
				pstmt.setString(4, this.getMainIndexFlag());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSqlTemp() == null || this.getSqlTemp().equals("null")) {
				...
				pstmt.setString(18, this.getSqlTemp());
			}
			if(this.getJson() == null || this.getJson().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getJson());
			}
			...
			if(this.getITableName() == null || this.getITableName().equals("null")) {
				...
				pstmt.setString(22, this.getITableName());
			}
			if(this.getIColName() == null || this.getIColName().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getIColName());
			}
			// set where condition
			if(this.getBaseCode() == null || this.getBaseCode().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getBaseCode());
			}
			if(this.getAgentGrade() == null || this.getAgentGrade().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRAssessIndexP WHERE  BaseCode = ? AND AgentGrade = ? AND IndexCode = ? AND MainIndexFlag = ?", 
	...
	public LRAssessIndexPSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRAssessIndexPSet aLRAssessIndexPSet = new LRAssessIndexPSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRAssessIndexPSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRAssessIndexP is querying for all data!");
			 }
			 mSql.append("select * from LRAssessIndexP ");
			...
			{
				i++;
				LRAssessIndexPSchema s1 = new LRAssessIndexPSchema();
				s1.setSchema(rs,i);
				aLRAssessIndexPSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLRAssessIndexPSet;
	}

	public LRAssessIndexPSet executeQuery(String sql, List bv)
	{
	...
	public LRAssessIndexPSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRAssessIndexPSet aLRAssessIndexPSet = new LRAssessIndexPSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRAssessIndexP");
			LRAssessIndexPSchema aSchema = this.getSchema();
	...
	public LRAssessIndexPSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRAssessIndexPSet aLRAssessIndexPSet = new LRAssessIndexPSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRAssessIndexP " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LRAssessIndexPSet
 */
public LRAssessIndexPSet getData()
{
    int tCount = 0;
    LRAssessIndexPSet tLRAssessIndexPSet = new LRAssessIndexPSet();
    LRAssessIndexPSchema tLRAssessIndexPSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRAssessIndexPDB";
    ...
    {
        tCount = 1;
        tLRAssessIndexPSchema = new LRAssessIndexPSchema();
        tLRAssessIndexPSchema.setSchema(mResultSet, 1);
        tLRAssessIndexPSet.add(tLRAssessIndexPSchema);
