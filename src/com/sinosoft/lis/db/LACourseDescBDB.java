...
/*
 * <p>ClassName: LACourseDescBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LACourseDescBDB extends LACourseDescBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LACourseDescBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LACourseDescBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LACourseDescBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCourseType() == null || this.getCourseType().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
				pstmt.setString(3, this.getCourseType());
			}
			if(this.getCourseSeries() == null || this.getCourseSeries().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getCourseSeries());
			}
			if(this.getCourseGrade() == null || this.getCourseGrade().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getCourseGrade());
			}
			if(this.getCourseNo() == null || this.getCourseNo().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getCourseNo());
			}
			if(this.getCourseProfit() == null || this.getCourseProfit().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getCourseProfit());
			}
			if(this.getCourseName() == null || this.getCourseName().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getCourseName());
			}
			pstmt.setDouble(9, this.getCourseHour());
			if(this.getTeacherGrade() == null || this.getTeacherGrade().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getTeacherGrade());
			}
			if(this.getStopFlag() == null || this.getStopFlag().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getStopFlag());
			}
			...
			if(this.getCourseID() == null || this.getCourseID().equals("null")) {
				...
				pstmt.setString(21, this.getCourseID());
			}
			if(this.getChannelcode() == null || this.getChannelcode().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getChannelcode());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(23, 91);
			} else {
				pstmt.setDate(23, Date.valueOf(this.getMakeDate1()));
			}
			if(this.getMakeTime1() == null || this.getMakeTime1().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getMakeTime1());
			}
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LACourseDescB WHERE  CourseID = ? AND Channelcode = ? AND MakeDate1 = ? AND MakeTime1 = ? AND Operator1 = ?", 
	...
	public LACourseDescBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LACourseDescBSet aLACourseDescBSet = new LACourseDescBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LACourseDescBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LACourseDescB is querying for all data!");
			 }
			 mSql.append("select * from LACourseDescB ");
			...
			{
				i++;
				LACourseDescBSchema s1 = new LACourseDescBSchema();
				s1.setSchema(rs,i);
				aLACourseDescBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLACourseDescBSet;
	}

	public LACourseDescBSet executeQuery(String sql, List bv)
	{
	...
	public LACourseDescBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LACourseDescBSet aLACourseDescBSet = new LACourseDescBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LACourseDescB");
			LACourseDescBSchema aSchema = this.getSchema();
	...
	public LACourseDescBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LACourseDescBSet aLACourseDescBSet = new LACourseDescBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LACourseDescB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LACourseDescBSet
 */
public LACourseDescBSet getData()
{
    int tCount = 0;
    LACourseDescBSet tLACourseDescBSet = new LACourseDescBSet();
    LACourseDescBSchema tLACourseDescBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LACourseDescBDB";
    ...
    {
        tCount = 1;
        tLACourseDescBSchema = new LACourseDescBSchema();
        tLACourseDescBSchema.setSchema(mResultSet, 1);
        tLACourseDescBSet.add(tLACourseDescBSchema);
