...
import com.sinosoft.lis.vschema.LRBaseBSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LRBaseBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRBaseBDB extends LRBaseBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRBaseBDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LRBaseB" );
		mflag = true;
	}
	...
	{
		LRBaseBSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LRBaseB WHERE  EdorNo = ?");
		...
		SQLString sqlObj = new SQLString("LRBaseB");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LRBaseB SET  EdorNo = ? , EdorType = ? , BaseCode = ? , Name = ? , Status = ? , Remark = ? , Reason = ? , BranchType = ? , BranchType2 = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Operator2 = ? , MakeDate2 = ? , MakeTime2 = ? , ModifyDate2 = ? , ModifyTime2 = ? WHERE  EdorNo = ?");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
			...
			if(this.getStatus() == null || this.getStatus().equals("null")) {
				...
				pstmt.setString(5, this.getStatus());
			}
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LRBaseB VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRBaseB WHERE  EdorNo = ?", 
	...
	public LRBaseBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRBaseBSet aLRBaseBSet = new LRBaseBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRBaseBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRBaseB is querying for all data!");
			 }
			 mSql.append("select * from LRBaseB ");
			...
			{
				i++;
				LRBaseBSchema s1 = new LRBaseBSchema();
				s1.setSchema(rs,i);
				aLRBaseBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LRBaseBSet executeQuery(String sql, List bv)
	{
	...
	public LRBaseBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRBaseBSet aLRBaseBSet = new LRBaseBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRBaseB");
			LRBaseBSchema aSchema = this.getSchema();
	...
	public LRBaseBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRBaseBSet aLRBaseBSet = new LRBaseBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRBaseB " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LRBaseBSet
 */
public LRBaseBSet getData()
{
    int tCount = 0;
    LRBaseBSet tLRBaseBSet = new LRBaseBSet();
    LRBaseBSchema tLRBaseBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRBaseBDB";
    ...
    {
        tCount = 1;
        tLRBaseBSchema = new LRBaseBSchema();
        tLRBaseBSchema.setSchema(mResultSet, 1);
        tLRBaseBSet.add(tLRBaseBSchema);
