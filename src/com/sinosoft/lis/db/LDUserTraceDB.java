...
/*
 * <p>ClassName: LDUserTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LDUserTraceDB extends LDUserTraceSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDUserTraceDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDUserTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDUserTraceDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt.setDouble(1, this.getTraceNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getTraceType() == null || this.getTraceType().equals("null")) {
				...
				pstmt.setString(4, this.getTraceType());
			}
			if(this.getTraceContent() == null || this.getTraceContent().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getTraceContent());
			}
			if(this.getClientIP() == null || this.getClientIP().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getClientIP());
			}
			...
			// set where condition
			pstmt.setDouble(10, this.getTraceNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDUserTrace WHERE  TraceNo = ?", 
	...
	public LDUserTraceSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDUserTraceSet aLDUserTraceSet = new LDUserTraceSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LDUserTraceSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LDUserTrace is querying for all data!");
			 }
			 mSql.append("select * from LDUserTrace ");
			...
			{
				i++;
				LDUserTraceSchema s1 = new LDUserTraceSchema();
				s1.setSchema(rs,i);
				aLDUserTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDUserTraceSet;
	}

	public LDUserTraceSet executeQuery(String sql, List bv)
	{
	...
	public LDUserTraceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDUserTraceSet aLDUserTraceSet = new LDUserTraceSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDUserTrace");
			LDUserTraceSchema aSchema = this.getSchema();
	...
	public LDUserTraceSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LDUserTraceSet aLDUserTraceSet = new LDUserTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDUserTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDUserTraceSet
 */
public LDUserTraceSet getData()
{
    int tCount = 0;
    LDUserTraceSet tLDUserTraceSet = new LDUserTraceSet();
    LDUserTraceSchema tLDUserTraceSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDUserTraceDB";
    ...
    {
        tCount = 1;
        tLDUserTraceSchema = new LDUserTraceSchema();
        tLDUserTraceSchema.setSchema(mResultSet, 1);
        tLDUserTraceSet.add(tLDUserTraceSchema);
