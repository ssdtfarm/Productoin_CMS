...
/*
 * <p>ClassName: LRIndexVsCommCDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRIndexVsCommCDB extends LRIndexVsCommCSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRIndexVsCommCDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRIndexVsCommCSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LRIndexVsCommCDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getBakeMonth() == null || this.getBakeMonth().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBakeMonth());
			}
			...
			if(this.getWageCode() == null || this.getWageCode().equals("null")) {
				...
				pstmt.setString(7, this.getWageCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getIndexSerise() == null || this.getIndexSerise().equals("null")) {
				...
				pstmt.setString(7, this.getIndexSerise());
			}
			if(this.getWageCode() == null || this.getWageCode().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getWageCode());
			}
			if(this.getWageName() == null || this.getWageName().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getWageName());
			}
			if(this.getIndexCode() == null || this.getIndexCode().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getIndexCode());
			}
			...
			if(this.getBakeMonth() == null || this.getBakeMonth().equals("null")) {
				...
				pstmt.setString(17, this.getBakeMonth());
			}
			if(this.getType() == null || this.getType().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getType());
			}
			if(this.getBaseCode() == null || this.getBaseCode().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getBaseCode());
			}
			if(this.getManageCom() == null || this.getManageCom().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
			...
			if(this.getAgentGrade() == null || this.getAgentGrade().equals("null")) {
				...
				pstmt.setString(21, this.getAgentGrade());
			}
			if(this.getIndexType() == null || this.getIndexType().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getIndexType());
			}
			if(this.getWageCode() == null || this.getWageCode().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getWageCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRIndexVsCommC WHERE  BakeMonth = ? AND Type = ? AND BaseCode = ? AND ManageCom = ? AND AgentGrade = ? AND IndexType = ? AND WageCode = ?", 
	...
	public LRIndexVsCommCSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexVsCommCSet aLRIndexVsCommCSet = new LRIndexVsCommCSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRIndexVsCommCSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRIndexVsCommC is querying for all data!");
			 }
			 mSql.append("select * from LRIndexVsCommC ");
			...
			{
				i++;
				LRIndexVsCommCSchema s1 = new LRIndexVsCommCSchema();
				s1.setSchema(rs,i);
				aLRIndexVsCommCSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLRIndexVsCommCSet;
	}

	public LRIndexVsCommCSet executeQuery(String sql, List bv)
	{
	...
	public LRIndexVsCommCSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexVsCommCSet aLRIndexVsCommCSet = new LRIndexVsCommCSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRIndexVsCommC");
			LRIndexVsCommCSchema aSchema = this.getSchema();
	...
	public LRIndexVsCommCSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRIndexVsCommCSet aLRIndexVsCommCSet = new LRIndexVsCommCSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRIndexVsCommC " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LRIndexVsCommCSet
 */
public LRIndexVsCommCSet getData()
{
    int tCount = 0;
    LRIndexVsCommCSet tLRIndexVsCommCSet = new LRIndexVsCommCSet();
    LRIndexVsCommCSchema tLRIndexVsCommCSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRIndexVsCommCDB";
    ...
    {
        tCount = 1;
        tLRIndexVsCommCSchema = new LRIndexVsCommCSchema();
        tLRIndexVsCommCSchema.setSchema(mResultSet, 1);
        tLRIndexVsCommCSet.add(tLRIndexVsCommCSchema);
