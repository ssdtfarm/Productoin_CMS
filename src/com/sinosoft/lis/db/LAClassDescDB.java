...
/*
 * <p>ClassName: LAClassDescDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAClassDescDB extends LAClassDescSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAClassDescDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAClassDescSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAClassDescDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getClassNo() == null || this.getClassNo().equals("null")) {
				...
				pstmt.setString(12, this.getClassNo());
			}
			if(this.getChannelCode() == null || this.getChannelCode().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getChannelCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAClassDesc WHERE  ClassNo = ? AND ChannelCode = ?", 
	...
	public LAClassDescSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAClassDescSet aLAClassDescSet = new LAClassDescSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAClassDescSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAClassDesc is querying for all data!");
			 }
			 mSql.append("select * from LAClassDesc ");
			...
			{
				i++;
				LAClassDescSchema s1 = new LAClassDescSchema();
				s1.setSchema(rs,i);
				aLAClassDescSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAClassDescSet;
	}

	public LAClassDescSet executeQuery(String sql, List bv)
	{
	...
	public LAClassDescSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAClassDescSet aLAClassDescSet = new LAClassDescSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAClassDesc");
			LAClassDescSchema aSchema = this.getSchema();
	...
	public LAClassDescSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAClassDescSet aLAClassDescSet = new LAClassDescSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAClassDesc " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAClassDescSet
 */
public LAClassDescSet getData()
{
    int tCount = 0;
    LAClassDescSet tLAClassDescSet = new LAClassDescSet();
    LAClassDescSchema tLAClassDescSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAClassDescDB";
    ...
    {
        tCount = 1;
        tLAClassDescSchema = new LAClassDescSchema();
        tLAClassDescSchema.setSchema(mResultSet, 1);
        tLAClassDescSet.add(tLAClassDescSchema);
