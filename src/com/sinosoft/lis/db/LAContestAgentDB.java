...
/*
 * <p>ClassName: LAContestAgentDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAContestAgentDB extends LAContestAgentSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAContestAgentDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAContestAgentSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAContestAgentDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAContestAgent WHERE  ContestCode = ? AND AgentCode = ?", 
	...
	public LAContestAgentSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAContestAgentSet aLAContestAgentSet = new LAContestAgentSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAContestAgentSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAContestAgent is querying for all data!");
			 }
			 mSql.append("select * from LAContestAgent ");
			...
			{
				i++;
				LAContestAgentSchema s1 = new LAContestAgentSchema();
				s1.setSchema(rs,i);
				aLAContestAgentSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAContestAgentSet;
	}

	public LAContestAgentSet executeQuery(String sql, List bv)
	{
	...
	public LAContestAgentSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAContestAgentSet aLAContestAgentSet = new LAContestAgentSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAContestAgent");
			LAContestAgentSchema aSchema = this.getSchema();
	...
	public LAContestAgentSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAContestAgentSet aLAContestAgentSet = new LAContestAgentSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAContestAgent " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAContestAgentSet
 */
public LAContestAgentSet getData()
{
    int tCount = 0;
    LAContestAgentSet tLAContestAgentSet = new LAContestAgentSet();
    LAContestAgentSchema tLAContestAgentSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAContestAgentDB";
    ...
    {
        tCount = 1;
        tLAContestAgentSchema = new LAContestAgentSchema();
        tLAContestAgentSchema.setSchema(mResultSet, 1);
        tLAContestAgentSet.add(tLAContestAgentSchema);
