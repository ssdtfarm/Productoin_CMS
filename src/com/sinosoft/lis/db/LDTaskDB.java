...
/*
 * <p>ClassName: LDTaskDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LDTaskDB extends LDTaskSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDTaskDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDTaskSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getTaskDescribe() == null || this.getTaskDescribe().equals("null")) {
				pstmt.setNull(2, 12);
			} else {
				pstmt.setString(2, this.getTaskDescribe());
			}
			if(this.getTaskClass() == null || this.getTaskClass().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
				pstmt.setString(3, this.getTaskClass());
			}
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LDTask VALUES( ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getTaskCode() == null || this.getTaskCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getTaskCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDTask WHERE  TaskCode = ?", 
	...
	public LDTaskSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDTaskSet aLDTaskSet = new LDTaskSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LDTaskSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LDTask is querying for all data!");
			 }
			 mSql.append("select * from LDTask ");
			...
			{
				i++;
				LDTaskSchema s1 = new LDTaskSchema();
				s1.setSchema(rs,i);
				aLDTaskSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LDTaskSet executeQuery(String sql, List bv)
	{
	...
	public LDTaskSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDTaskSet aLDTaskSet = new LDTaskSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDTask");
			LDTaskSchema aSchema = this.getSchema();
	...
	public LDTaskSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LDTaskSet aLDTaskSet = new LDTaskSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDTask " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LDTaskSet
 */
public LDTaskSet getData()
{
    int tCount = 0;
    LDTaskSet tLDTaskSet = new LDTaskSet();
    LDTaskSchema tLDTaskSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDTaskDB";
    ...
    {
        tCount = 1;
        tLDTaskSchema = new LDTaskSchema();
        tLDTaskSchema.setSchema(mResultSet, 1);
