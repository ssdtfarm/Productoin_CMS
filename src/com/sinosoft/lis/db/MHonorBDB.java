...
import com.sinosoft.lis.schema.MHonorBSchema;
import com.sinosoft.lis.vschema.MHonorBSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: MHonorBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class MHonorBDB extends MHonorBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public MHonorBDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "MHonorB" );
		mflag = true;
	}
	...
	{
		MHonorBSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM MHonorB WHERE  IDX = ?");
		...
		SQLString sqlObj = new SQLString("MHonorB");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE MHonorB SET  IDX = ? , StatYear = ? , StatBatch = ? , AgentCode = ? , AgentName = ? , AgentGrade = ? , BranchAttr = ? , AwardType = ? , AwardName = ? , StartDate = ? , EndDate = ? , Remarks = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Operator1 = ? , MakeDate1 = ? , MakeTime1 = ? WHERE  IDX = ?");
			if(this.getIDX() == null || this.getIDX().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getIDX());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO MHonorB VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getIDX() == null || this.getIDX().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getIDX());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM MHonorB WHERE  IDX = ?", 
	...
	public MHonorBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		MHonorBSet aMHonorBSet = new MHonorBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 MHonorBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table MHonorB is querying for all data!");
			 }
			 mSql.append("select * from MHonorB ");
			...
			{
				i++;
				MHonorBSchema s1 = new MHonorBSchema();
				s1.setSchema(rs,i);
				aMHonorBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public MHonorBSet executeQuery(String sql, List bv)
	{
	...
	public MHonorBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		MHonorBSet aMHonorBSet = new MHonorBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("MHonorB");
			MHonorBSchema aSchema = this.getSchema();
	...
	public MHonorBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		MHonorBSet aMHonorBSet = new MHonorBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update MHonorB " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return MHonorBSet
 */
public MHonorBSet getData()
{
    int tCount = 0;
    MHonorBSet tMHonorBSet = new MHonorBSet();
    MHonorBSchema tMHonorBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "MHonorBDB";
    ...
    {
        tCount = 1;
        tMHonorBSchema = new MHonorBSchema();
        tMHonorBSchema.setSchema(mResultSet, 1);
        tMHonorBSet.add(tMHonorBSchema);
