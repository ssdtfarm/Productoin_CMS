...
/*
 * <p>ClassName: LARelationshipDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LARelationshipDB extends LARelationshipSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LARelationshipDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LARelationshipSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LARelationshipDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getRelationID() == null || this.getRelationID().equals("null")) {
				...
				pstmt.setString(1, this.getRelationID());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getIAgentCode() == null || this.getIAgentCode().equals("null")) {
				...
				pstmt.setString(7, this.getIAgentCode());
			}
			if(this.getIAgentGroup() == null || this.getIAgentGroup().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getIAgentGroup());
			}
			if(this.getIAgentGrade() == null || this.getIAgentGrade().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getIAgentGrade());
			}
			pstmt.setInt(10, this.getDeep());
			...
			if(this.getPBreakDate() == null || this.getPBreakDate().equals("null")) {
				...
				pstmt.setDate(14, Date.valueOf(this.getPBreakDate()));
			}
			pstmt.setInt(15, this.getINo());
			...
			if(this.getRelationID() == null || this.getRelationID().equals("null")) {
				...
				pstmt.setString(21, this.getRelationID());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LARelationship WHERE  RelationID = ?", 
	...
	public LARelationshipSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARelationshipSet aLARelationshipSet = new LARelationshipSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LARelationshipSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LARelationship is querying for all data!");
			 }
			 mSql.append("select * from LARelationship ");
			...
			{
				i++;
				LARelationshipSchema s1 = new LARelationshipSchema();
				s1.setSchema(rs,i);
				aLARelationshipSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLARelationshipSet;
	}

	public LARelationshipSet executeQuery(String sql, List bv)
	{
	...
	public LARelationshipSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARelationshipSet aLARelationshipSet = new LARelationshipSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LARelationship");
			LARelationshipSchema aSchema = this.getSchema();
	...
	public LARelationshipSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LARelationshipSet aLARelationshipSet = new LARelationshipSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LARelationship " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LARelationshipSet
 */
public LARelationshipSet getData()
{
    int tCount = 0;
    LARelationshipSet tLARelationshipSet = new LARelationshipSet();
    LARelationshipSchema tLARelationshipSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LARelationshipDB";
    ...
    {
        tCount = 1;
        tLARelationshipSchema = new LARelationshipSchema();
        tLARelationshipSchema.setSchema(mResultSet, 1);
        tLARelationshipSet.add(tLARelationshipSchema);
