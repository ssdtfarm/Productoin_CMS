...
/*
 * <p>ClassName: LAAgentFPUnApprovalBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: LAAgentFP_Setting
 */
public class LAAgentFPUnApprovalBDB extends LAAgentFPUnApprovalBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAAgentFPUnApprovalBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAAgentFPUnApprovalBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAAgentFPUnApprovalBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				...
				pstmt.setString(31, this.getOperator1());
			}
			// set where condition
			if(this.getAgentCode() == null || this.getAgentCode().equals("null")) {
				pstmt.setNull(32, 12);
			} else {
				pstmt.setString(32, this.getAgentCode());
			}
			if(this.getFinanceStartMonth() == null || this.getFinanceStartMonth().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
				pstmt.setString(33, this.getFinanceStartMonth());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(34, 91);
			} else {
				pstmt.setDate(34, Date.valueOf(this.getMakeDate1()));
			}
			if(this.getMakeTime1() == null || this.getMakeTime1().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getMakeTime1());
			}
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAgentFPUnApprovalB WHERE  AgentCode = ? AND FinanceStartMonth = ? AND MakeDate1 = ? AND MakeTime1 = ? AND Operator1 = ?", 
	...
	public LAAgentFPUnApprovalBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentFPUnApprovalBSet aLAAgentFPUnApprovalBSet = new LAAgentFPUnApprovalBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAAgentFPUnApprovalBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAAgentFPUnApprovalB is querying for all data!");
			 }
			 mSql.append("select * from LAAgentFPUnApprovalB ");
			...
			{
				i++;
				LAAgentFPUnApprovalBSchema s1 = new LAAgentFPUnApprovalBSchema();
				s1.setSchema(rs,i);
				aLAAgentFPUnApprovalBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAAgentFPUnApprovalBSet;
	}

	public LAAgentFPUnApprovalBSet executeQuery(String sql, List bv)
	{
	...
	public LAAgentFPUnApprovalBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentFPUnApprovalBSet aLAAgentFPUnApprovalBSet = new LAAgentFPUnApprovalBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAAgentFPUnApprovalB");
			LAAgentFPUnApprovalBSchema aSchema = this.getSchema();
	...
	public LAAgentFPUnApprovalBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAAgentFPUnApprovalBSet aLAAgentFPUnApprovalBSet = new LAAgentFPUnApprovalBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAAgentFPUnApprovalB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAAgentFPUnApprovalBSet
 */
public LAAgentFPUnApprovalBSet getData()
{
    int tCount = 0;
    LAAgentFPUnApprovalBSet tLAAgentFPUnApprovalBSet = new LAAgentFPUnApprovalBSet();
    LAAgentFPUnApprovalBSchema tLAAgentFPUnApprovalBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAAgentFPUnApprovalBDB";
    ...
    {
        tCount = 1;
        tLAAgentFPUnApprovalBSchema = new LAAgentFPUnApprovalBSchema();
        tLAAgentFPUnApprovalBSchema.setSchema(mResultSet, 1);
        tLAAgentFPUnApprovalBSet.add(tLAAgentFPUnApprovalBSchema);
    ...
    return tLAAgentFPUnApprovalBSet;
}
/**
 * 关闭数据集
 * @return boolean
