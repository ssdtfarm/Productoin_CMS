...
public class LDCOMDB extends LDCOMSchema
{
	...
	**/
	...
	public LDCOMSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LDCOMSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LDCOM is querying for all data!");
			 }
			 mSql.append("select * from LDCOM ");
	...
	public LDCOMSet executeQuery(String sql, List bv)
	{
	...
	public LDCOMSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LDCOMSet aLDCOMSet = new LDCOMSet();
