...
/*
 * <p>ClassName: DailyAFYCDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class DailyAFYCDB extends DailyAFYCSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public DailyAFYCDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		DailyAFYCSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(6, this.getAFYC());
			pstmt.setDouble(7, this.getAFYP());
			if(this.getTrx_Code() == null || this.getTrx_Code().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getTrx_Code());
			}
			if(this.getCalculation_Date() == null || this.getCalculation_Date().equals("null")) {
				pstmt.setNull(9, 91);
			} else {
				pstmt.setDate(9, Date.valueOf(this.getCalculation_Date()));
			}
			if(this.getBusiness_Date() == null || this.getBusiness_Date().equals("null")) {
				pstmt.setNull(10, 91);
			} else {
				pstmt.setDate(10, Date.valueOf(this.getBusiness_Date()));
			}
			pstmt.setDouble(11, this.getBatch_No());
			if(this.getBatch_Run_Date() == null || this.getBatch_Run_Date().equals("null")) {
				pstmt.setNull(12, 91);
			} else {
				pstmt.setDate(12, Date.valueOf(this.getBatch_Run_Date()));
			}
			// set where condition
			pstmt.executeUpdate();
			pstmt.close();
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM DailyAFYC WHERE ", 
	...
	public DailyAFYCSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		DailyAFYCSet aDailyAFYCSet = new DailyAFYCSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 DailyAFYCSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table DailyAFYC is querying for all data!");
			 }
			 mSql.append("select * from DailyAFYC ");
			...
			{
				i++;
				DailyAFYCSchema s1 = new DailyAFYCSchema();
				s1.setSchema(rs,i);
				aDailyAFYCSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aDailyAFYCSet;
	}

	public DailyAFYCSet executeQuery(String sql, List bv)
	{
	...
	public DailyAFYCSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		DailyAFYCSet aDailyAFYCSet = new DailyAFYCSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("DailyAFYC");
			DailyAFYCSchema aSchema = this.getSchema();
	...
	public DailyAFYCSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		DailyAFYCSet aDailyAFYCSet = new DailyAFYCSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update DailyAFYC " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return DailyAFYCSet
 */
public DailyAFYCSet getData()
{
    int tCount = 0;
    DailyAFYCSet tDailyAFYCSet = new DailyAFYCSet();
    DailyAFYCSchema tDailyAFYCSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "DailyAFYCDB";
    ...
    {
        tCount = 1;
        tDailyAFYCSchema = new DailyAFYCSchema();
        tDailyAFYCSchema.setSchema(mResultSet, 1);
        tDailyAFYCSet.add(tDailyAFYCSchema);
