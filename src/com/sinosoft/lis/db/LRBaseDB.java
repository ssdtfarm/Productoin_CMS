...
import com.sinosoft.lis.vschema.LRBaseSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LRBaseDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRBaseDB extends LRBaseSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRBaseDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LRBase" );
		mflag = true;
	}
	...
	{
		LRBaseSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LRBase WHERE  BaseCode = ?");
		...
		SQLString sqlObj = new SQLString("LRBase");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LRBase SET  BaseCode = ? , Name = ? , Status = ? , Remark = ? , Reason = ? , BranchType = ? , BranchType2 = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? WHERE  BaseCode = ?");
			if(this.getBaseCode() == null || this.getBaseCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBaseCode());
			...
			if(this.getBranchType() == null || this.getBranchType().equals("null")) {
				...
				pstmt.setString(6, this.getBranchType());
			}
			if(this.getBranchType2() == null || this.getBranchType2().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getBranchType2());
			}
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LRBase VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getBaseCode() == null || this.getBaseCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBaseCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRBase WHERE  BaseCode = ?", 
	...
	public LRBaseSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRBaseSet aLRBaseSet = new LRBaseSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRBaseSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRBase is querying for all data!");
			 }
			 mSql.append("select * from LRBase ");
			...
			{
				i++;
				LRBaseSchema s1 = new LRBaseSchema();
				s1.setSchema(rs,i);
				aLRBaseSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LRBaseSet executeQuery(String sql, List bv)
	{
	...
	public LRBaseSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRBaseSet aLRBaseSet = new LRBaseSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRBase");
			LRBaseSchema aSchema = this.getSchema();
	...
	public LRBaseSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRBaseSet aLRBaseSet = new LRBaseSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRBase " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LRBaseSet
 */
public LRBaseSet getData()
{
    int tCount = 0;
    LRBaseSet tLRBaseSet = new LRBaseSet();
    LRBaseSchema tLRBaseSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRBaseDB";
    ...
    {
        tCount = 1;
        tLRBaseSchema = new LRBaseSchema();
        tLRBaseSchema.setSchema(mResultSet, 1);
        tLRBaseSet.add(tLRBaseSchema);
