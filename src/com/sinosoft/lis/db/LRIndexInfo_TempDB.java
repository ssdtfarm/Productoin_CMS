...
/*
 * <p>ClassName: LRIndexInfo_TempDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRIndexInfo_TempDB extends LRIndexInfo_TempSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRIndexInfo_TempDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRIndexInfo_TempSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LRIndexInfo_TempDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRIndexInfo_Temp WHERE  WageNo = ? AND IndexType = ? AND BranchType = ? AND BranchType2 = ? AND BaseCode = ? AND IndexCode = ? AND AgentCode = ?", 
	...
	public LRIndexInfo_TempSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexInfo_TempSet aLRIndexInfo_TempSet = new LRIndexInfo_TempSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRIndexInfo_TempSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRIndexInfo_Temp is querying for all data!");
			 }
			 mSql.append("select * from LRIndexInfo_Temp ");
			...
			{
				i++;
				LRIndexInfo_TempSchema s1 = new LRIndexInfo_TempSchema();
				s1.setSchema(rs,i);
				aLRIndexInfo_TempSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLRIndexInfo_TempSet;
	}

	public LRIndexInfo_TempSet executeQuery(String sql, List bv)
	{
	...
	public LRIndexInfo_TempSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexInfo_TempSet aLRIndexInfo_TempSet = new LRIndexInfo_TempSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRIndexInfo_Temp");
			LRIndexInfo_TempSchema aSchema = this.getSchema();
	...
	public LRIndexInfo_TempSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRIndexInfo_TempSet aLRIndexInfo_TempSet = new LRIndexInfo_TempSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRIndexInfo_Temp " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LRIndexInfo_TempSet
 */
public LRIndexInfo_TempSet getData()
{
    int tCount = 0;
    LRIndexInfo_TempSet tLRIndexInfo_TempSet = new LRIndexInfo_TempSet();
    LRIndexInfo_TempSchema tLRIndexInfo_TempSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRIndexInfo_TempDB";
    ...
    {
        tCount = 1;
        tLRIndexInfo_TempSchema = new LRIndexInfo_TempSchema();
        tLRIndexInfo_TempSchema.setSchema(mResultSet, 1);
        tLRIndexInfo_TempSet.add(tLRIndexInfo_TempSchema);
