...
import com.sinosoft.lis.vschema.LAHOLSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LAHOLDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAHOLDB extends LAHOLSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAHOLDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LAHOL" );
		mflag = true;
	}
	...
	{
		LAHOLSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LAHOL WHERE  AgentCode = ? AND IDX = ?");
		...
		SQLString sqlObj = new SQLString("LAHOL");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LAHOL SET  AgentCode = ? , AgentGroup = ? , ManageCom = ? , IDX = ? , LeaveStartDate = ? , LeaveEndDate = ? , Remarks = ? , BranchType = ? , DiagCertificate = ? , CancelLeave = ? , LeaveType = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? WHERE  AgentCode = ? AND IDX = ?");
			if(this.getAgentCode() == null || this.getAgentCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getAgentCode());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LAHOL VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getAgentCode() == null || this.getAgentCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getAgentCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAHOL WHERE  AgentCode = ? AND IDX = ?", 
	...
	public LAHOLSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAHOLSet aLAHOLSet = new LAHOLSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAHOLSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAHOL is querying for all data!");
			 }
			 mSql.append("select * from LAHOL ");
			...
			{
				i++;
				LAHOLSchema s1 = new LAHOLSchema();
				s1.setSchema(rs,i);
				aLAHOLSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LAHOLSet executeQuery(String sql, List bv)
	{
	...
	public LAHOLSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAHOLSet aLAHOLSet = new LAHOLSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAHOL");
			LAHOLSchema aSchema = this.getSchema();
	...
	public LAHOLSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAHOLSet aLAHOLSet = new LAHOLSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAHOL " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LAHOLSet
 */
public LAHOLSet getData()
{
    int tCount = 0;
    LAHOLSet tLAHOLSet = new LAHOLSet();
    LAHOLSchema tLAHOLSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAHOLDB";
    ...
    {
        tCount = 1;
        tLAHOLSchema = new LAHOLSchema();
        tLAHOLSchema.setSchema(mResultSet, 1);
        tLAHOLSet.add(tLAHOLSchema);
