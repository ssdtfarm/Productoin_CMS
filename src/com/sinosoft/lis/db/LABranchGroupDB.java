...
/*
 * <p>ClassName: LABranchGroupDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LABranchGroupDB extends LABranchGroupSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LABranchGroupDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LABranchGroupSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LABranchGroupDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LABranchGroup WHERE  AgentGroup = ?", 
	...
	public LABranchGroupSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LABranchGroupSet aLABranchGroupSet = new LABranchGroupSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LABranchGroupSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LABranchGroup is querying for all data!");
			 }
			 mSql.append("select * from LABranchGroup ");
			...
			{
				i++;
				LABranchGroupSchema s1 = new LABranchGroupSchema();
				s1.setSchema(rs,i);
				aLABranchGroupSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLABranchGroupSet;
	}

	public LABranchGroupSet executeQuery(String sql, List bv)
	{
	...
	public LABranchGroupSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LABranchGroupSet aLABranchGroupSet = new LABranchGroupSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LABranchGroup");
			LABranchGroupSchema aSchema = this.getSchema();
	...
	public LABranchGroupSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LABranchGroupSet aLABranchGroupSet = new LABranchGroupSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LABranchGroup " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LABranchGroupSet
 */
public LABranchGroupSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLABranchGroupSchema.setSchema(mResultSet, 1);
        tLABranchGroupSet.add(tLABranchGroupSchema);
