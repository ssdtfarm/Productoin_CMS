...
/*
 * <p>ClassName: LecturertDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LecturertDB extends LecturertSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LecturertDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LecturertSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getLecturertCode() == null || this.getLecturertCode().equals("null")) {
				...
				pstmt.setString(24, this.getLecturertCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM Lecturert WHERE  LecturertCode = ?", 
	...
	public LecturertSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LecturertSet aLecturertSet = new LecturertSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LecturertSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table Lecturert is querying for all data!");
			 }
			 mSql.append("select * from Lecturert ");
			...
			{
				i++;
				LecturertSchema s1 = new LecturertSchema();
				s1.setSchema(rs,i);
				aLecturertSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLecturertSet;
	}

	public LecturertSet executeQuery(String sql, List bv)
	{
	...
	public LecturertSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LecturertSet aLecturertSet = new LecturertSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("Lecturert");
			LecturertSchema aSchema = this.getSchema();
	...
	public LecturertSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LecturertSet aLecturertSet = new LecturertSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update Lecturert " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LecturertSet
 */
public LecturertSet getData()
{
    int tCount = 0;
    LecturertSet tLecturertSet = new LecturertSet();
    LecturertSchema tLecturertSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LecturertDB";
    ...
    {
        tCount = 1;
        tLecturertSchema = new LecturertSchema();
        tLecturertSchema.setSchema(mResultSet, 1);
        tLecturertSet.add(tLecturertSchema);
