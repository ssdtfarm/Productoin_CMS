	...
	**/
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(10, this.getNodeOrder());
			// set where condition
			if(this.getNodeCode() == null || this.getNodeCode().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LDMenu VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getNodeCode() == null || this.getNodeCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getNodeCode());
	...
	public LDMenuSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LDMenuSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LDMenu is querying for all data!");
			 }
			 mSql.append("select * from LDMenu ");
	...
	public LDMenuSet executeQuery(String sql, List bv)
	{
	...
	public LDMenuSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LDMenuSet aLDMenuSet = new LDMenuSet();
