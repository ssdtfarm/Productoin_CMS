...
import com.sinosoft.lis.schema.LALoanSchema;
import com.sinosoft.lis.vschema.LALoanSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LALoanDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LALoanDB extends LALoanSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LALoanDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LALoan" );
		mflag = true;
	}
	...
	{
		LALoanSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LALoan WHERE  ID = ?");
		...
		SQLString sqlObj = new SQLString("LALoan");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LALoan VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getID() == null || this.getID().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getID());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LALoan WHERE  ID = ?", 
	...
	public LALoanSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LALoanSet aLALoanSet = new LALoanSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LALoanSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LALoan is querying for all data!");
			 }
			 mSql.append("select * from LALoan ");
			...
			{
				i++;
				LALoanSchema s1 = new LALoanSchema();
				s1.setSchema(rs,i);
				aLALoanSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LALoanSet executeQuery(String sql, List bv)
	{
	...
	public LALoanSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LALoanSet aLALoanSet = new LALoanSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LALoan");
			LALoanSchema aSchema = this.getSchema();
	...
	public LALoanSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LALoanSet aLALoanSet = new LALoanSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LALoan " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LALoanSet
 */
public LALoanSet getData()
{
    int tCount = 0;
    LALoanSet tLALoanSet = new LALoanSet();
    LALoanSchema tLALoanSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LALoanDB";
    ...
    {
        tCount = 1;
        tLALoanSchema = new LALoanSchema();
        tLALoanSchema.setSchema(mResultSet, 1);
        tLALoanSet.add(tLALoanSchema);
