...
import com.sinosoft.lis.schema.LARelationshipCSchema;
import com.sinosoft.lis.vschema.LARelationshipCSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LARelationshipCDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LARelationshipCDB extends LARelationshipCSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LARelationshipCDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LARelationshipC" );
		mflag = true;
	}

	public LARelationshipCDB()
	{
		con = null;
		db = new DBOper( "LARelationshipC" );
		mflag = false;
	}
	...
	{
		LARelationshipCSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LARelationshipCDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LARelationshipC WHERE  BackupMonth = ? AND BackType = ? AND RelationID = ? AND Operator1 = ? AND BackupDate = ? AND BackupTime = ?");
		...
		SQLString sqlObj = new SQLString("LARelationshipC");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LARelationshipC VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getBackupMonth() == null || this.getBackupMonth().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBackupMonth());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LARelationshipC WHERE  BackupMonth = ? AND BackType = ? AND RelationID = ? AND Operator1 = ? AND BackupDate = ? AND BackupTime = ?", 
	...
	public LARelationshipCSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARelationshipCSet aLARelationshipCSet = new LARelationshipCSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LARelationshipCSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LARelationshipC is querying for all data!");
			 }
			 mSql.append("select * from LARelationshipC ");
			...
			{
				i++;
				LARelationshipCSchema s1 = new LARelationshipCSchema();
				s1.setSchema(rs,i);
				aLARelationshipCSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLARelationshipCSet;
	}

	public LARelationshipCSet executeQuery(String sql, List bv)
	{
	...
	public LARelationshipCSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARelationshipCSet aLARelationshipCSet = new LARelationshipCSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LARelationshipC");
			LARelationshipCSchema aSchema = this.getSchema();
	...
	public LARelationshipCSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LARelationshipCSet aLARelationshipCSet = new LARelationshipCSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LARelationshipC " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LARelationshipCSet
 */
public LARelationshipCSet getData()
{
    int tCount = 0;
    LARelationshipCSet tLARelationshipCSet = new LARelationshipCSet();
    LARelationshipCSchema tLARelationshipCSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LARelationshipCDB";
    ...
    {
        tCount = 1;
        tLARelationshipCSchema = new LARelationshipCSchema();
        tLARelationshipCSchema.setSchema(mResultSet, 1);
        tLARelationshipCSet.add(tLARelationshipCSchema);
