...
/*
 * <p>ClassName: LAAgentGradeBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: LAAgentGrade_Setting
 */
public class LAAgentGradeBDB extends LAAgentGradeBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAAgentGradeBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAAgentGradeBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAAgentGradeBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getDummyGradeFlagl() == null || this.getDummyGradeFlagl().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getDummyGradeFlagl());
			}
			...
			if(this.getGradeCode() == null || this.getGradeCode().equals("null")) {
				...
				pstmt.setString(25, this.getGradeCode());
			}
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAgentGradeB WHERE  GradeCode = ? AND Operator1 = ? AND MakeDate1 = ? AND MakeTime1 = ?", 
	...
	public LAAgentGradeBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentGradeBSet aLAAgentGradeBSet = new LAAgentGradeBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAAgentGradeBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAAgentGradeB is querying for all data!");
			 }
			 mSql.append("select * from LAAgentGradeB ");
			...
			{
				i++;
				LAAgentGradeBSchema s1 = new LAAgentGradeBSchema();
				s1.setSchema(rs,i);
				aLAAgentGradeBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAAgentGradeBSet;
	}

	public LAAgentGradeBSet executeQuery(String sql, List bv)
	{
	...
	public LAAgentGradeBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentGradeBSet aLAAgentGradeBSet = new LAAgentGradeBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAAgentGradeB");
			LAAgentGradeBSchema aSchema = this.getSchema();
	...
	public LAAgentGradeBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAAgentGradeBSet aLAAgentGradeBSet = new LAAgentGradeBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAAgentGradeB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAAgentGradeBSet
 */
public LAAgentGradeBSet getData()
{
    int tCount = 0;
    LAAgentGradeBSet tLAAgentGradeBSet = new LAAgentGradeBSet();
    LAAgentGradeBSchema tLAAgentGradeBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAAgentGradeBDB";
    ...
    {
        tCount = 1;
        tLAAgentGradeBSchema = new LAAgentGradeBSchema();
        tLAAgentGradeBSchema.setSchema(mResultSet, 1);
        tLAAgentGradeBSet.add(tLAAgentGradeBSchema);
