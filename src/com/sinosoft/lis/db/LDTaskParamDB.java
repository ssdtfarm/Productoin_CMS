...
/*
 * <p>ClassName: LDTaskParamDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LDTaskParamDB extends LDTaskParamSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDTaskParamDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDTaskParamSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDTaskParamDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getParamName() == null || this.getParamName().equals("null")) {
				...
				pstmt.setString(3, this.getParamName());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getParamValue() == null || this.getParamValue().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getParamValue());
			}
			...
			if(this.getTaskCode() == null || this.getTaskCode().equals("null")) {
				...
				pstmt.setString(10, this.getTaskCode());
			}
			if(this.getTaskPlanCode() == null || this.getTaskPlanCode().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getTaskPlanCode());
			}
			if(this.getParamName() == null || this.getParamName().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LDTaskParam VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getTaskCode() == null || this.getTaskCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getTaskCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDTaskParam WHERE  TaskCode = ? AND TaskPlanCode = ? AND ParamName = ?", 
	...
	public LDTaskParamSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDTaskParamSet aLDTaskParamSet = new LDTaskParamSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LDTaskParamSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LDTaskParam is querying for all data!");
			 }
			 mSql.append("select * from LDTaskParam ");
			...
			{
				i++;
				LDTaskParamSchema s1 = new LDTaskParamSchema();
				s1.setSchema(rs,i);
				aLDTaskParamSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDTaskParamSet;
	}

	public LDTaskParamSet executeQuery(String sql, List bv)
	{
	...
	public LDTaskParamSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDTaskParamSet aLDTaskParamSet = new LDTaskParamSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDTaskParam");
			LDTaskParamSchema aSchema = this.getSchema();
	...
	public LDTaskParamSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LDTaskParamSet aLDTaskParamSet = new LDTaskParamSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDTaskParam " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDTaskParamSet
 */
public LDTaskParamSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLDTaskParamSchema.setSchema(mResultSet, 1);
