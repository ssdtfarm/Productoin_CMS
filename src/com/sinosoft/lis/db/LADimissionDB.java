...
/*
 * <p>ClassName: LADimissionDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LADimissionDB extends LADimissionSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LADimissionDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LADimissionSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LADimissionDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LADimission WHERE  AgentCode = ?", 
	...
	public LADimissionSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LADimissionSet aLADimissionSet = new LADimissionSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LADimissionSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LADimission is querying for all data!");
			 }
			 mSql.append("select * from LADimission ");
			...
			{
				i++;
				LADimissionSchema s1 = new LADimissionSchema();
				s1.setSchema(rs,i);
				aLADimissionSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLADimissionSet;
	}

	public LADimissionSet executeQuery(String sql, List bv)
	{
	...
	public LADimissionSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LADimissionSet aLADimissionSet = new LADimissionSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LADimission");
			LADimissionSchema aSchema = this.getSchema();
	...
	public LADimissionSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LADimissionSet aLADimissionSet = new LADimissionSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LADimission " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LADimissionSet
 */
public LADimissionSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLADimissionSchema.setSchema(mResultSet, 1);
        tLADimissionSet.add(tLADimissionSchema);
