...
/*
 * <p>ClassName: LAClassOpenDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAClassOpenDB extends LAClassOpenSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAClassOpenDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAClassOpenSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAClassOpenDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOpenClassID() == null || this.getOpenClassID().equals("null")) {
				...
				pstmt.setString(15, this.getOpenClassID());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAClassOpen WHERE  OpenClassID = ?", 
	...
	public LAClassOpenSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAClassOpenSet aLAClassOpenSet = new LAClassOpenSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAClassOpenSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAClassOpen is querying for all data!");
			 }
			 mSql.append("select * from LAClassOpen ");
			...
			{
				i++;
				LAClassOpenSchema s1 = new LAClassOpenSchema();
				s1.setSchema(rs,i);
				aLAClassOpenSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAClassOpenSet;
	}

	public LAClassOpenSet executeQuery(String sql, List bv)
	{
	...
	public LAClassOpenSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAClassOpenSet aLAClassOpenSet = new LAClassOpenSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAClassOpen");
			LAClassOpenSchema aSchema = this.getSchema();
	...
	public LAClassOpenSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAClassOpenSet aLAClassOpenSet = new LAClassOpenSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAClassOpen " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAClassOpenSet
 */
public LAClassOpenSet getData()
{
    int tCount = 0;
    LAClassOpenSet tLAClassOpenSet = new LAClassOpenSet();
    LAClassOpenSchema tLAClassOpenSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAClassOpenDB";
    ...
    {
        tCount = 1;
        tLAClassOpenSchema = new LAClassOpenSchema();
        tLAClassOpenSchema.setSchema(mResultSet, 1);
        tLAClassOpenSet.add(tLAClassOpenSchema);
