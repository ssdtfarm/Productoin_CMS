...
/*
 * <p>ClassName: LACommisionBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LACommisionBDB extends LACommisionBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LACommisionBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LACommisionBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LACommisionBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				...
				pstmt.setString(57, this.getOperator1());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(58, 91);
			} else {
				pstmt.setDate(58, Date.valueOf(this.getMakeDate1()));
			}
			if(this.getMakeTime1() == null || this.getMakeTime1().equals("null")) {
				pstmt.setNull(59, 12);
			} else {
				pstmt.setString(59, this.getMakeTime1());
			}
			// set where condition
			if(this.getCommisionSN() == null || this.getCommisionSN().equals("null")) {
				pstmt.setNull(60, 12);
			} else {
				pstmt.setString(60, this.getCommisionSN());
			}
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				pstmt.setNull(61, 12);
			} else {
				pstmt.setString(61, this.getOperator1());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(62, 91);
			} else {
				pstmt.setDate(62, Date.valueOf(this.getMakeDate1()));
			}
			if(this.getMakeTime1() == null || this.getMakeTime1().equals("null")) {
				pstmt.setNull(63, 12);
			} else {
				pstmt.setString(63, this.getMakeTime1());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LACommisionB WHERE  CommisionSN = ? AND Operator1 = ? AND MakeDate1 = ? AND MakeTime1 = ?", 
	...
	public LACommisionBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LACommisionBSet aLACommisionBSet = new LACommisionBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LACommisionBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LACommisionB is querying for all data!");
			 }
			 mSql.append("select * from LACommisionB ");
			...
			{
				i++;
				LACommisionBSchema s1 = new LACommisionBSchema();
				s1.setSchema(rs,i);
				aLACommisionBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLACommisionBSet;
	}

	public LACommisionBSet executeQuery(String sql, List bv)
	{
	...
	public LACommisionBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LACommisionBSet aLACommisionBSet = new LACommisionBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LACommisionB");
			LACommisionBSchema aSchema = this.getSchema();
	...
	public LACommisionBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LACommisionBSet aLACommisionBSet = new LACommisionBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LACommisionB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LACommisionBSet
 */
public LACommisionBSet getData()
{
    int tCount = 0;
    LACommisionBSet tLACommisionBSet = new LACommisionBSet();
    LACommisionBSchema tLACommisionBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LACommisionBDB";
    ...
    try
    {
        ...
        tLACommisionBSchema.setSchema(mResultSet, 1);
