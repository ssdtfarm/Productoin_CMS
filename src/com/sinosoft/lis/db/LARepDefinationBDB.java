...
/*
 * <p>ClassName: LARepDefinationBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LARepDefinationBDB extends LARepDefinationBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LARepDefinationBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LARepDefinationBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LARepDefinationBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				...
				pstmt.setDate(12, Date.valueOf(this.getMakeDate1()));
			}
			if(this.getMakeTime1() == null || this.getMakeTime1().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getMakeTime1());
			}
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getOperator1());
			}
			// set where condition
			if(this.getReportName() == null || this.getReportName().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getReportName());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(16, 91);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LARepDefinationB WHERE  ReportName = ? AND MakeDate1 = ? AND MakeTime1 = ? AND Operator1 = ?", 
	...
	public LARepDefinationBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARepDefinationBSet aLARepDefinationBSet = new LARepDefinationBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LARepDefinationBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LARepDefinationB is querying for all data!");
			 }
			 mSql.append("select * from LARepDefinationB ");
			...
			{
				i++;
				LARepDefinationBSchema s1 = new LARepDefinationBSchema();
				s1.setSchema(rs,i);
				aLARepDefinationBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLARepDefinationBSet;
	}

	public LARepDefinationBSet executeQuery(String sql, List bv)
	{
	...
	public LARepDefinationBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARepDefinationBSet aLARepDefinationBSet = new LARepDefinationBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LARepDefinationB");
			LARepDefinationBSchema aSchema = this.getSchema();
	...
	public LARepDefinationBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LARepDefinationBSet aLARepDefinationBSet = new LARepDefinationBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LARepDefinationB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LARepDefinationBSet
 */
public LARepDefinationBSet getData()
{
    int tCount = 0;
    LARepDefinationBSet tLARepDefinationBSet = new LARepDefinationBSet();
    LARepDefinationBSchema tLARepDefinationBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LARepDefinationBDB";
    ...
    {
        tCount = 1;
        tLARepDefinationBSchema = new LARepDefinationBSchema();
        tLARepDefinationBSchema.setSchema(mResultSet, 1);
        tLARepDefinationBSet.add(tLARepDefinationBSchema);
