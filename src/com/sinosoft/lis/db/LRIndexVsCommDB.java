...
/*
 * <p>ClassName: LRIndexVsCommDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRIndexVsCommDB extends LRIndexVsCommSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRIndexVsCommDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRIndexVsCommSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LRIndexVsCommDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRIndexVsComm WHERE  BaseCode = ? AND ManageCom = ? AND AgentGrade = ? AND IndexType = ? AND WageCode = ?", 
	...
	public LRIndexVsCommSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexVsCommSet aLRIndexVsCommSet = new LRIndexVsCommSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRIndexVsCommSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRIndexVsComm is querying for all data!");
			 }
			 mSql.append("select * from LRIndexVsComm ");
			...
			{
				i++;
				LRIndexVsCommSchema s1 = new LRIndexVsCommSchema();
				s1.setSchema(rs,i);
				aLRIndexVsCommSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLRIndexVsCommSet;
	}

	public LRIndexVsCommSet executeQuery(String sql, List bv)
	{
	...
	public LRIndexVsCommSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexVsCommSet aLRIndexVsCommSet = new LRIndexVsCommSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRIndexVsComm");
			LRIndexVsCommSchema aSchema = this.getSchema();
	...
	public LRIndexVsCommSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRIndexVsCommSet aLRIndexVsCommSet = new LRIndexVsCommSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRIndexVsComm " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LRIndexVsCommSet
 */
public LRIndexVsCommSet getData()
{
    int tCount = 0;
    LRIndexVsCommSet tLRIndexVsCommSet = new LRIndexVsCommSet();
    LRIndexVsCommSchema tLRIndexVsCommSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRIndexVsCommDB";
    ...
    {
        tCount = 1;
        tLRIndexVsCommSchema = new LRIndexVsCommSchema();
        tLRIndexVsCommSchema.setSchema(mResultSet, 1);
        tLRIndexVsCommSet.add(tLRIndexVsCommSchema);
