...
public class LDCodeDB extends LDCodeSchema
{
	...
	**/
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LDCode SET  CodeType = ? , Code = ? , CodeName = ? , CodeAlias = ? , ComCode = ? , OtherSign = ? WHERE  CodeType = ? AND Code = ?");
			if(this.getCodeType() == null || this.getCodeType().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCodeType());
			...
			if(this.getCodeAlias() == null || this.getCodeAlias().equals("null")) {
				...
				pstmt.setString(4, this.getCodeAlias());
			}
			...
			if(this.getOtherSign() == null || this.getOtherSign().equals("null")) {
				...
				pstmt.setString(6, this.getOtherSign());
			}
			// set where condition
			if(this.getCodeType() == null || this.getCodeType().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getCodeType());
			}
			if(this.getCode() == null || this.getCode().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDCode WHERE  CodeType = ? AND Code = ?", 
	...
	public LDCodeSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LDCodeSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LDCode is querying for all data!");
			 }
			 mSql.append("select * from LDCode ");
	...
	public LDCodeSet executeQuery(String sql, List bv)
	{
	...
	public LDCodeSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LDCodeSet aLDCodeSet = new LDCodeSet();
...
public LDCodeSet getData()
{
    ...
    try
    {
        ...
        tLDCodeSchema.setSchema(mResultSet, 1);
