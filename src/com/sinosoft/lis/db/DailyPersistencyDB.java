...
/*
 * <p>ClassName: DailyPersistencyDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class DailyPersistencyDB extends DailyPersistencySchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public DailyPersistencyDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		DailyPersistencySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "DailyPersistencyDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(6, this.getLapse__ANP_At_Issue());
			pstmt.setDouble(7, this.getExposure__ANP_At_Issue());
			if(this.getCalculation_Date() == null || this.getCalculation_Date().equals("null")) {
				pstmt.setNull(8, 91);
			} else {
				pstmt.setDate(8, Date.valueOf(this.getCalculation_Date()));
			}
			if(this.getBusiness_Date() == null || this.getBusiness_Date().equals("null")) {
				pstmt.setNull(9, 91);
			} else {
				pstmt.setDate(9, Date.valueOf(this.getBusiness_Date()));
			}
			pstmt.setDouble(10, this.getBatch_No());
			if(this.getBatch_Run_Date() == null || this.getBatch_Run_Date().equals("null")) {
				pstmt.setNull(11, 91);
			} else {
				pstmt.setDate(11, Date.valueOf(this.getBatch_Run_Date()));
			}
			// set where condition
			pstmt.executeUpdate();
			pstmt.close();
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM DailyPersistency WHERE ", 
	...
	public DailyPersistencySet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		DailyPersistencySet aDailyPersistencySet = new DailyPersistencySet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 DailyPersistencySchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table DailyPersistency is querying for all data!");
			 }
			 mSql.append("select * from DailyPersistency ");
			...
			{
				i++;
				DailyPersistencySchema s1 = new DailyPersistencySchema();
				s1.setSchema(rs,i);
				aDailyPersistencySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aDailyPersistencySet;
	}

	public DailyPersistencySet executeQuery(String sql, List bv)
	{
	...
	public DailyPersistencySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		DailyPersistencySet aDailyPersistencySet = new DailyPersistencySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("DailyPersistency");
			DailyPersistencySchema aSchema = this.getSchema();
	...
	public DailyPersistencySet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		DailyPersistencySet aDailyPersistencySet = new DailyPersistencySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update DailyPersistency " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return DailyPersistencySet
 */
public DailyPersistencySet getData()
{
    int tCount = 0;
    DailyPersistencySet tDailyPersistencySet = new DailyPersistencySet();
    DailyPersistencySchema tDailyPersistencySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "DailyPersistencyDB";
    ...
    {
        tCount = 1;
        tDailyPersistencySchema = new DailyPersistencySchema();
        tDailyPersistencySchema.setSchema(mResultSet, 1);
        tDailyPersistencySet.add(tDailyPersistencySchema);
