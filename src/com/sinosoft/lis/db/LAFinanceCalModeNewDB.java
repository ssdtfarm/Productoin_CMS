...
/*
 * <p>ClassName: LAFinanceCalModeNewDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAFinanceCalModeNewDB extends LAFinanceCalModeNewSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAFinanceCalModeNewDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAFinanceCalModeNewSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAFinanceCalModeNewDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getTitleCode() == null || this.getTitleCode().equals("null")) {
				...
				pstmt.setString(2, this.getTitleCode());
			}
			pstmt.setInt(3, this.getSubjectOrder());
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSubjectCode() == null || this.getSubjectCode().equals("null")) {
				...
				pstmt.setString(4, this.getSubjectCode());
			}
			if(this.getSubjectName() == null || this.getSubjectName().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getSubjectName());
			}
			if(this.getAgentSeries() == null || this.getAgentSeries().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getAgentSeries());
			}
			if(this.getGradeSeries() == null || this.getGradeSeries().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getGradeSeries());
			}
			...
			pstmt.setInt(9, this.getMonthDeviation());
			if(this.getOperation() == null || this.getOperation().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getOperation());
			}
			...
			if(this.getB1() == null || this.getB1().equals("null")) {
				...
				pstmt.setString(12, this.getB1());
			}
			if(this.getB2() == null || this.getB2().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getB2());
			}
			if(this.getB3() == null || this.getB3().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getB3());
			}
			if(this.getOperator() == null || this.getOperator().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
			...
			pstmt.setInt(22, this.getSubjectOrder());
			if(this.getOperator() == null || this.getOperator().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAFinanceCalModeNew WHERE  ReportCode = ? AND TitleCode = ? AND SubjectOrder = ? AND Operator = ? AND MakeDate = ? AND MakeTime = ? AND ModifyDate = ? AND ModifyTime = ?", 
	...
	public LAFinanceCalModeNewSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAFinanceCalModeNewSet aLAFinanceCalModeNewSet = new LAFinanceCalModeNewSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAFinanceCalModeNewSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAFinanceCalModeNew is querying for all data!");
			 }
			 mSql.append("select * from LAFinanceCalModeNew ");
			...
			{
				i++;
				LAFinanceCalModeNewSchema s1 = new LAFinanceCalModeNewSchema();
				s1.setSchema(rs,i);
				aLAFinanceCalModeNewSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAFinanceCalModeNewSet;
	}

	public LAFinanceCalModeNewSet executeQuery(String sql, List bv)
	{
	...
	public LAFinanceCalModeNewSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAFinanceCalModeNewSet aLAFinanceCalModeNewSet = new LAFinanceCalModeNewSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAFinanceCalModeNew");
			LAFinanceCalModeNewSchema aSchema = this.getSchema();
	...
	public LAFinanceCalModeNewSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAFinanceCalModeNewSet aLAFinanceCalModeNewSet = new LAFinanceCalModeNewSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAFinanceCalModeNew " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAFinanceCalModeNewSet
 */
public LAFinanceCalModeNewSet getData()
{
    int tCount = 0;
    LAFinanceCalModeNewSet tLAFinanceCalModeNewSet = new LAFinanceCalModeNewSet();
    LAFinanceCalModeNewSchema tLAFinanceCalModeNewSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAFinanceCalModeNewDB";
    ...
    {
        tCount = 1;
        tLAFinanceCalModeNewSchema = new LAFinanceCalModeNewSchema();
        tLAFinanceCalModeNewSchema.setSchema(mResultSet, 1);
        tLAFinanceCalModeNewSet.add(tLAFinanceCalModeNewSchema);
    ...
    return tLAFinanceCalModeNewSet;
}
/**
 * 关闭数据集
 * @return boolean
