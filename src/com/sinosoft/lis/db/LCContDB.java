...
public class LCContDB extends LCContSchema
{
	...
	**/
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCCont WHERE  CompanyCode = ? AND MainPolNo = ?");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCompanyCode() == null || this.getCompanyCode().equals("null")) {
				...
				pstmt.setString(37, this.getCompanyCode());
			}
			if(this.getMainPolNo() == null || this.getMainPolNo().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getMainPolNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCCont WHERE  CompanyCode = ? AND MainPolNo = ?", 
	...
	public LCContSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LCContSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LCCont is querying for all data!");
			 }
			 mSql.append("select * from LCCont ");
	...
	public LCContSet executeQuery(String sql, List bv)
	{
	...
	public LCContSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LCContSet aLCContSet = new LCContSet();
