...
/*
 * <p>ClassName: LDMaxNoDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LDMaxNoDB extends LDMaxNoSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDMaxNoDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDMaxNoSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getNoType() == null || this.getNoType().equals("null")) {
				...
				pstmt.setString(1, this.getNoType());
			}
			if(this.getNoLimit() == null || this.getNoLimit().equals("null")) {
				pstmt.setNull(2, 12);
			} else {
				pstmt.setString(2, this.getNoLimit());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
		...
		SQLString sqlObj = new SQLString("LDMaxNo");
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(3, this.getMaxNo());
			// set where condition
			if(this.getNoType() == null || this.getNoType().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getNoType());
			}
			if(this.getNoLimit() == null || this.getNoLimit().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getNoLimit());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDMaxNo WHERE  NoType = ? AND NoLimit = ?", 
	...
	public LDMaxNoSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDMaxNoSet aLDMaxNoSet = new LDMaxNoSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LDMaxNoSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LDMaxNo is querying for all data!");
			 }
			 mSql.append("select * from LDMaxNo ");
			...
			{
				i++;
				LDMaxNoSchema s1 = new LDMaxNoSchema();
				s1.setSchema(rs,i);
				aLDMaxNoSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LDMaxNoSet executeQuery(String sql, List bv)
	{
	...
	public LDMaxNoSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDMaxNoSet aLDMaxNoSet = new LDMaxNoSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDMaxNo");
			LDMaxNoSchema aSchema = this.getSchema();
	...
	public LDMaxNoSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LDMaxNoSet aLDMaxNoSet = new LDMaxNoSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDMaxNo " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LDMaxNoSet
 */
public LDMaxNoSet getData()
{
    int tCount = 0;
    LDMaxNoSet tLDMaxNoSet = new LDMaxNoSet();
    LDMaxNoSchema tLDMaxNoSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDMaxNoDB";
    ...
    {
        tCount = 1;
        tLDMaxNoSchema = new LDMaxNoSchema();
        tLDMaxNoSchema.setSchema(mResultSet, 1);
        tLDMaxNoSet.add(tLDMaxNoSchema);
