...
/*
 * <p>ClassName: LAStatSegmentDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAStatSegmentDB extends LAStatSegmentSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAStatSegmentDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAStatSegmentSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAStatSegmentDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getStatType() == null || this.getStatType().equals("null")) {
				...
				pstmt.setString(2, this.getStatType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getYearMonth() == null || this.getYearMonth().equals("null")) {
				...
				pstmt.setString(10, this.getYearMonth());
			}
			if(this.getStatType() == null || this.getStatType().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getStatType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAStatSegment WHERE  YearMonth = ? AND StatType = ?", 
	...
	public LAStatSegmentSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAStatSegmentSet aLAStatSegmentSet = new LAStatSegmentSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAStatSegmentSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAStatSegment is querying for all data!");
			 }
			 mSql.append("select * from LAStatSegment ");
			...
			{
				i++;
				LAStatSegmentSchema s1 = new LAStatSegmentSchema();
				s1.setSchema(rs,i);
				aLAStatSegmentSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAStatSegmentSet;
	}

	public LAStatSegmentSet executeQuery(String sql, List bv)
	{
	...
	public LAStatSegmentSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAStatSegmentSet aLAStatSegmentSet = new LAStatSegmentSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAStatSegment");
			LAStatSegmentSchema aSchema = this.getSchema();
	...
	public LAStatSegmentSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAStatSegmentSet aLAStatSegmentSet = new LAStatSegmentSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAStatSegment " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAStatSegmentSet
 */
public LAStatSegmentSet getData()
{
    int tCount = 0;
    LAStatSegmentSet tLAStatSegmentSet = new LAStatSegmentSet();
    LAStatSegmentSchema tLAStatSegmentSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAStatSegmentDB";
    ...
    {
        tCount = 1;
        tLAStatSegmentSchema = new LAStatSegmentSchema();
        tLAStatSegmentSchema.setSchema(mResultSet, 1);
        tLAStatSegmentSet.add(tLAStatSegmentSchema);
