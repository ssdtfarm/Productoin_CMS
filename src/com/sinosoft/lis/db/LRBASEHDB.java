...
/*
 * <p>ClassName: LRBASEHDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRBASEHDB extends LRBASEHSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRBASEHDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRBASEHSchema tSchema = this.getSchema();
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRBASEH WHERE ", 
	...
	public LRBASEHSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRBASEHSet aLRBASEHSet = new LRBASEHSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRBASEHSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRBASEH is querying for all data!");
			 }
			 mSql.append("select * from LRBASEH ");
			...
			{
				i++;
				LRBASEHSchema s1 = new LRBASEHSchema();
				s1.setSchema(rs,i);
				aLRBASEHSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LRBASEHSet executeQuery(String sql, List bv)
	{
	...
	public LRBASEHSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRBASEHSet aLRBASEHSet = new LRBASEHSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRBASEH");
			LRBASEHSchema aSchema = this.getSchema();
	...
	public LRBASEHSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRBASEHSet aLRBASEHSet = new LRBASEHSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRBASEH " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LRBASEHSet
 */
public LRBASEHSet getData()
{
    int tCount = 0;
    LRBASEHSet tLRBASEHSet = new LRBASEHSet();
    LRBASEHSchema tLRBASEHSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRBASEHDB";
    ...
    {
        tCount = 1;
        tLRBASEHSchema = new LRBASEHSchema();
        tLRBASEHSchema.setSchema(mResultSet, 1);
        tLRBASEHSet.add(tLRBASEHSchema);
