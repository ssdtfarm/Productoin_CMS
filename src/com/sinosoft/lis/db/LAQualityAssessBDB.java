...
/*
 * <p>ClassName: LAQualityAssessBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAQualityAssessBDB extends LAQualityAssessBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAQualityAssessBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAQualityAssessBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAQualityAssessBDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAQualityAssessB WHERE  AgentCode = ? AND MakeDate1 = ? AND MakeTime1 = ? AND Operator1 = ?", 
	...
	public LAQualityAssessBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAQualityAssessBSet aLAQualityAssessBSet = new LAQualityAssessBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAQualityAssessBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAQualityAssessB is querying for all data!");
			 }
			 mSql.append("select * from LAQualityAssessB ");
			...
			{
				i++;
				LAQualityAssessBSchema s1 = new LAQualityAssessBSchema();
				s1.setSchema(rs,i);
				aLAQualityAssessBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAQualityAssessBSet;
	}

	public LAQualityAssessBSet executeQuery(String sql, List bv)
	{
	...
	public LAQualityAssessBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAQualityAssessBSet aLAQualityAssessBSet = new LAQualityAssessBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAQualityAssessB");
			LAQualityAssessBSchema aSchema = this.getSchema();
	...
	public LAQualityAssessBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAQualityAssessBSet aLAQualityAssessBSet = new LAQualityAssessBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAQualityAssessB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAQualityAssessBSet
 */
public LAQualityAssessBSet getData()
{
    int tCount = 0;
    LAQualityAssessBSet tLAQualityAssessBSet = new LAQualityAssessBSet();
    LAQualityAssessBSchema tLAQualityAssessBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAQualityAssessBDB";
    ...
    {
        tCount = 1;
        tLAQualityAssessBSchema = new LAQualityAssessBSchema();
        tLAQualityAssessBSchema.setSchema(mResultSet, 1);
        tLAQualityAssessBSet.add(tLAQualityAssessBSchema);
