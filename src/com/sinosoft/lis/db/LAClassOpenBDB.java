...
/*
 * <p>ClassName: LAClassOpenBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAClassOpenBDB extends LAClassOpenBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAClassOpenBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAClassOpenBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAClassOpenBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getTrainStartDate() == null || this.getTrainStartDate().equals("null")) {
				pstmt.setNull(4, 91);
			} else {
				pstmt.setDate(4, Date.valueOf(this.getTrainStartDate()));
			}
			if(this.getTrainEndDate() == null || this.getTrainEndDate().equals("null")) {
				pstmt.setNull(5, 91);
			} else {
				pstmt.setDate(5, Date.valueOf(this.getTrainEndDate()));
			}
			...
			if(this.getManagecomName() == null || this.getManagecomName().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getManagecomName());
			}
			...
			if(this.getOpenClassID() == null || this.getOpenClassID().equals("null")) {
				...
				pstmt.setString(18, this.getOpenClassID());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(19, 91);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAClassOpenB WHERE  OpenClassID = ? AND MakeDate1 = ? AND MakeTime1 = ? AND Operator1 = ?", 
	...
	public LAClassOpenBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAClassOpenBSet aLAClassOpenBSet = new LAClassOpenBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAClassOpenBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAClassOpenB is querying for all data!");
			 }
			 mSql.append("select * from LAClassOpenB ");
			...
			{
				i++;
				LAClassOpenBSchema s1 = new LAClassOpenBSchema();
				s1.setSchema(rs,i);
				aLAClassOpenBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAClassOpenBSet;
	}

	public LAClassOpenBSet executeQuery(String sql, List bv)
	{
	...
	public LAClassOpenBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAClassOpenBSet aLAClassOpenBSet = new LAClassOpenBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAClassOpenB");
			LAClassOpenBSchema aSchema = this.getSchema();
	...
	public LAClassOpenBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAClassOpenBSet aLAClassOpenBSet = new LAClassOpenBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAClassOpenB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAClassOpenBSet
 */
public LAClassOpenBSet getData()
{
    int tCount = 0;
    LAClassOpenBSet tLAClassOpenBSet = new LAClassOpenBSet();
    LAClassOpenBSchema tLAClassOpenBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAClassOpenBDB";
    ...
    {
        tCount = 1;
        tLAClassOpenBSchema = new LAClassOpenBSchema();
        tLAClassOpenBSchema.setSchema(mResultSet, 1);
        tLAClassOpenBSet.add(tLAClassOpenBSchema);
