...
import com.sinosoft.lis.schema.LDCOMBSchema;
import com.sinosoft.lis.vschema.LDCOMBSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LDCOMBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LDCOMBDB extends LDCOMBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDCOMBDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LDCOMB" );
		mflag = true;
	}
	...
	{
		LDCOMBSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LDCOMB WHERE  COMCODE = ? AND OPERATOR1 = ? AND MAKEDATE1 = ? AND MAKETIME1 = ?");
		...
		SQLString sqlObj = new SQLString("LDCOMB");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LDCOMB SET  COMCODE = ? , NAME = ? , SHORTNAME = ? , COMGRADE = ? , UPCOMCODE = ? , CITYCODE = ? , CITYTYPE = ? , AREACODE = ? , FOUNDDATE = ? , ADDRESS = ? , ZIPCODE = ? , PHONE = ? , FAX = ? , EMAIL = ? , SIGN = ? , OPERATOR = ? , MAKEDATE = ? , MAKETIME = ? , MODIFYDATE = ? , MODIFYTIME = ? , OPERATOR1 = ? , MAKEDATE1 = ? , MAKETIME1 = ? WHERE  COMCODE = ? AND OPERATOR1 = ? AND MAKEDATE1 = ? AND MAKETIME1 = ?");
			if(this.getCOMCODE() == null || this.getCOMCODE().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCOMCODE());
			...
			if(this.getSHORTNAME() == null || this.getSHORTNAME().equals("null")) {
				...
				pstmt.setString(3, this.getSHORTNAME());
			}
			if(this.getCOMGRADE() == null || this.getCOMGRADE().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getCOMGRADE());
			}
			if(this.getUPCOMCODE() == null || this.getUPCOMCODE().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getUPCOMCODE());
			}
			if(this.getCITYCODE() == null || this.getCITYCODE().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getCITYCODE());
			}
			if(this.getCITYTYPE() == null || this.getCITYTYPE().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getCITYTYPE());
			}
			if(this.getAREACODE() == null || this.getAREACODE().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getAREACODE());
			}
			if(this.getFOUNDDATE() == null || this.getFOUNDDATE().equals("null")) {
				pstmt.setNull(9, 91);
			} else {
				pstmt.setDate(9, Date.valueOf(this.getFOUNDDATE()));
			}
			if(this.getADDRESS() == null || this.getADDRESS().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getADDRESS());
			}
			...
			if(this.getEMAIL() == null || this.getEMAIL().equals("null")) {
				...
				pstmt.setString(14, this.getEMAIL());
			}
			if(this.getSIGN() == null || this.getSIGN().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getSIGN());
			}
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LDCOMB VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getCOMCODE() == null || this.getCOMCODE().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCOMCODE());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDCOMB WHERE  COMCODE = ? AND OPERATOR1 = ? AND MAKEDATE1 = ? AND MAKETIME1 = ?", 
	...
	public LDCOMBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDCOMBSet aLDCOMBSet = new LDCOMBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LDCOMBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LDCOMB is querying for all data!");
			 }
			 mSql.append("select * from LDCOMB ");
			...
			{
				i++;
				LDCOMBSchema s1 = new LDCOMBSchema();
				s1.setSchema(rs,i);
				aLDCOMBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LDCOMBSet executeQuery(String sql, List bv)
	{
	...
	public LDCOMBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDCOMBSet aLDCOMBSet = new LDCOMBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDCOMB");
			LDCOMBSchema aSchema = this.getSchema();
	...
	public LDCOMBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LDCOMBSet aLDCOMBSet = new LDCOMBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDCOMB " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LDCOMBSet
 */
public LDCOMBSet getData()
{
    int tCount = 0;
    LDCOMBSet tLDCOMBSet = new LDCOMBSet();
    LDCOMBSchema tLDCOMBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDCOMBDB";
    ...
    {
        tCount = 1;
        tLDCOMBSchema = new LDCOMBSchema();
        tLDCOMBSchema.setSchema(mResultSet, 1);
        tLDCOMBSet.add(tLDCOMBSchema);
