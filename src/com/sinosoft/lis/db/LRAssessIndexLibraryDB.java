...
/*
 * <p>ClassName: LRAssessIndexLibraryDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRAssessIndexLibraryDB extends LRAssessIndexLibrarySchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRAssessIndexLibraryDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRAssessIndexLibrarySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LRAssessIndexLibraryDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getIndexCode() == null || this.getIndexCode().equals("null")) {
				...
				pstmt.setString(1, this.getIndexCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCalType() == null || this.getCalType().equals("null")) {
				...
				pstmt.setString(8, this.getCalType());
			}
			...
			if(this.getResultType() == null || this.getResultType().equals("null")) {
				...
				pstmt.setString(10, this.getResultType());
			}
			if(this.getCalPrpty() == null || this.getCalPrpty().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getCalPrpty());
			}
			if(this.getDefaultValue() == null || this.getDefaultValue().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getDefaultValue());
			}
			if(this.getKind() == null || this.getKind().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getKind());
			}
			...
			if(this.getCalSql() == null || this.getCalSql().equals("null")) {
				...
				pstmt.setString(15, this.getCalSql());
			}
			if(this.getSqlTemp() == null || this.getSqlTemp().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getSqlTemp());
			}
			if(this.getJson() == null || this.getJson().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getJson());
			}
			...
			if(this.getBranchType() == null || this.getBranchType().equals("null")) {
				...
				pstmt.setString(19, this.getBranchType());
			}
			if(this.getBranchType2() == null || this.getBranchType2().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getBranchType2());
			}
			// set where condition
			if(this.getIndexCode() == null || this.getIndexCode().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getIndexCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRAssessIndexLibrary WHERE  IndexCode = ?", 
	...
	public LRAssessIndexLibrarySet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRAssessIndexLibrarySet aLRAssessIndexLibrarySet = new LRAssessIndexLibrarySet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRAssessIndexLibrarySchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRAssessIndexLibrary is querying for all data!");
			 }
			 mSql.append("select * from LRAssessIndexLibrary ");
			...
			{
				i++;
				LRAssessIndexLibrarySchema s1 = new LRAssessIndexLibrarySchema();
				s1.setSchema(rs,i);
				aLRAssessIndexLibrarySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLRAssessIndexLibrarySet;
	}

	public LRAssessIndexLibrarySet executeQuery(String sql, List bv)
	{
	...
	public LRAssessIndexLibrarySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRAssessIndexLibrarySet aLRAssessIndexLibrarySet = new LRAssessIndexLibrarySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRAssessIndexLibrary");
			LRAssessIndexLibrarySchema aSchema = this.getSchema();
	...
	public LRAssessIndexLibrarySet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRAssessIndexLibrarySet aLRAssessIndexLibrarySet = new LRAssessIndexLibrarySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRAssessIndexLibrary " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LRAssessIndexLibrarySet
 */
public LRAssessIndexLibrarySet getData()
{
    int tCount = 0;
    LRAssessIndexLibrarySet tLRAssessIndexLibrarySet = new LRAssessIndexLibrarySet();
    LRAssessIndexLibrarySchema tLRAssessIndexLibrarySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRAssessIndexLibraryDB";
    ...
    {
        tCount = 1;
        tLRAssessIndexLibrarySchema = new LRAssessIndexLibrarySchema();
        tLRAssessIndexLibrarySchema.setSchema(mResultSet, 1);
        tLRAssessIndexLibrarySet.add(tLRAssessIndexLibrarySchema);
    ...
    return tLRAssessIndexLibrarySet;
}
/**
 * 关闭数据集
 * @return boolean
