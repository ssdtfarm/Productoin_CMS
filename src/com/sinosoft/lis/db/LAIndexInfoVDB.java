...
/*
 * <p>ClassName: LAIndexInfoVDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LAIndexInfoVDB extends LAIndexInfoVSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAIndexInfoVDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAIndexInfoVSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAIndexInfoVDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAIndexInfoV WHERE  WageNo = ? AND IndexType = ? AND AgentCode = ?", 
	...
	public LAIndexInfoVSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAIndexInfoVSet aLAIndexInfoVSet = new LAIndexInfoVSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAIndexInfoVSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAIndexInfoV is querying for all data!");
			 }
			 mSql.append("select * from LAIndexInfoV ");
			...
			{
				i++;
				LAIndexInfoVSchema s1 = new LAIndexInfoVSchema();
				s1.setSchema(rs,i);
				aLAIndexInfoVSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAIndexInfoVSet;
	}

	public LAIndexInfoVSet executeQuery(String sql, List bv)
	{
	...
	public LAIndexInfoVSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAIndexInfoVSet aLAIndexInfoVSet = new LAIndexInfoVSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAIndexInfoV");
			LAIndexInfoVSchema aSchema = this.getSchema();
	...
	public LAIndexInfoVSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAIndexInfoVSet aLAIndexInfoVSet = new LAIndexInfoVSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAIndexInfoV " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAIndexInfoVSet
 */
public LAIndexInfoVSet getData()
{
    int tCount = 0;
    LAIndexInfoVSet tLAIndexInfoVSet = new LAIndexInfoVSet();
    LAIndexInfoVSchema tLAIndexInfoVSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAIndexInfoVDB";
    ...
    {
        tCount = 1;
        tLAIndexInfoVSchema = new LAIndexInfoVSchema();
        tLAIndexInfoVSchema.setSchema(mResultSet, 1);
        tLAIndexInfoVSet.add(tLAIndexInfoVSchema);
