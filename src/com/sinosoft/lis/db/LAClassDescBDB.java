...
/*
 * <p>ClassName: LAClassDescBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAClassDescBDB extends LAClassDescBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAClassDescBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAClassDescBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAClassDescBDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getClassNo() == null || this.getClassNo().equals("null")) {
				...
				pstmt.setString(1, this.getClassNo());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getReceiGrade() == null || this.getReceiGrade().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getReceiGrade());
			}
			if(this.getClassSeries() == null || this.getClassSeries().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getClassSeries());
			}
			if(this.getStopFlag() == null || this.getStopFlag().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getStopFlag());
			}
			...
			if(this.getClassNo() == null || this.getClassNo().equals("null")) {
				...
				pstmt.setString(15, this.getClassNo());
			}
			if(this.getChannelCode() == null || this.getChannelCode().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getChannelCode());
			}
			if(this.getMakeDate1() == null || this.getMakeDate1().equals("null")) {
				pstmt.setNull(17, 91);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAClassDescB WHERE  ClassNo = ? AND ChannelCode = ? AND MakeDate1 = ? AND MakeTime1 = ? AND Operator1 = ?", 
	...
	public LAClassDescBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAClassDescBSet aLAClassDescBSet = new LAClassDescBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAClassDescBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAClassDescB is querying for all data!");
			 }
			 mSql.append("select * from LAClassDescB ");
			...
			{
				i++;
				LAClassDescBSchema s1 = new LAClassDescBSchema();
				s1.setSchema(rs,i);
				aLAClassDescBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAClassDescBSet;
	}

	public LAClassDescBSet executeQuery(String sql, List bv)
	{
	...
	public LAClassDescBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAClassDescBSet aLAClassDescBSet = new LAClassDescBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAClassDescB");
			LAClassDescBSchema aSchema = this.getSchema();
	...
	public LAClassDescBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAClassDescBSet aLAClassDescBSet = new LAClassDescBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAClassDescB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAClassDescBSet
 */
public LAClassDescBSet getData()
{
    int tCount = 0;
    LAClassDescBSet tLAClassDescBSet = new LAClassDescBSet();
    LAClassDescBSchema tLAClassDescBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAClassDescBDB";
    ...
    {
        tCount = 1;
        tLAClassDescBSchema = new LAClassDescBSchema();
        tLAClassDescBSchema.setSchema(mResultSet, 1);
        tLAClassDescBSet.add(tLAClassDescBSchema);
