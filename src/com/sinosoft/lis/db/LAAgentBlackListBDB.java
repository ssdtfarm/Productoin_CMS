...
/*
 * <p>ClassName: LAAgentBlackListBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAAgentBlackListBDB extends LAAgentBlackListBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAAgentBlackListBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAAgentBlackListBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAAgentBlackListBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getInsuranceCompany() == null || this.getInsuranceCompany().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getInsuranceCompany());
			}
			if(this.getSource() == null || this.getSource().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getSource());
			}
			if(this.getHandlingMethod() == null || this.getHandlingMethod().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getHandlingMethod());
			}
			...
			if(this.getExpiryDate() == null || this.getExpiryDate().equals("null")) {
				pstmt.setNull(9, 91);
			} else {
				pstmt.setDate(9, Date.valueOf(this.getExpiryDate()));
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAgentBlackListB WHERE  IDNo = ? AND Operator1 = ? AND MakeDate1 = ? AND MakeTime1 = ?", 
	...
	public LAAgentBlackListBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentBlackListBSet aLAAgentBlackListBSet = new LAAgentBlackListBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAAgentBlackListBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAAgentBlackListB is querying for all data!");
			 }
			 mSql.append("select * from LAAgentBlackListB ");
			...
			{
				i++;
				LAAgentBlackListBSchema s1 = new LAAgentBlackListBSchema();
				s1.setSchema(rs,i);
				aLAAgentBlackListBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAAgentBlackListBSet;
	}

	public LAAgentBlackListBSet executeQuery(String sql, List bv)
	{
	...
	public LAAgentBlackListBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentBlackListBSet aLAAgentBlackListBSet = new LAAgentBlackListBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAAgentBlackListB");
			LAAgentBlackListBSchema aSchema = this.getSchema();
	...
	public LAAgentBlackListBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAAgentBlackListBSet aLAAgentBlackListBSet = new LAAgentBlackListBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAAgentBlackListB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAAgentBlackListBSet
 */
public LAAgentBlackListBSet getData()
{
    int tCount = 0;
    LAAgentBlackListBSet tLAAgentBlackListBSet = new LAAgentBlackListBSet();
    LAAgentBlackListBSchema tLAAgentBlackListBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAAgentBlackListBDB";
    ...
    {
        tCount = 1;
        tLAAgentBlackListBSchema = new LAAgentBlackListBSchema();
        tLAAgentBlackListBSchema.setSchema(mResultSet, 1);
        tLAAgentBlackListBSet.add(tLAAgentBlackListBSchema);
