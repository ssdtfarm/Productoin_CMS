...
/*
 * <p>ClassName: LAAwardTypeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAAwardTypeDB extends LAAwardTypeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAAwardTypeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAAwardTypeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAAwardTypeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAwardType() == null || this.getAwardType().equals("null")) {
				...
				pstmt.setString(2, this.getAwardType());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAwardType WHERE  IDX = ?", 
	...
	public LAAwardTypeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAwardTypeSet aLAAwardTypeSet = new LAAwardTypeSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAAwardTypeSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAAwardType is querying for all data!");
			 }
			 mSql.append("select * from LAAwardType ");
			...
			{
				i++;
				LAAwardTypeSchema s1 = new LAAwardTypeSchema();
				s1.setSchema(rs,i);
				aLAAwardTypeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAAwardTypeSet;
	}

	public LAAwardTypeSet executeQuery(String sql, List bv)
	{
	...
	public LAAwardTypeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAwardTypeSet aLAAwardTypeSet = new LAAwardTypeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAAwardType");
			LAAwardTypeSchema aSchema = this.getSchema();
	...
	public LAAwardTypeSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAAwardTypeSet aLAAwardTypeSet = new LAAwardTypeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAAwardType " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAAwardTypeSet
 */
public LAAwardTypeSet getData()
{
    int tCount = 0;
    LAAwardTypeSet tLAAwardTypeSet = new LAAwardTypeSet();
    LAAwardTypeSchema tLAAwardTypeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAAwardTypeDB";
    ...
    {
        tCount = 1;
        tLAAwardTypeSchema = new LAAwardTypeSchema();
        tLAAwardTypeSchema.setSchema(mResultSet, 1);
        tLAAwardTypeSet.add(tLAAwardTypeSchema);
