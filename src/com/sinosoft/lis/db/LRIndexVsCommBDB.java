...
/*
 * <p>ClassName: LRIndexVsCommBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LRIndexVsCommBDB extends LRIndexVsCommBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LRIndexVsCommBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRIndexVsCommBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LRIndexVsCommBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getIndexSerise() == null || this.getIndexSerise().equals("null")) {
				...
				pstmt.setString(6, this.getIndexSerise());
			}
			if(this.getWageCode() == null || this.getWageCode().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
			...
			if(this.getWageName() == null || this.getWageName().equals("null")) {
				...
				pstmt.setString(8, this.getWageName());
			}
			if(this.getIndexCode() == null || this.getIndexCode().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getIndexCode());
			}
			...
			if(this.getBaseCode() == null || this.getBaseCode().equals("null")) {
				...
				pstmt.setString(15, this.getBaseCode());
			}
			if(this.getManageCom() == null || this.getManageCom().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
			...
			if(this.getAgentGrade() == null || this.getAgentGrade().equals("null")) {
				...
				pstmt.setString(17, this.getAgentGrade());
			}
			if(this.getIndexType() == null || this.getIndexType().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getIndexType());
			}
			if(this.getWageCode() == null || this.getWageCode().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getWageCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRIndexVsCommB WHERE  EdorNo = ? AND BaseCode = ? AND ManageCom = ? AND AgentGrade = ? AND IndexType = ? AND WageCode = ?", 
	...
	public LRIndexVsCommBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexVsCommBSet aLRIndexVsCommBSet = new LRIndexVsCommBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LRIndexVsCommBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LRIndexVsCommB is querying for all data!");
			 }
			 mSql.append("select * from LRIndexVsCommB ");
			...
			{
				i++;
				LRIndexVsCommBSchema s1 = new LRIndexVsCommBSchema();
				s1.setSchema(rs,i);
				aLRIndexVsCommBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLRIndexVsCommBSet;
	}

	public LRIndexVsCommBSet executeQuery(String sql, List bv)
	{
	...
	public LRIndexVsCommBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRIndexVsCommBSet aLRIndexVsCommBSet = new LRIndexVsCommBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRIndexVsCommB");
			LRIndexVsCommBSchema aSchema = this.getSchema();
	...
	public LRIndexVsCommBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LRIndexVsCommBSet aLRIndexVsCommBSet = new LRIndexVsCommBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRIndexVsCommB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LRIndexVsCommBSet
 */
public LRIndexVsCommBSet getData()
{
    int tCount = 0;
    LRIndexVsCommBSet tLRIndexVsCommBSet = new LRIndexVsCommBSet();
    LRIndexVsCommBSchema tLRIndexVsCommBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRIndexVsCommBDB";
    ...
    {
        tCount = 1;
        tLRIndexVsCommBSchema = new LRIndexVsCommBSchema();
        tLRIndexVsCommBSchema.setSchema(mResultSet, 1);
        tLRIndexVsCommBSet.add(tLRIndexVsCommBSchema);
