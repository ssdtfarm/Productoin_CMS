...
import com.sinosoft.lis.vschema.LAAgentCSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LAAgentCDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: LAAgentCd
 */
public class LAAgentCDB extends LAAgentCSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAAgentCDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LAAgentC" );
		mflag = true;
	}
	...
	{
		LAAgentCSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LAAgentC WHERE  BakMonth = ? AND BakType = ? AND AgentCode = ?");
		...
		SQLString sqlObj = new SQLString("LAAgentC");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LAAgentC VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getBakMonth() == null || this.getBakMonth().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBakMonth());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAgentC WHERE  BakMonth = ? AND BakType = ? AND AgentCode = ?", 
	...
	public LAAgentCSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentCSet aLAAgentCSet = new LAAgentCSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAAgentCSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAAgentC is querying for all data!");
			 }
			 mSql.append("select * from LAAgentC ");
			...
			{
				i++;
				LAAgentCSchema s1 = new LAAgentCSchema();
				s1.setSchema(rs,i);
				aLAAgentCSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LAAgentCSet executeQuery(String sql, List bv)
	{
	...
	public LAAgentCSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentCSet aLAAgentCSet = new LAAgentCSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAAgentC");
			LAAgentCSchema aSchema = this.getSchema();
	...
	public LAAgentCSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAAgentCSet aLAAgentCSet = new LAAgentCSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAAgentC " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LAAgentCSet
 */
public LAAgentCSet getData()
{
    int tCount = 0;
    LAAgentCSet tLAAgentCSet = new LAAgentCSet();
    LAAgentCSchema tLAAgentCSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAAgentCDB";
    ...
    {
        tCount = 1;
        tLAAgentCSchema = new LAAgentCSchema();
        tLAAgentCSchema.setSchema(mResultSet, 1);
        tLAAgentCSet.add(tLAAgentCSchema);
