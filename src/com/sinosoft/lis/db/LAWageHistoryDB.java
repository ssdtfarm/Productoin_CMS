...
public class LAWageHistoryDB extends LAWageHistorySchema
{
	...
	**/
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LAWageHistory WHERE  WageNo = ? AND ManageCom = ? AND BranchType = ?");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LAWageHistory SET  WageNo = ? , ManageCom = ? , AClass = ? , BranchType = ? , Noti = ? , State = ? , AssessApproveDate = ? , WageApproveDate = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , BranchType2 = ? WHERE  WageNo = ? AND ManageCom = ? AND BranchType = ?");
			if(this.getWageNo() == null || this.getWageNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getWageNo());
			...
			if(this.getAssessApproveDate() == null || this.getAssessApproveDate().equals("null")) {
				pstmt.setNull(7, 91);
			} else {
				pstmt.setDate(7, Date.valueOf(this.getAssessApproveDate()));
			}
			if(this.getWageApproveDate() == null || this.getWageApproveDate().equals("null")) {
				pstmt.setNull(8, 91);
			} else {
				pstmt.setDate(8, Date.valueOf(this.getWageApproveDate()));
			}
			...
			if(this.getWageNo() == null || this.getWageNo().equals("null")) {
				...
				pstmt.setString(15, this.getWageNo());
			}
			if(this.getManageCom() == null || this.getManageCom().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LAWageHistory VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getWageNo() == null || this.getWageNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getWageNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAWageHistory WHERE  WageNo = ? AND ManageCom = ? AND BranchType = ?", 
	...
	public LAWageHistorySet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAWageHistorySchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAWageHistory is querying for all data!");
			 }
			 mSql.append("select * from LAWageHistory ");
	...
	public LAWageHistorySet executeQuery(String sql, List bv)
	{
	...
	public LAWageHistorySet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAWageHistorySet aLAWageHistorySet = new LAWageHistorySet();
