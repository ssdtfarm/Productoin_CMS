...
public class LAAgentBlackListDB extends LAAgentBlackListSchema
{
	...
	**/
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LAAgentBlackList WHERE  IDNo = ?");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LAAgentBlackList SET  IDNo = ? , Name = ? , AgentCode = ? , InsuranceCompany = ? , Source = ? , HandlingMethod = ? , Reason = ? , EffectiveDate = ? , ExpiryDate = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? WHERE  IDNo = ?");
			if(this.getIDNo() == null || this.getIDNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getIDNo());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LAAgentBlackList VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getIDNo() == null || this.getIDNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getIDNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAgentBlackList WHERE  IDNo = ?", 
	...
	public LAAgentBlackListSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAAgentBlackListSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAAgentBlackList is querying for all data!");
			 }
			 mSql.append("select * from LAAgentBlackList ");
	...
	public LAAgentBlackListSet executeQuery(String sql, List bv)
	{
	...
	public LAAgentBlackListSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAAgentBlackListSet aLAAgentBlackListSet = new LAAgentBlackListSet();
