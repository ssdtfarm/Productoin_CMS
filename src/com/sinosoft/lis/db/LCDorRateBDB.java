...
import com.sinosoft.lis.vschema.LCDorRateBSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LCDorRateBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LCDorRateBDB extends LCDorRateBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCDorRateBDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCDorRateB" );
		mflag = true;
	}
	...
	{
		LCDorRateBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCDorRateBDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCDorRateB WHERE  ProductCode = ?");
		...
		SQLString sqlObj = new SQLString("LCDorRateB");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCDorRateB SET  ProductCode = ? , DorRate = ? , EffectiveYM = ? , EffectiveEndYM = ? , DeleteFlag = ? , OperationDate = ? , OPERATOR = ? , MAKEDATE = ? , MAKETIME = ? , MODIFYDATE = ? , MODIFYTIME = ? WHERE  ProductCode = ?");
			if(this.getProductCode() == null || this.getProductCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getProductCode());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCDorRateB VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getProductCode() == null || this.getProductCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getProductCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCDorRateB WHERE  ProductCode = ?", 
	...
	public LCDorRateBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCDorRateBSet aLCDorRateBSet = new LCDorRateBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LCDorRateBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LCDorRateB is querying for all data!");
			 }
			 mSql.append("select * from LCDorRateB ");
			...
			{
				i++;
				LCDorRateBSchema s1 = new LCDorRateBSchema();
				s1.setSchema(rs,i);
				aLCDorRateBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCDorRateBSet;
	}

	public LCDorRateBSet executeQuery(String sql, List bv)
	{
	...
	public LCDorRateBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCDorRateBSet aLCDorRateBSet = new LCDorRateBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCDorRateB");
			LCDorRateBSchema aSchema = this.getSchema();
	...
	public LCDorRateBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LCDorRateBSet aLCDorRateBSet = new LCDorRateBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCDorRateB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCDorRateBSet
 */
public LCDorRateBSet getData()
{
    int tCount = 0;
    LCDorRateBSet tLCDorRateBSet = new LCDorRateBSet();
    LCDorRateBSchema tLCDorRateBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCDorRateBDB";
    ...
    {
        tCount = 1;
        tLCDorRateBSchema = new LCDorRateBSchema();
        tLCDorRateBSchema.setSchema(mResultSet, 1);
        tLCDorRateBSet.add(tLCDorRateBSchema);
