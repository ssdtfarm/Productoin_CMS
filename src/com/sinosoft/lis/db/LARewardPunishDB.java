...
/*
 * <p>ClassName: LARewardPunishDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LARewardPunishDB extends LARewardPunishSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LARewardPunishDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LARewardPunishSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LARewardPunishDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getIDX() == null || this.getIDX().equals("null")) {
				...
				pstmt.setString(22, this.getIDX());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LARewardPunish WHERE  AgentCode = ? AND IDX = ?", 
	...
	public LARewardPunishSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARewardPunishSet aLARewardPunishSet = new LARewardPunishSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LARewardPunishSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LARewardPunish is querying for all data!");
			 }
			 mSql.append("select * from LARewardPunish ");
			...
			{
				i++;
				LARewardPunishSchema s1 = new LARewardPunishSchema();
				s1.setSchema(rs,i);
				aLARewardPunishSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLARewardPunishSet;
	}

	public LARewardPunishSet executeQuery(String sql, List bv)
	{
	...
	public LARewardPunishSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LARewardPunishSet aLARewardPunishSet = new LARewardPunishSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LARewardPunish");
			LARewardPunishSchema aSchema = this.getSchema();
	...
	public LARewardPunishSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LARewardPunishSet aLARewardPunishSet = new LARewardPunishSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LARewardPunish " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LARewardPunishSet
 */
public LARewardPunishSet getData()
{
    int tCount = 0;
    LARewardPunishSet tLARewardPunishSet = new LARewardPunishSet();
    LARewardPunishSchema tLARewardPunishSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LARewardPunishDB";
    ...
    {
        tCount = 1;
        tLARewardPunishSchema = new LARewardPunishSchema();
        tLARewardPunishSchema.setSchema(mResultSet, 1);
