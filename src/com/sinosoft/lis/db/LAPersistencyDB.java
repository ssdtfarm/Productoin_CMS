...
import com.sinosoft.lis.schema.LAPersistencySchema;
import com.sinosoft.lis.vschema.LAPersistencySet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LAPersistencyDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAPersistencyDB extends LAPersistencySchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAPersistencyDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LAPersistency" );
		mflag = true;
	}

	public LAPersistencyDB()
	{
		con = null;
		db = new DBOper( "LAPersistency" );
		mflag = false;
	}
	...
	{
		LAPersistencySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAPersistencyDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LAPersistency WHERE  PersistencySN = ?");
		...
		SQLString sqlObj = new SQLString("LAPersistency");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPersistencySN() == null || this.getPersistencySN().equals("null")) {
				...
				pstmt.setString(47, this.getPersistencySN());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LAPersistency VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getPersistencySN() == null || this.getPersistencySN().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getPersistencySN());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAPersistency WHERE  PersistencySN = ?", 
	...
	public LAPersistencySet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAPersistencySet aLAPersistencySet = new LAPersistencySet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAPersistencySchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAPersistency is querying for all data!");
			 }
			 mSql.append("select * from LAPersistency ");
			...
			{
				i++;
				LAPersistencySchema s1 = new LAPersistencySchema();
				s1.setSchema(rs,i);
				aLAPersistencySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAPersistencySet;
	}

	public LAPersistencySet executeQuery(String sql, List bv)
	{
	...
	public LAPersistencySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAPersistencySet aLAPersistencySet = new LAPersistencySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAPersistency");
			LAPersistencySchema aSchema = this.getSchema();
	...
	public LAPersistencySet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAPersistencySet aLAPersistencySet = new LAPersistencySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAPersistency " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAPersistencySet
 */
public LAPersistencySet getData()
{
    int tCount = 0;
    LAPersistencySet tLAPersistencySet = new LAPersistencySet();
    LAPersistencySchema tLAPersistencySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAPersistencyDB";
    ...
    {
        tCount = 1;
        tLAPersistencySchema = new LAPersistencySchema();
        tLAPersistencySchema.setSchema(mResultSet, 1);
        tLAPersistencySet.add(tLAPersistencySchema);
