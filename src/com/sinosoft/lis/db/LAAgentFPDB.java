...
/*
 * <p>ClassName: LAAgentFPDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: LAAgentFP_Setting
 */
public class LAAgentFPDB extends LAAgentFPSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAAgentFPDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAAgentFPSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getFinanceStartMonth() == null || this.getFinanceStartMonth().equals("null")) {
				...
				pstmt.setString(25, this.getFinanceStartMonth());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAgentFP WHERE  AgentCode = ? AND FinanceStartMonth = ?", 
	...
	public LAAgentFPSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentFPSet aLAAgentFPSet = new LAAgentFPSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAAgentFPSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAAgentFP is querying for all data!");
			 }
			 mSql.append("select * from LAAgentFP ");
			...
			{
				i++;
				LAAgentFPSchema s1 = new LAAgentFPSchema();
				s1.setSchema(rs,i);
				aLAAgentFPSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAAgentFPSet;
	}

	public LAAgentFPSet executeQuery(String sql, List bv)
	{
	...
	public LAAgentFPSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAAgentFPSet aLAAgentFPSet = new LAAgentFPSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAAgentFP");
			LAAgentFPSchema aSchema = this.getSchema();
	...
	public LAAgentFPSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAAgentFPSet aLAAgentFPSet = new LAAgentFPSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAAgentFP " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LAAgentFPSet
 */
public LAAgentFPSet getData()
{
    int tCount = 0;
    LAAgentFPSet tLAAgentFPSet = new LAAgentFPSet();
    LAAgentFPSchema tLAAgentFPSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAAgentFPDB";
    ...
    {
        tCount = 1;
        tLAAgentFPSchema = new LAAgentFPSchema();
        tLAAgentFPSchema.setSchema(mResultSet, 1);
        tLAAgentFPSet.add(tLAAgentFPSchema);
