...
public class LATreeDB extends LATreeSchema
{
	...
	**/
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LATree WHERE  AgentCode = ?", 
	...
	public LATreeSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LATreeSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LATree is querying for all data!");
			 }
			 mSql.append("select * from LATree ");
	...
	public LATreeSet executeQuery(String sql, List bv)
	{
	...
	public LATreeSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LATreeSet aLATreeSet = new LATreeSet();
