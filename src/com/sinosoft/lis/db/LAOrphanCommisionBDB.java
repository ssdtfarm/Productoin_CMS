...
/*
 * <p>ClassName: LAOrphanCommisionBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: SqlServerSchemaMaker
 */
public class LAOrphanCommisionBDB extends LAOrphanCommisionBSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LAOrphanCommisionBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAOrphanCommisionBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAOrphanCommisionBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getServiceAgentCode() == null || this.getServiceAgentCode().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
				pstmt.setString(3, this.getServiceAgentCode());
			}
			if(this.getLeaderAgentCode() == null || this.getLeaderAgentCode().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getLeaderAgentCode());
			}
			if(this.getISLeader() == null || this.getISLeader().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getISLeader());
			}
			pstmt.setDouble(6, this.getSAgentAccountAmt());
			pstmt.setDouble(7, this.getLeaderAccountAmt());
			pstmt.setDouble(8, this.getCompanyAccountAmt());
			if(this.getAssignDate() == null || this.getAssignDate().equals("null")) {
				pstmt.setNull(9, 91);
			} else {
				pstmt.setDate(9, Date.valueOf(this.getAssignDate()));
			}
			if(this.getASAReceivedDate() == null || this.getASAReceivedDate().equals("null")) {
				pstmt.setNull(10, 91);
			} else {
				pstmt.setDate(10, Date.valueOf(this.getASAReceivedDate()));
			}
			...
			if(this.getCommisionSN() == null || this.getCommisionSN().equals("null")) {
				...
				pstmt.setString(20, this.getCommisionSN());
			}
			if(this.getOperator1() == null || this.getOperator1().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAOrphanCommisionB WHERE  CommisionSN = ? AND Operator1 = ? AND MakeDate1 = ? AND MakeTime1 = ?", 
	...
	public LAOrphanCommisionBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAOrphanCommisionBSet aLAOrphanCommisionBSet = new LAOrphanCommisionBSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			 ...
			 LAOrphanCommisionBSchema aSchemaNew = this.getSchema();
			 ...
			 if (j == 0) {
			 	...
			 	throw new IllegalArgumentException("Table LAOrphanCommisionB is querying for all data!");
			 }
			 mSql.append("select * from LAOrphanCommisionB ");
			...
			{
				i++;
				LAOrphanCommisionBSchema s1 = new LAOrphanCommisionBSchema();
				s1.setSchema(rs,i);
				aLAOrphanCommisionBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAOrphanCommisionBSet;
	}

	public LAOrphanCommisionBSet executeQuery(String sql, List bv)
	{
	...
	public LAOrphanCommisionBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LAOrphanCommisionBSet aLAOrphanCommisionBSet = new LAOrphanCommisionBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAOrphanCommisionB");
			LAOrphanCommisionBSchema aSchema = this.getSchema();
	...
	public LAOrphanCommisionBSet executeQuery(String sql, List bv, int nStart, int nCount)
	{
		PreparedStatement stmt = null;
		ResultSet rs = null;
		LAOrphanCommisionBSet aLAOrphanCommisionBSet = new LAOrphanCommisionBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAOrphanCommisionB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAOrphanCommisionBSet
 */
public LAOrphanCommisionBSet getData()
{
    int tCount = 0;
    LAOrphanCommisionBSet tLAOrphanCommisionBSet = new LAOrphanCommisionBSet();
    LAOrphanCommisionBSchema tLAOrphanCommisionBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAOrphanCommisionBDB";
    ...
    {
        tCount = 1;
        tLAOrphanCommisionBSchema = new LAOrphanCommisionBSchema();
        tLAOrphanCommisionBSchema.setSchema(mResultSet, 1);
        tLAOrphanCommisionBSet.add(tLAOrphanCommisionBSchema);
