...
public class LRRuleInforBLReport {
    ...
    private String filePathString="LRRuleInfoReport";
    private String sysvar = "OnwerExportFilePath";

	private String currentDate = PubFun.getCurrentDate();
	//private String currentTime = PubFun.getCurrentTime();

	public LRRuleInforBLReport() {
	}
	  SSRS tSSRS = new SSRS();
	...
	public boolean getInputData(VData cInputData) {
        ...
        WageName = (String)transferData.getValueByName("WageName");
        IndexCode = (String)transferData.getValueByName("IndexCode");
        IndexName = (String)transferData.getValueByName("IndexName");
        StateRule = (String)transferData.getValueByName("StateRule");
	...
	private boolean printReport() {
		...
		templetFileName="LRRuleInfo.xls";// 模板名字
		reportFileName="LRRuleInfo_" + this.mGlobalInput.Operator.trim() + "_" + currentDate + reportTime + ".xls";// 要生成的报表的名字,不支持中文
		rvFNStartWith="LRRuleInfo_" + this.mGlobalInput.Operator.trim() + "_" + currentDate;// 删除服务器上缓存的文件名以rvFNStartWith开头的报表
...
//			String strSQL="select (select codename from ldcode where codetype = 'branchtype' and code =bom.BranchType ),bom.wagecode,bom.wagename,decode(bom.state, 'N', '无效', 'Y', '有效'),assess.indexcode,assess.indexname,decode(assess.state, 'N', '无效', 'Y', '有效'),(select codename from ldcode where codetype = 'ibrmscommandtype' and code =assess.datatype),(select codename from ldcode where codetype = 'logic' and code =assess.caltype ),assess.description from lrindex bom,lrassessindexlibrary assess where bom.wagecode=assess.wagecode and 1=1";
			...
			String strSQL="select (select codename from ldcode where codetype = 'branchtype' and code =bom.BranchType ),bom.wagecode,bom.wagename,bom.state,assess.indexcode,assess.indexname,assess.state,(select codename from ldcode where codetype = 'ibrmscommandtype' and code =assess.datatype),(select codename from ldcode where codetype = 'logic' and code =assess.caltype ),assess.description from lrindex bom,lrassessindexlibrary assess where bom.wagecode=assess.wagecode and 1=1";

			   	 
			String strParam="and bom.WageCode like'"+WageCode+"%'"+" "+"and bom.WageName like'"+WageName+"%'"+" "+"and bom.BranchType like'"+BranchType+"%'"+" "+"and bom.state like'"+State+"%'"+" "+"and assess.indexcode like'"+IndexCode+"%'"+" "+"and assess.indexname like'"+IndexName+"%'"+" "+"and assess.state like'"+StateRule+"%'"+" order by bom.WageCode";
			...
			if (tlistTable.size() == 0) {
				...
				tError.errorMessage = "查询结束，系统中没有本次执行所要打印的数据！";
