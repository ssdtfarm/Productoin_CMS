...
function getAsyncUrl(treeNode) {
 ...
 function zTreeBeforeClick(treeId, treeNode){ 	
	 ...
	 if(treeNode.code.substring(0,1)=='W'){
		 tmp = "./LAindexRuleR.jsp?IndexType="+treeNode.parentNode.parentNode.code+"&AgentGrade="+treeNode.parentNode.code+"&WageCode="+treeNode.code+"&BaseCode="+fm.BaseCode.value;
	 }else if(treeNode.code.substring(0,1)=='R'){
		 tmp = "./editor/indexR.jsp?IndexType="+treeNode.indextype+"&State="+fm.State.value+"&BaseCode="+fm.BaseCode.value+"&IndexCode="+treeNode.code+"&EditMode=para&AgentGrade="+treeNode.agentgrade;
	 }else if(treeNode.code.substring(0,1)=='2') {
		 $("#detailTarget")[0].src = "";
		 $("#detail").hide();
		 return true;
	 }else if(treeNode.code.substring(0,1)!='0'&&treeNode.indextype=='23'){
		 tmp = "./LAindexResultR.jsp?IndexType="+treeNode.indextype+"&AgentGrade="+treeNode.code+"&BaseCode="+fm.BaseCode.value+"&BranchType="+fm.HideBranchType.value;
...
//	} else if(treeNode.code.substring(0,1)!='0'&&treeNode.code.substring(0,1)!='B'){//加入对根节点的特殊处理tangyj20130917
		 ...
		 tmp = "./LAindexR.jsp?IndexType="+treeNode.indextype+"&AgentGrade="+treeNode.code+"&BaseCode="+fm.BaseCode.value;
	 }else{
		 $("#detailTarget")[0].src = "";
		 $("#detail").hide();
		 return true;
...
$(function(){
	var settingMenu = {    
			...
			asyncParam: ["code","type","indextype","agentgrade"],    
