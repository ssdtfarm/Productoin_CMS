...
function removeClick(){
    ...
    //TODO 处理逻辑放到这里
    var tSel = IndexGrid.getSelNo();	
	if( tSel == 0 || tSel == null ){
		alert( I18NMsg("BOM_Pleaseselectrecordfrom'Listaboutitemtitlehas'") );
		return false;
	}else {
		fm.WageCode.value = IndexGrid.getRowColData(tSel-1,2);
	}
    submitfm();
    fm.hideOperate.value="";
    return true;
...
function addClick(){
    ...
    //TODO 处理逻辑放到这里
    var tSel = RuleGrid.getSelNo();	
	if( tSel == 0 || tSel == null ){
		alert( I18NMsg("BOM_Pleaseselectrecordfrom'Listaboutitemtitledoesnothave'") );
		return false;
	}else {
		fm.WageCode.value = RuleGrid.getRowColData(tSel-1,2);
	}
    submitfm();
    fm.hideOperate.value="";
    return true;
