...
var itemContainer={};
if(!editMode){
	editMode = "edit";//para--配置參數模式,edit--編輯規則模式
}
$.ajax({
	type: "GET",
	url: "./load.jsp",
...
//	    	if(this.model.get('listType'))
    	...
    	if(this.model.get('listType')=='bom') {                 //bom列表
    		if(val=='('){
				...
				var drB = $$(item,{nam:'刪除一個括號',val:'del)',pModel:this.model.get('pModel'),listType:'attr'});
    	...
    	} else if(this.model.get('listType')=='attr'){          //詞條列表
    		...
    		if(val==')'){
	    		...
	    		//this.model.get('pModel').model.set({currval:'請選擇'});
    		...
    		this.model.get('pModel').model.set({currval2:'請選擇'});
    		if(val=='end'){
    		...
    		if(valType == 'input'){
	    		...
	    		var rightB = $$(item,{nam:')',val:')',pModel:tBom,listType:'attr',itemid:''});
					var drB = $$(item,{nam:'刪除一個括號',val:'del)',pModel:tBom,listType:'attr'});
    	...
    	}	else if(this.model.get('listType')=='conn'){         //连接词（或者/並且）
...
function initBom(tObject){
	...
	var dB = $$(item,{nam:'刪除一個括號',val:'del(',pModel:tObject,listType:'bom'});
	...
	var newItem5 = $$(item,{nam:'小於',val:'<',pModel:tObject,listType:'op'});
	var newItem6 = $$(item,{nam:'小於等於',val:'<=',pModel:tObject,listType:'op'});
	var newItem7 = $$(item,{nam:'等於',val:'=',pModel:tObject,listType:'op'});
	var newItem8 = $$(item,{nam:'大於等於',val:'>=',pModel:tObject,listType:'op'});
	var newItem9 = $$(item,{nam:'大於',val:'>',pModel:tObject,listType:'op'});
	var newItem10 = $$(item,{nam:'不等於',val:'<>',pModel:tObject,listType:'op'});
	var newItem11 = $$(item,{nam:'結束 ',val:'end',pModel:tObject,listType:'op'});
	...
	var new2Item5 = $$(item,{nam:'小於',val:'<',pModel:tObject,listType:'op2'});
	var new2Item6 = $$(item,{nam:'小於等於',val:'<=',pModel:tObject,listType:'op2'});
	var new2Item7 = $$(item,{nam:'等於',val:'=',pModel:tObject,listType:'op2'});
	var new2Item8 = $$(item,{nam:'大於等於',val:'>=',pModel:tObject,listType:'op2'});
	var new2Item9 = $$(item,{nam:'大於',val:'>',pModel:tObject,listType:'op2'});
	var new2Item10 = $$(item,{nam:'不等於',val:'<>',pModel:tObject,listType:'op2'});
	var new2Item11 = $$(item,{nam:'結束',val:'end',pModel:tObject,listType:'op2'});
	...
	//初始化val
	var mitem = $$(item,{nam:'輸入值(數字)',val:'輸入值(數字)',pModel:tObject,listType:'val',valType:'input'});
	var mitem2 = $$(item,{nam:'輸入值(字符串)',val:'輸入值(字符串)',pModel:tObject,listType:'val',valType:'input'});
	var mitem3 = $$(item,{nam:'輸入值(日期)',val:'輸入值(日期)',pModel:tObject,listType:'val',valType:'input'});
	...
	var dip = $$(item,{nam:'刪除一個括號',val:'刪除一個括號',pModel:tObject,listType:'val',valType:'del'});
	...
	var mitem2 = $$(item,{nam:'輸入值（字符串）',val:'輸入值（字符串）',pModel:tObject,listType:'val2',valType:'input'});
	tObject.append(mitem2,'.vallist2');
	var mitem3 = $$(item,{nam:'輸入值(日期)',val:'輸入值(日期)',pModel:tObject,listType:'val2',valType:'input'});
	tObject.append(mitem3,'.vallist2');
	//初始化val2
	var mitem = $$(item,{nam:'輸入值(數字)',val:'輸入值(數字)',pModel:tObject,listType:'val2',valType:'input'});
	tObject.append(mitem,'.vallist2');
	//var ip = $$(item,{nam:')',val:')',pModel:tObject,listType:'val2',valType:'add'});
	//var dip = $$(item,{nam:'刪除一個括號',val:'刪除一個括號',pModel:tObject,listType:'val2',valType:'del'});
...
var bom = $$({
	//model:{objType:'bom',currbom:'請選擇',currattr:'的',currop:'操作符',currval:'請選擇'}, 
	model:{objType:'bom',currbom:'請選擇',currattr:'',currop:'',currval:''}, 
	view:{
	...
	controller:{
		'create':function(){
			...
			var dB = $$(item,{nam:'刪除一個括號',val:'del(',pModel:this,listType:'bom'});
  		...
  		//var rightB = $$(item,{nam:')',val:')',pModel:this,listType:'attr',itemid:''});
			//var drB = $$(item,{nam:'刪除一個括號',val:'del)',pModel:this,listType:'attr'});
  		...
  		var newItem5 = $$(item,{nam:'小於',val:'<',pModel:this,listType:'op'});
  		var newItem6 = $$(item,{nam:'小於等於',val:'<=',pModel:this,listType:'op'});
  		var newItem7 = $$(item,{nam:'等於',val:'=',pModel:this,listType:'op'});
  		var newItem8 = $$(item,{nam:'大於等於',val:'>=',pModel:this,listType:'op'});
  		var newItem9 = $$(item,{nam:'大於',val:'>',pModel:this,listType:'op'});
  		var newItem10 = $$(item,{nam:'不等於',val:'<>',pModel:this,listType:'op'});
  		...
  		var new2Item5 = $$(item,{nam:'小於',val:'<',pModel:this,listType:'op2'});
  		var new2Item6 = $$(item,{nam:'小於等於',val:'<=',pModel:this,listType:'op2'});
  		var new2Item7 = $$(item,{nam:'等於',val:'=',pModel:this,listType:'op2'});
  		var new2Item8 = $$(item,{nam:'大於等於',val:'>=',pModel:this,listType:'op2'});
  		var new2Item9 = $$(item,{nam:'大於',val:'>',pModel:this,listType:'op2'});
  		var new2Item10 = $$(item,{nam:'不等於',val:'<>',pModel:this,listType:'op2'});
  		...
  		//初始化val
  		/*var mitem = $$(item,{nam:'輸入值',val:'輸入值',pModel:this,listType:'val',valType:'input'});
  		this.append(mitem,'.vallist');
  		var ip = $$(item,{nam:')',val:')',pModel:this,listType:'val',valType:'add'});
			var dip = $$(item,{nam:'刪除一個括號',val:'刪除一個括號',pModel:this,listType:'val',valType:'del'});
  		...
  		for(var i in bomContainer){
			...
			//初始化val2
  		var mitem = $$(item,{nam:'輸入值',val:'輸入值',pModel:this,listType:'val2',valType:'input'});
  		this.append(mitem,'.vallist2');
  		//var ip = $$(item,{nam:')',val:')',pModel:this,listType:'val2',valType:'add'});
			//var dip = $$(item,{nam:'刪除一個括號',val:'刪除一個括號',pModel:this,listType:'val2',valType:'del'});
...
var statement = $$({
	...
	view:{
		format:'<div class="statement">'+
							...
							'<div class="boms" />'+
							'<span title="刪除規則" class="dels edit">X</span>'+
					'</div><br/>',
	...
	controller:{
		'create': function(){
			var item1 = $$(item,{nam:'並且',val:'and',pModel:this,listType:'conn'});
  	...
  	'click span.dels': function(){
  		...
  		if(confirm('確定要刪除')){
  			//刪除
  			this.destroy();
  			//如果刪除了第一個statement，需要将第二個statement的currconn置空
  			//if(!pStat){
				var found = false;
...
var block = $$({
	...
	view:{
		format:'<div>'+
							...
							'<div class="addb edit"><a title="增加規則" href="javascript:void(0);">+</a></div>'+
					'</div>',
		style:'& {position:relative;}'+
	...
	controller:{
  	...
  	'click a': function(){
  		...
  		var stat;
  		if(this.size()>0){
  			stat = $$(statement,{currconn:'並且',block:this,objid:(new Date()).getTime()});
  		} else {
  	...
  	'addstatement': function(model){
  		...
  		/*if(this.size()>0){
  			stat = $$(statement,{currconn:'並且',block:this});
  		} else {
...
var ifthen = $$({
	...
	view:{
		format:'<div style="background-color:#F5F5DC;margin-bottom:5px;" class="ifthen">'+
							...
							'<div class="delit edit"><a title="刪除規則塊" href="javascript:void(0);">X</a></div>'+
					'</div>',
	...
	controller:{
  	'create': function(){
  		...
  		var thenb = $$(block,{currblock:'那麼',objType:'then',objid:(new Date()).getTime()});
...
var ifthenelse = $$({
	...
	view:{
		format:'<div style="border:3px #F5F5DC solid;margin-bottom:5px;">'+
							'<div class="ifthens"/>'+
							'<div class="add edit" style="border-bottom:3px #F5F5DC solid;"><a class="addb" title="增加規則块" href="javascript:void(0);">+</a></div>'+
							'<div class="elseblock"/>'+
	...
	controller:{
		'create': function(){
  		...
  		var elseb = $$(block,{currblock:'否則',objType:'else',objid:(new Date()).getTime()});
...
var ifthenelsetable = $$({
	...
	view:{
		format:'<div>'+
							...
							'<button style="display:none class="save">保存規則</button>'+
							'</div>'+
							'<div id="sbutton" style="display:none;float:left">'+
							'<button style="display:none class="sflag" >設置個性化標記</button>'+
							'</div>'+
							'<div id="qbutton" style="display:none;float:left">'+
							'<button style="display:none class="qflag" >取消個性化標記</button>  '+
							'</div>'+
							'<div id="pbutton" style="display:none;float:left">'+
							'<button style="display:none class="savepara" >保存參數</button>'+
							'</div>'+
					'</div>',
		style:'& a.addb {text-decoration:none;font-size:150%;font-weight:bold;font-family:arial;}'
		},
	controller:{
  	...
  	'click button.savepara': function(){
  		...
  		for(var i = 0; i < app.ifthenelse.ifthen.length; i++){
  			...
  			var tp = sql.slice(sql.indexOf("輸入值(")+4,sql.indexOf("輸入值(")+6);
  			var tp2 = sql2.slice(sql2.indexOf("輸入值(")+4,sql2.indexOf("輸入值(")+6);
  			while(sql.indexOf("輸入值(")!=-1){
  				if(tp=="數字"){
  					arr.push("N");
  				}else if(tp=="日期"){
  					arr.push("D");
  				...
  				sql = sql.replace("輸入值","");
  				tp = sql.slice(sql.indexOf("輸入值(")+4,sql.indexOf("輸入值(")+6);
  			}
  			
  			while(sql2.indexOf("輸入值(")!=-1){
  				if(tp2=="數字"){
  					arr2.push("N");
  				}else if(tp2=="日期"){
  					arr2.push("D");
  				...
  				sql2 = sql2.replace("輸入值","");
  				tp2 = sql2.slice(sql2.indexOf("輸入值(")+4,sql2.indexOf("輸入值(")+6);
  			}
...
function getObj(){
	...
	$(".ifthen").each(function(){
		...
		$('.ifblock .bomdiv', this).each(function(){
			if($('.val',$(this)).text() == '輸入值'){
				var t = {itemid:$('.itemid',$(this)).text(),itemnam:$('.attr',$(this)).text(),objType:'decith'};
				//var id = $('.itemid',$(this)).text();
				//if(!eval("ifO."+id)){
				//	eval("ifO."+id + "=true");
					t.itemnam = "輸入值:"+(count++)+" (如果)"
					app.ths.push(t);
					app.ifs.push(t);
				//}
			}else if($('.val',$(this)).text() == '輸入值(數字)'){
				var t = {itemid:$('.itemid',$(this)).text(),itemnam:$('.attr',$(this)).text(),objType:'decith'};
				//var id = $('.itemid',$(this)).text();
				//if(!eval("ifO."+id)){
				//	eval("ifO."+id + "=true");
					t.itemnam = "輸入值-數字:"+(count++)+" (如果)"
					app.ths.push(t);
					app.ifs.push(t);
				//}
			}else if($('.val',$(this)).text() == '輸入值(字符串)'){
				var t = {itemid:$('.itemid',$(this)).text(),itemnam:$('.attr',$(this)).text(),objType:'decith'};
				//var id = $('.itemid',$(this)).text();
				//if(!eval("ifO."+id)){
				//	eval("ifO."+id + "=true");
					t.itemnam = "輸入值-字符串:"+(count++)+" (如果)"
					app.ths.push(t);
					app.ifs.push(t);
				//}
			}if($('.val',$(this)).text() == '輸入值(日期)'){
				var t = {itemid:$('.itemid',$(this)).text(),itemnam:$('.attr',$(this)).text(),objType:'decith'};
				//var id = $('.itemid',$(this)).text();
				//if(!eval("ifO."+id)){
				//	eval("ifO."+id + "=true");
					t.itemnam = "輸入值-日期:"+(count++)+" (如果)"
					app.ths.push(t);
					app.ifs.push(t);
				//}
			}
  					...
  					if(objType2 == 'ifthen'){
  						...
  						this.view.$('.ifblock .bomdiv').each(function(){
  							if($('.val',$(this)).text() == '輸入值'){
									...
									ifthen.deci.push(t);
									//alert(ifthen.deci.length);
								}else if($('.val',$(this)).text() == '輸入值(日期)'){
  									var t = {};
									t.objType='decith';
									t.itemnam = "輸入值-日期(YYYY-MM-DD):"+(count++)+" (如果)";
									ifthen.deci.push(t);
									//alert(ifthen.deci.length);
								}
  							if($('.val2',$(this)).text() == '輸入值'){
									var t = {};
								t.objType='decith';
								t.itemnam = "輸入值:"+(count++)+" (如果)";
								ifthen.deci.push(t);
								//alert(ifthen.deci.length);
							}else if($('.val2',$(this)).text() == '輸入值(數字)'){
									var t = {};
								t.objType='decith';
								t.itemnam = "輸入值-數字:"+(count++)+" (如果)";
								ifthen.deci.push(t);
								//alert(ifthen.deci.length);
							}else if($('.val2',$(this)).text() == '輸入值(字符串)'){
									var t = {};
								t.objType='decith';
								t.itemnam = "輸入值-字符串:"+(count++)+" (如果)";
								ifthen.deci.push(t);
								//alert(ifthen.deci.length);
							}else if($('.val2',$(this)).text() == '輸入值(日期)'){
									var t = {};
								t.objType='decith';
								t.itemnam = "輸入值-日期(YYYY-MM-DD):"+(count++)+" (如果)";
								ifthen.deci.push(t);
							...
							this.view.$('.thenblock .bomdiv').each(function(){    
								if($('.val',$(this)).text() == '輸入值'){
									...
									//alert(ifthen.deci.length);
  									...
  									var t = {};
									t.objType='decith';
									t.itemnam = "輸入值-日期(YYYY-MM-DD):"+(count++)+" (那麼)";
									ifthen.deci.push(t);
									//alert(ifthen.deci.length);
								}
  							if($('.val2',$(this)).text() == '輸入值'){
							...
							this.view.$('.bomdiv').each(function(){                 
								if($('.val',$(this)).text() == '輸入值'){
									...
									//alert(ifthen.deci.length);
  									...
  									var t = {};
									t.objType='decith';
									t.itemnam = "輸入值-數字:"+(count++);
									app.ifthenelse._else.deci.push(t);
									//alert(ifthen.deci.length);
								}else if($('.val',$(this)).text() == '輸入值(字符串)'){
  									var t = {};
									t.objType='decith';
									t.itemnam = "輸入值-字符串:"+(count++);
									app.ifthenelse._else.deci.push(t);
									//alert(ifthen.deci.length);
								}else if($('.val',$(this)).text() == '輸入值(日期)'){
  									var t = {};
									t.objType='decith';
									t.itemnam = "輸入值-日期(YYYY-MM-DD):"+(count++);
									app.ifthenelse._else.deci.push(t);
									//alert(ifthen.deci.length);
								}
  							if($('.val2',$(this)).text() == '輸入值'){
