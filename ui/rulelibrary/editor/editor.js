...
function initBom(tObject){
	...
	//初始化val
	var mitem = $$(item,{nam:'输入值',val:'输入值',pModel:tObject,listType:'val',valType:'input'});
	...
	//初始化val2
	var mitem = $$(item,{nam:'输入值',val:'输入值',pModel:tObject,listType:'val2',valType:'input'});
...
var bom = $$({
	...
	controller:{
  	...
  	'click span.val2': function(){
  		...
  		var isb = $("ul.vallist2").is(":visible");
  		$('.list').hide();
	    this.view.$('ul.vallist2').toggle(!isb);
  	}
  }
});
...
var ifthen = $$({
	...
	view:{
		format:'<div style="background-color:#F5F5DC;margin-bottom:5px;" class="ifthen">'+
							'<div class="ifblock"/>'+
							'<div class="thenblock"/>'+
							'<div class="deciblock"><table style="border-collapse:collapse;margin-top:5px;" class="dstable"><tr class="head" style="background-color:wheat;"/></table><button class="addrow" id="addrowbutton" style="display:none;">+</button></div>'+
...
var ifthenelsetable = $$({
	...
	view:{
		format:'<div>'+
							...
							'<div id="ebutton" style="">'+
							'<button class="save">保存规则</button>'+
							'</div>'+
							'<div id="pbutton" style="display:none;">'+
							'<button class="savepara" >保存</button>'+
							'</div>'+
					'</div>',
		style:'& a.addb {text-decoration:none;font-size:150%;font-weight:bold;font-family:arial;}'
		},
	controller:{
  	...
  	'click button.savepara': function(){
  		...
  		$.post( "./save.jsp" ,{basecode:basecode,wagecode:wagecode,indexcode:indexcode,ifthenpara:ifthenstr,elsepara:elsestr,json:$.toJSON(app),t:(new Date()).getTime(),oper:'savepara'} , function(data){
				alert(data);
			});
  	},
  	'initRule': function(){
  		...
  		var t = this;
			if(indexcode){
					$.get( "./load.jsp" ,{indexcode:indexcode,oper:'rule',editmode:editMode,basecode:basecode,t:(new Date()).getTime()} , function(data){
						eval("app="+data);
						if(app){
							t.empty();
							initApp();
