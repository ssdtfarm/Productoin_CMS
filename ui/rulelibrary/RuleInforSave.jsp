...
<%
    ...
    RuleInforUI tRuleInforUI = new RuleInforUI();
    ...
    //        
    String tRadio[] = request.getParameterValues("InpProGirdSel");//1:该行被选中；0:该行未被选中
    ...
    try {
        tRuleInforUI.submitData(tVData,tOperate);
    } catch(Exception ex) {
        Content = bundle.getString("Savefailed,reason:") + ex.toString();
        FlagStr = "Fail";
    }
    if (!FlagStr.equals("Fail")) {
        tError = tRuleInforUI.mErrors;
        if (!tError.needDealError()) {
            mResult = tRuleInforUI.getResult(); //获取返回值
