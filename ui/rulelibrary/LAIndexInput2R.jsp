...
<head>
  ...
  <script src="./LAIndexInputR.js" charset="UTF-8"></script>
  <%@include file="./LAIndexInitR.jsp"%>
</head>
...
<body  onload="initForms();initElementtype();">
    <form action="./LAIndexSave.jsp" method=post name=fm target="fraSubmit">     
      ...
      <div id='div0' style='width:100%'>           
        <table>
            <tr class=common>
            <td style="width: 5%; float: right; margin: 2px;" valign="top">
				<div>
					<div style="margin: 2px;font-weight: bold;text-align: left;font-size: 9pt;color: #000000"><%=bundle.getString("C_ExistentWageListofCurrentTitle")%></div>
						<div id="spanIndexGrid"></div>
							
						</div>
			</td>
		 <td class=common style="width: 3%;">		
             <input class="cssButton" style="display:none" id='remove' name='remove' buttonname='remove' value=" >> " type="button" onclick="return removeClick();"/><br>
            <input class="cssButton" style="display:none" id='add' name='add' buttonname='add' value=" << " type="button" onclick="return addClick();"/>
         </td>	
