...
//    var strSQL = "select PhraseType,(select codename from ldcode where codetype = 'phrase'and code = a.PhraseType),Discription,Template from  LRTermPhrase a where Termid ='"+fm.all("TermId").value+"'"
//        //+ tReturn
//    ;
...
//    // 书写SQL语句
//    var tReturn = getManageComLimitlike("managecom");
//    var strSQL = "select name,paratype from lrTermpara a where Termid = '"+fm.all("TermId").value+"'"
//        //+ tReturn
//    ;
//    var strSQLexcel= strSQL;
//    fm.all("tSQL").value=strSQL;
...
function save(){
	...
	var mySql = new SqlClass();
    mySql.setJspName("../../rulelibrary/RuleManagementSql.jsp");
    mySql.setSqlId("LRTermUpdateInput.js_Save1");
    mySql.setValue("p_indexcode",fm.all("TermId").value);
    result = easyQueryVer3(mySql.getString(), 1, 0, 1);
    var s=decodeEasyQueryResult(result);
	if(s){
		if(s[0]!=fm.Remark.value){
