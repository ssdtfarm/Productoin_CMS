...
function easyQueryRuleGrid(){
    initRuleGrid();
    // 书写SQL语句
	var strSQL = "select indexCode,description from LRAssessIndexLibrary a where  wagecode='"+fm.all('WageCode').value+"'  and branchtype='"+fm.all('BranchType').value+"' "
	+" and  not exists(select 1 from lrindexvscommp where a.indexcode=indexcode and wagecode=a.wagecode and baseCode='"+fm.all('BaseCode').value+"' and agentgrade='"+fm.all('AgentGrade').value+"' and branchtype=a.branchtype　and　indextype='"+fm.IndexType.value+"')";
