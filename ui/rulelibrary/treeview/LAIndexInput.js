...
function easyQueryIndexGrid(){
    ...
    var strSQL = "select wagename,wagecode from LRIndexVsCommP where branchtype ='"+fm.BranchType.value+"' and basecode='"+fm.BaseCode.value+"' and agentgrade='"+fm.AgentGrade.value+"' and indextype='"+fm.IndexType.value+"'"
...
function easyQueryRuleGrid(){
    ...
    // 书写SQL语句
    var tReturn = getManageComLimitlike("managecom");
    var strSQL = "select wagename,wagecode from LRIndex a where branchtype ='"+fm.BranchType.value+"'  and not exists (select 1 from LRIndexVsCommP "
	       +"where branchtype=a.branchtype and wagename=a.wagename and wagecode=a.wagecode and agentgrade='"+fm.AgentGrade.value+"' and basecode='"+fm.BaseCode.value+"' and indextype='"+fm.IndexType.value+"') "
	       + getWherePart('IndexSerise','IndexSerise')
	       +"order by wagecode";
    var strSQLexcel= strSQL;
    fm.tSQL.value=strSQL;
...
{	
	try{
		top.opener.afterModified();
	}
	catch(ex)
	{
		myAlert( ""+I18NMsg("G0000028472")+"" + ex );
	}
	top.close();
