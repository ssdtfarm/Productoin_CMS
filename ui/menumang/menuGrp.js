...
function beforeSubmit()
{
    if (fm.all('MenuGrpCode').value =="") {
        alert(I18NMsg("Thisagentsinformationdoesnotexist"));
        return false;
    }
    return true;
}
...
//  	    alert(I18NMsg("Currentoperatorisnotthecreateroftherolebeenselected!"));
//  	    return;
//        }
...
function queryClick()
{
  ...
  //改动6----------------------------------------------
	mySql.setSqlId("SearchMenu");  
	mySql.setValue("p_MenuGrpName",MenuGrpName);
	mySql.setValue("p_MenuGrpCode",MenuGrpCode);
	mySql.setValue("p_MenuGrpDescription",MenuGrpDescription);
	mySql.setValue("p_MenuSign",MenuSign);
...
//	var sqlStr = "select ParentNodeCode,ChildFlag,nodename,nodecode,NodeSign from LDMenu order by nodeorder";
	//改动7----------------------------------------------
	mySql.setSqlId("showAllMenuInUnselect");  
	//改动7----------------------------------------------end
	//使用模拟数据源，必须写在拆分之前
...
//	    	 addNode(unselectArray,j);
	        addNode(unselectArray,j+1);//2015-03-05 modify by gepeng
	    }
	}
...
//    	var strTemp1 = easyQueryVer3(sqlStr,1,0, (i-1)*200 + 1);
    ...
    for (var i = 0; i < unselectArray.length; i++) {
    	if (unselectArray[i][5] == 1)
    	    continue;
    	removeArray[index] = unselectArray[i][4];
    	index++;
    }
...
function afterSubmit(FlagStr)
{
    ...
    if (fm.all('action').value == "insert") {
        if (FlagStr == "success")
            alert(I18NMsg("AddRolesucceeded!"));
        else
            alert(I18NMsg("AddRolefailed!"));
    }

    if (fm.all('action').value == "update") {
        if (FlagStr == "success")
            alert(I18NMsg("ModifyRolesucceeded!"));
        else
            alert(I18NMsg("ModifyRolefailed!"));
    }

    if (fm.all('action').value == "delete") {
        if (FlagStr == "success")
            alert(I18NMsg("DeleteRolesucceeded!"));
        else
            alert(I18NMsg("DeleteRolefailed!"));
    }
}
