...
function GoTo(){
	...
	//用菜单项名称更新窗体标题栏（MINIM）
  var strTitle = "";
  while (arrTitle.length != 0) {
    strTitle += " - " + arrTitle.shift();
  }
  top.window.document.title = strTitle;  	
  
	//alert("aa"+strTitle);
	
	try
	{
	//var urlStr = "./MenuClickSubmit.jsp?Title=" + strTitle ;
  //sFeatures：查询窗口样式                                     
  //var sFeatures = "status:no;help:0;close:0;dialogWidth:1px;dialogHeight:1px;";
  //var hiddform = window.open(urlStr, "", sFeatures);
	top.fraNext.location.href="./MenuClickSubmit.jsp?Title=" + strTitle;
...
//保存菜单项的全路径，用来显示在标题栏上，MINIM增加
//在OpenMenu()增加，在LowItem()中删除
  ...
  var tFlag = 1;
	...
	if(CCnt!=null&&!CCnt.Shw) 
	  if (RememberStatus) {
	    ...
	    OpnTmr=setTimeout("Unfld()",UnfoldDelay);
	  }
	status=HP.Arr[16];
	
	//记录菜单项名称
  for (i=0; i<arrTitle.length; i++) {
    if (arrTitle[i] == HP.Arr[16]) tFlag = 0;
  }
	if (tFlag) arrTitle.push(HP.Arr[16]);
