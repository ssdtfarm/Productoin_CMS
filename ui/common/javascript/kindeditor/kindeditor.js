...
pointRange);
					...
					if (!isEnd && startNode.nodeType == 1) {
						...
						return {node: startNode, pos: startNode.nodeType == 1 ? 1 : 
...
startNode.nodeValue.length};
			...
			this.isControl = (startNode.nodeType == 1 && startNode === endNode && range.startOffset 
	...
	this.addRange = function(keRange) {
		...
		if (KE.browser.IE) {
			...
			if (!this.range.item) {
				...
				if (node == keRange.endNode && KE.util.getNodeType(node) == 1 && 

KE.util.getNodeTextLength(node) == 0) {
	...
	this.comparePoints = function(how, range) {
		var compareNodes = function(nodeA, posA, nodeB, posB) {
			...
			if (KE.browser.IE) {
				...
				var rangeA, rangeB;
				if (how == 'START_TO_START' || how == 'START_TO_END') rangeA = getStartRange

(nodeA, posA, true);
				else rangeA = getStartRange(nodeA, posA, false);
				if (how == 'START_TO_START' || how == 'END_TO_START') rangeB = getStartRange
...
(nodeB, posB, true);
		...
		if (how == 'START_TO_START') return compareNodes(this.startNode, this.startPos, range.startNode, 

range.startPos);
		if (how == 'START_TO_END') return compareNodes(this.startNode, this.startPos, range.endNode, 

range.endPos);
		if (how == 'END_TO_START') return compareNodes(this.endNode, this.endPos, range.startNode, 

range.startPos);
		if (how == 'END_TO_END') return compareNodes(this.endNode, this.endPos, range.endNode, 
...
range.endPos);
	...
	this.setTextStart = function(node, pos) {
		...
		KE.eachNode(node, function(n) {
			if (KE.util.getNodeType(n) == 3 && n.nodeValue.length > 0 || KE.util.getNodeType(n) == 
				...
				if (type == 1) {
					...
					if (isStarted) {
						...
						if (node == startNode && node == endNode) {
							...
							textNode = extractTextNode(node, startPos, 
...
node.nodeValue.length);
							...
							textNode = extractTextNode(node, 0, 
	...
	this.wrap = function(tagName, attributes) {
		...
		KE.eachNode(parentNode, function(node) {
			...
			if (node.nodeType == 1) {
				...
				if (isStarted) {
					if (node == startNode && node == endNode) {
						var rangeNode = self.wrapTextNode(node, startPos, endPos, 
...
node.nodeValue.length, element, attributes);
						keRange.setStart(rangeNode, 0);
					} else if (node == endNode) {
						var rangeNode = self.wrapTextNode(node, 0, endPos, element, 

attributes);
						keRange.setEnd(rangeNode, rangeNode.nodeType == 1 ? 0 : 

rangeNode.nodeValue.length);
						return false;
					} else {
						self.wrapTextNode(node, 0, node.nodeValue.length, element, 
					...
					for (var j = 0, len = attr.length; j < len; j++) {
						if (attr[j] == '*') {
							...
							if (node.attributes.length == 0) {
								KE.util.removeParent(node);
								break;
							} else if (node.attributes[0].name == 'style' && 
	...
	getHtml : function(html, htmlTags, urlType) {
		...
		if (isFilter) {
			KE.each(htmlTags, function(key, val) {
				var arr = key.split(',');
				for (var i = 0, len = arr.length; i < len; i++) htmlTagHash[arr[i]] = 

KE.util.arrayToHash(val);
			});
		}
		...
		html = html.replace(re, function($0, $1, $2, $3, $4, $5, $6) {
			...
			if (attr !== '') {
				attr = attr.replace(/\s*([\w-:]+)=([^\s"'<>]+|"[^"]*"|'[^']*')/g, function($0, 

$1, $2) {
					var key = $1.toLowerCase();
					var val = $2 || '';
					if (isFilter) {
						if (key.charAt(0) === "." || (key !== "style" && typeof 

htmlTagHash[tagName][key] == "undefined")) return ' ';
					}
					if (val === '') {
						val = '""';
					} else {
...
function($0, $1, $2) {
								var k = $1.toLowerCase();
								if (isFilter) {
									if (typeof htmlTagHash[tagName]['style'] 

== "undefined" && typeof htmlTagHash[tagName]['.' + k] == "undefined") return '';
								}
				...
				attr = attr.replace(/\s+(checked|selected|disabled|readonly)(\s+|$)/ig, 

function($0, $1) {
					var key = $1.toLowerCase();
					if (isFilter) {
						if (key.charAt(0) === "." || typeof htmlTagHash[tagName][key] == 
...
endNewline;
		...
		var trimHtml = function(inHtml) {
			var outHtml = inHtml.replace(new RegExp('<(' + reg + ')[^>]*><\\/(' + reg + ')>', 'ig'), 
...
KE.util = {
	...
	getNodeType : function(node) {
		return (node.nodeType == 1 && KE.util.inArray(node.tagName.toLowerCase(), KE.setting.noEndTags)) 
...
? 88 : node.nodeType;
	...
	resize : function(id, width, height, isCheck, isResizeWidth) {
		...
		if (diff >= 0) {
			...
			g.newTextarea.style.height = (((KE.browser.IE && KE.browser.VERSION < 8) || 

document.compatMode != 'CSS1Compat') ? diff - 2 : diff) + 'px';
		}
	},
	...
	setSelection : function(id) {
		...
		if (!KE.browser.IE || keSel.range.item || keSel.range.parentElement().ownerDocument === 
	...
	addNewlineEvent : function(id) {
		...
		KE.event.add(g.iframeDoc, 'keydown', function(e) {
			...
			if (g.newlineTag.toLowerCase() == 'br') {
				...
				if (!KE.util.inArray(tagName, ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'pre', 
...
KE.dialog = function(arg){
	...
	this.show = function() {
		...
		var iframe = (KE.g[id].dialogStack.length == 0 && KE.g[id].dialog) ? KE.g[id].dialog : 
...
KE.readonly = function(id, isReadonly) {
	...
	var g = KE.g[id];
	if (KE.browser.IE && KE.browser.VERSION > 6) g.iframeDoc.body.contentEditable = isReadonly ? 'false' : 
...
KE.create = function(id, mode) {
	...
	if (KE.browser.IE && KE.browser.VERSION < 7) try { document.execCommand('BackgroundImageCache', false, 
...
true); }catch(e){}
	...
	KE.util.drag(id, bottomRight, container, function(objTop, objLeft, objWidth, objHeight, top, left) {
		if (KE.g[id].resizeMode == 2) KE.util.resize(id, (objWidth + left) + 'px', (objHeight + top) + 
...
false);
	...
	KE.g[id].setHtmlHooks.push(function(html) {
		return html.replace(/(<[^>]*)(href|src)="([^"]+)"([^>]*>)/ig, function(full, start, key, src, 
...
'.font-weight',
			...
			a : ['href', 'target', 'name'],
			embed : ['src', 'width', 'height', 'type', 'loop', 'autostart', 'quality', '.width', 

'.height', 'align', 'allowscriptaccess', '/'],
					...
					return function() {
						...
						for (var m = 0; m < num; m++) {
							for (var n = 0; n < num; n++) {
								...
								if (m < x && n < y) cell.style.backgroundColor = 
