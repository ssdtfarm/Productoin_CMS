...
function verifyInput()
{
	...
	for (formsNum=0; formsNum<window.document.forms.length; formsNum++)
	{
		...
		{
			//将所有输入的部分的'过滤，by hanxl，2008年11月11日 09时37分30秒
			window.document.forms[formsNum].elements[elementsNum].value=window.document.forms[formsNum].elements[elementsNum].value.replace(public_Regex,"").trim();
...
function verifyElement(strInfo, strValue, boxName)
{
	...
	for (var k=0; k<operStack.length; k++)
	{
		...
		if (operStack[k] == "|")
		{
			...
			{
				//alert("校验参数设置有误");
				alert(I18NMsg('VerifyInput_Calibrationparameterissetwrong!'));
			}
		passFlag = typeStack[k + 1];
	}

	//var strVerifyErrInfo = "输入有误，可能是如下错误中的一个： \n";
var strVerifyErrInfo = I18NMsg('VerifyInput_EnteredIncorrectly,MayBeOneoftheFollowingErrors:')+"\n";
	if (!passFlag)
	{
		while (arrVerifyErrInfo != "")
		{
...
function verifyMustNull(vName, strValue) {
	if (strValue != "")
	{
		...
		arrVerifyErrInfo.push(vName +" "+I18NMsg('VerifyInput_MustBeEmpty!')+"\n");
		return false;
	}
	return true;
}
...
function verifyNotNull(vName, strValue)
{
	if (strValue == "")
	{
		...
		arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_CanNotBeEmpty!')+"\n");
		return false;
	}
	return true;
}
...
//	if (strValue != "" && isNaN(parseFloat(strValue)) && !isNumeric(strValue))
	if (strValue != "" && !isNumeric(strValue))
	{
		...
		arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_IsNotaValidNumber!')+"\n");
		return false;
	}
	return true;
}
...
function verifyDate(vName, strValue) {
	if (strValue != "" && !isDate(strValue)&&!isDateN(strValue))
	{
		...
		arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_IsNotaValidDateFormat(YYYY-MM-DD)or(YYYYMMDD)!')+"\n");
		return false;
	}
	return true;
}
...
function verifyEmail (vName, strValue)
{
	if (strValue != "")
	{
		...
		if (len > 50)
		{
			...
			arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_Thelengthoftheemailaddresscannotbeover50!'));
			return false;
		}
		var pos1 = s.indexOf("@");
		var pos2 = s.indexOf(".");
		...
		if ((pos1 <= 0)||(pos1 == len)||(pos2 <= 0)||(pos2 == len))
		{
			...
			arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_PleaseEnteraValidEmail!'));
			return false;
		}
		else
		{
		...
		if ( !isCharsInBag( s, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.-_@"))
		{
			...
			arrVerifyErrInfo.push(vName +" "+I18NMsg('VerifyInput_emailaddresscanonlyincludecharrcter')+"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.-_@\n"+I18NMsg('VerifyInput_PleaseRe-enter!'));
			return false;
		}
		var badChar ="><,[]{}?/+=|\:;!#$%^&()`";
		if (isCharsInBag(s,badChar))
		{
			...
			arrVerifyErrInfo.push(vName +" "+""+I18NMsg('VerifyInput_PleaseDoNotEnterCharactersintheEmailAddress')+"" + badChar +"\n");
			return false;
		}
	}
	return true;
...
function verifyDecimal(vName, strValue) {
		...
		arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_Not0to1Decimal!')+"\n");
		return false;
	}
	return true;
}
...
function verifyInteger(vName, strValue) {
	if (strValue != "" && !isInteger(strValue))
	{
		...
		arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_IsNotaValidInteger!')+"\n");
		return false;
	}
	return true;
}
	...
	switch (oper)
	{
		case "=" :
			if (strValue.length != parseInt(len))
			{
				...
				arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_DoesNotMeettheRequiredInputLength!LengthNeedstoBeEqualTo') + len+"\n");
				return false;
			}
			break;
		case ">" :
			if (strValue.length <= parseInt(len))
			{
				...
				arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_DoesNotMeettheRequiredInputLength!LengthMustBeGreaterThan') + len+"\n");
				return false;
			}
			break;
		case "<" :
			if (strValue.length >= parseInt(len))
			{
				...
				arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_DoesNotMeettheRequiredInputLength!LengthMustBeLessThan') + len+"\n");
				return false;
			}
			break;
		case ">=" :
			if (strValue.length < parseInt(len))
			{
				...
				arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_DoesNotMeettheRequiredInputLength!LengthMustBeGreaterThanorEqualTo') + len+"\n");
				return false;
			}
			break;
		case "<=" :
			if (strValue.length > parseInt(len))
			{
				...
				arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_DoesNotMeettheRequiredInputLength!LengthMustBeLessThanorEqualTo') + len+"\n");
				return false;
			}
			break;
	}
...
function verifyValue(vName, strValue, vType)
{
	...
	switch (oper)
	{
		case "=" :
			if (parseFloat(strValue) != parseFloat(Val))
			{
				...
				arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_DoesNotMeettheRequiredValue!InputValuesneedtoBeEqualTo') + Val+"\n");
				return false;
			}
			break;
		case ">" :
			if (parseFloat(strValue) <= parseFloat(Val))
			{
				...
				arrVerifyErrInfo.push(vName + I18NMsg('VerifyInput_DoesNotMeettheRequiredValue!InputValuesneedtoBeGreaterThan') + Val+"\n");
				return false;
			}
			break;
		case "<" :
			if (parseFloat(strValue) >= parseFloat(Val))
			{
				...
				arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_DoesNotMeettheRequiredValue!InputValuesneedtoBeLessThan') + Val+"\n");
				return false;
			}
			break;
		case ">=" :
			if (parseFloat(strValue) < parseFloat(Val))
			{
				...
				arrVerifyErrInfo.push(vName +" "+I18NMsg('VerifyInput_DoesNotMeettheRequiredValue!InputValuesneedtoBeEqualorGreaterThan') + Val+"\n");
				return false;
			}
			break;
		case "<=" :
	...
	arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_Non-complianceValue!NeedtoEnteraValueLessThanorEqualTo')+ Val+"\n");
	return false;
	}
	break;
	}
...
function verifyCode(vName, strValue, vType, returnCode)
{
	...
	else
	{
		...
		if ((arrCode == false) || (arrCode == ""))
		{
			...
			arrVerifyErrInfo.push(I18NMsg('VerifyInput_CODEQueryFunctionError,PleaseContactYourAdministrator!')+"\n");
			return false;
		}
		else if (arrCode == "Code Query Faile")
			{
				...
				arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_TheDatabaseCodeQueryFails,ContactYourAdministrator!')+"\n");
				return false;
			}
			else
			{
	...
	if (!passFlag)
	{
		...
		arrVerifyErrInfo.push(vName +" "+ I18NMsg('VerifyInput_InputDoesNotMeettheRangeoftheDatabaseProvisions.')+"\n");
	}
	return passFlag;
}
...
function verifyInput2()
{
	...
	for (formsNum=0; formsNum<window.document.forms.length; formsNum++)
	{
		...
		for (elementsNum=0; elementsNum<window.document.forms[formsNum].elements.length; elementsNum++)
		{
			//将所有输入的部分的'过滤，by hanxl，2008年11月10日 15时49分31秒
			window.document.forms[formsNum].elements[elementsNum].value=window.document.forms[formsNum].elements[elementsNum].value.replace(public_Regex,"").trim();
