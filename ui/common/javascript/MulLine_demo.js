...
// ****************add by liujing 20121129 start***************

var ObjInstance ;
var arrayHide1 = new Array();   // 包含隐藏列的列号 add by liujing
var arrayHide2 = new Array();   // 保存隐藏列的列号
var oldWidth=new Array();      // 原始表头宽度
var oldValue=new Array();      //原来的表头值
var table;    // 表头
var index=1;      //交换列arrayHide2[a]的数组下标
// ****************add by liujing 20121129 end***************
...
function MulLineEnter(iFormName, iInstanceName)
{
    ...
    //liujing excle
    this.autoParse=0;    //设置是否支持粘贴数据（从EXCEL内）0-不允许粘贴  1-允许粘贴 
	this.rigthMenu=0;    //设置是否使用右键自定义菜单 0-不使用右键菜单，1-使用右键菜单
	
	this.parse = _parse;
    this.showMenu = _showMenu;
    this.hideMenu = _hideMenu;
    this.highLight = _highLight;
    this.lowLight = _lowLight;
...
/*******************************************************************************
 * 方法：提供选中的行数，条件是canSel设置为1 输入： 无 输出： 选择的行数
 * ***********************************************
 */
...
/*******************************************************************************
 * 方法：判断指定行是否选中，条件是canSel设置为1 输入： 无 输出： 如果选中，返回true，否则返回false
 * ***********************************************
 */
...

        {
			...
			)
            {
...

                {
                    ...
                    if (eval(this.formName + ".all('" + this.instanceName + j + "')[" + realNo + "].className=='code8'") || eval(this.formName + ".all('" + this.instanceName + j + "')[" + realNo + "].className=='codeselect'"))

                    {

                        eval(this.formName + ".all('" + this.instanceName + j + "')[" + realNo + "].className='codeselect'");
...

                    {
                        ...
                        eval(this.formName + ".all('" + this.instanceName + j + "')[" + realNo + "].className='mulnotreadonlyt'");
             	...
             	{        				
        				...
        				
								{
        				...
        				
									{
										...
										eval( this.formName + ".all('" + this.instanceName +j+"')[" + realNo + "].className='code8'");
        				...
        				
									{
										...
										eval( this.formName + ".all('" + this.instanceName +j+"')[" + realNo + "].className='mulreadonlyt'");
...
function _LoadMulLine(arrCols)
{
    ...
    this.arraySave3 = arrCols.Clone();// 保存原始信息
...
/*******************************************************************************
 * add by jinsh 带可编辑按纽单元格的mulline
 ******************************************************************************/
...
//保存原始表头每一列的宽度   
    for (var z = 0; z < arrCols.length; z++){
        table =document.getElementById(cStrPageName + "ExChange");
        if(table!=null){
	        oldWidth[z]=table.rows[0].cells[z].firstChild.style.width;
	        oldValue[z]=table.rows[0].cells[z].firstChild.value;
        }
    }
    
 //add by liujing 20121211 start
    
    ////  alert("列数"+ cObjInstance.arraySave.length);//包含序号列
    ////  alert(cObjInstance.arraySave[0][0]);//原始列序名称
    ////  alert("原列序："+cObjInstance.arraySave[0][0]+" "+cObjInstance.arraySave[1][0]+" "+cObjInstance.arraySave[2][0]+" "+cObjInstance.arraySave[3][0]+" "+
////    		  cObjInstance.arraySave[4][0]+" "+cObjInstance.arraySave[5][0]+" "+cObjInstance.arraySave[6][0]+" "+cObjInstance.arraySave[7][0]+" "+
////    		  cObjInstance.arraySave[8][0]);
//        alert("新列序："+table.rows[0].cells[1].firstChild.value+" "+table.rows[0].cells[2].firstChild.value+" "
//        		+table.rows[0].cells[3].firstChild.value+" "+table.rows[0].cells[4].firstChild.value+" "+table.rows[0].cells[5].firstChild.value+" "
//        		+table.rows[0].cells[6].firstChild.value+" "+table.rows[0].cells[7].firstChild.value+" "+table.rows[0].cells[8].firstChild.value);

//       alert("come");
//        z=this.arraySave.length-1;
//        table =document.getElementById(cStrPageName + "ExChange");
//        for (var k = 0; k < tab_alldata.length; k++)
//        {
//            if (tab_alldata[k].id.indexOf(this.instanceName + "ExChange") != -1)
//            {
//            	var g1=arrayHide2.length-1;
////            	alert(g1);
//            	if(g1>0){
//            		for(;g1>0;g1=g1-2){
////            			alert(1+"  g1"+g1);
////            			alert(g1+" "+table.rows[0].cells[arrayHide2[g1]].firstChild.value);
////            			alert(g1-1+" "+table.rows[0].cells[arrayHide2[g1-1]].firstChild.value);
////            			alert(arrayHide2[1]+" "+arrayHide2[2]+" "+arrayHide2[3]+" "+arrayHide2[4]+" "+arrayHide2[5]+" "+arrayHide2[6]);
////            		alert(arrayHide2[g1]+" change "+arrayHide2[g1-1]);
//            			table.rows[0].cells[arrayHide2[g1]].swapNode(table.rows[0].cells[arrayHide2[g1-1]]);
////            		alert(21);
////            		alert(3+"  g1"+g1);
////            		arrayHide2[g1]="undefined";
////            		arrayHide2[g1-1]="undefined";
//            		}
//            	}
//            	k=tab_alldata.length;
//            }
//        }
//    add by liujing 20121211 end
// ****************add by liujing 20121129 end*****************
...
/*******************************************************************************
 * 初始化Array列表的顺序 初始化默认为1,2,3,4,5.....依次递增 add by wanglei 2008-07-24
 ******************************************************************************/
function _InitArrCol(oldCol, cObjInstance)
{
    ObjInstance = cObjInstance;
	cObjInstance.totalPre=0;// add by liujing 20121113
...
//    z=cObjInstance.arraySave.length-1;
//    table =document.getElementById(cObjInstance.instanceName + "ExChange");
//    for (var k = 0; k < tab_alldata.length; k++)
//    {
//        if (tab_alldata[k].id.indexOf(cObjInstance.instanceName + "ExChange") != -1)
//        {
//        	var g1=arrayHide2.length-1;
////        	alert(g1);
//        	if(g1>0){
...
/*******************************************************************************
 * 根据拖动后的顺序数组(newColOrder),翻页时交换mulline的title的column add by wanglei 2008-07-24
 ******************************************************************************/
...
/*******************************************************************************
 * 根据拖动后的顺序数组(newColOrder),翻页时交换载入数组数据的顺序 add by wanglei 2008-07-24
 ******************************************************************************/
...
/*******************************************************************************
 * 方法：根据传入的数组，形成每行输入域的模版 二维数组格式：列名，列宽，列最大值，列是否能够输入
 ******************************************************************************/

function _SetFieldValue(strPageName, iArray, cObjInstance, eArray)// 新增一参数eArray
																	// 用来存放按纽信息.add
																	// by jinsh
{
    // alert(eArray);
...
// //加上radioBox宽
// userWidth = userWidth + parseInt(boxWidth);
// if (cObjInstance.selBoxEventFuncName != null &&
// cObjInstance.selBoxEventFuncName != "")
// {
// // text = text + "<td class=muline width='"+boxWidth+"'>";
// text = text + "<td class=muline>";
// text = text + "<input type=hidden name='Inp" + strPageName + "Sel'
// value=0><input class=mulcommon style='width:" + boxWidth + "' type=radio
// name=" + strPageName + "Sel onclick=\" " + cObjInstance.instanceName +
// ".radioClick(this," + fieldCount + ");" + cObjInstance.selBoxEventFuncName +
// "('span$PageName$$SpanId$','" + cObjInstance.selBoxEventFuncParm + "');\"";
// //alert(text);
// text = text + "</td>";
// //alert(text);
// // textTitle = textTitle + "<td class=mulinetitle
// style='width:"+boxWidth+"'>";
// textTitle = textTitle + "<td class=mulinetitle>";
// textTitle = textTitle + "<input class=mulinetitle readonly style='width:" +
// boxWidth + "'>";
// textTitle = textTitle + "</td>";
// }
// else
...
// text = text + "<input type=hidden name='Inp" + strPageName + "Sel'
// value=0><input class=mulcommon style='width:" + boxWidth + "' type=radio
// name=" + strPageName + "Sel onclick=\"return " + cObjInstance.instanceName +
// ".radioClick(this," + fieldCount + ");\"";
// text = text + "</td>";
...
// //加上checkBox宽
// userWidth = userWidth + parseInt(boxWidth);
// if (cObjInstance.chkBoxAllEventFuncName != null &&
// cObjInstance.chkBoxAllEventFuncName != "")
// {
// // textTitle = textTitle + "<td class=mulinetitle width='"+boxWidth+"'>";
// textTitle = textTitle + "<td class=mulinetitle>";
// textTitle = textTitle + "<input class=title type=checkbox name='checkAll" +
// cObjInstance.instanceName + "' onclick=\" " + cObjInstance.instanceName +
// ".checkAll(this," + fieldCount + ");" + cObjInstance.chkBoxAllEventFuncName +
// "(this.checked,this);\"" + " style='width:" + boxWidth + "'>";
// textTitle = textTitle + "</td>";
// }
// else
// {
...
// textTitle = textTitle + "<input class=title type=checkbox name='checkAll" +
// cObjInstance.instanceName + "' onclick=\" " + cObjInstance.instanceName +
// ".checkAll(this," + fieldCount + ");\" style='width:" + boxWidth + "'>";
// textTitle = textTitle + "</td>";
// }
// if (cObjInstance.chkBoxEventFuncName != null &&
// cObjInstance.chkBoxEventFuncName != "")
// {
// // text = text + "<td class=muline width='"+boxWidth+"'>";
// text = text + "<td class=muline>";
// text = text + "<input type=hidden name='Inp" + strPageName + "Chk'
// value=0><input class=mulcommon style='width:" + boxWidth + "' type=checkbox
// name=" + strPageName + "Chk onclick=\"" + cObjInstance.instanceName +
// ".checkBoxClick(this," + fieldCount + ");" + cObjInstance.chkBoxEventFuncName
// + "('span$PageName$$SpanId$','" + cObjInstance.chkBoxEventFuncParm + "');\"";
// text = text + "</td>";
// }
// else
// {
...
// text = text + "<input type=hidden name='Inp" + strPageName + "Chk'
// value=0><input class=mulcommon style='width:" + boxWidth + "' type=checkbox
// name=" + strPageName + "Chk onclick=\"return " + cObjInstance.instanceName +
// ".checkBoxClick(this," + fieldCount + ");\"";
// text = text + "</td>";
// }
// }
            ...
            for (i = 1; i < fieldCount; i++)
            {
                ...
                var tempTextTitle1 = tempText1 - 1;// add by wanglei 2008-07-25
													// for td e-resize
                if (tempText1 == 0)
                {
                    tempTextTitle1 = 0;
                ...
                {
					textTitle = textTitle
							+ "<td class=mulinetitle><input class=mulinetitle readonly value="
							+ tempText0 + " style='width :" + tempText1
							+ "' onmouseover=_showtitle(this);>";
					text = text + "<td class=muline width='" + tempText1
							+ "'><input name=" + strPageName + i + " value='"
							+ tempText15 + "' " + isReadOnly(tempText4)
							+ " dateFormat=short onclick=calendar();" + " class="
							+ isReadOnlyClass(tempText4) + " maxlength="
							+ tempText2 + " onmouseover=_showtitle(this); ";
				}
                //end
...
//update by liujing text = text + "<td class=muline width='" + tempText1 + "'><input name=" + strPageName + i + " value='" + tempText15 + "' " + isReadOnly(tempText4) + " class=" + isReadOnlyClass(tempText4) + " maxlength=" + tempText2 + " onmouseover=_showtitle(this); ";
                }
                if (tempText5 == null)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            // arrColName=fm.all('spanXXXID').all('XXX+Col')
							// 即对应spanID的列对象
                ...
                // text = text + " onkeyup='return " + cObjInstance.instanceName
				// + ".keyUp(\"$PageName$\");'";
            ...
            if ((eArray != null) && (eArray != '') && (eArray != ""))
            {
                ...
                // text = text + "<td class=muline><input readonly type=button
				// class=cssButton style='width:40' value='编辑'
				// onmouseover=_showtitle(this) onclick=\"return
				// jinsh(this);\">";
                // textTitle = textTitle + "<td class=mulinetitle><input
				// class=mulinetitle readonly style='width:40' value='编辑'
				// onmouseover=_showtitle(this)>";
                // text = text + "</td>";//jinsh20080927
                // textTitle = textTitle + "</td>";//jinsh20080927
                // text = text + "<td class=muline><input readonly type=button
				// class=cssButton style='width:40' value='删除'
				// onmouseover=_showtitle(this) onclick=\"return
				// jinsh(this);\">";
                // textTitle = textTitle + "<td class=mulinetitle><input
				// class=mulinetitle readonly style='width:40' value='删除'
				// onmouseover=_showtitle(this)>";
                // text = text + "</td>";//jinsh20080927
                // textTitle = textTitle + "</td>";//jinsh20080927
            }
...

{
  ...
  //add liujing  
    if(cObjInstance.rigthMenu=="1")
    {       
    	tHTML = tHTML + "<span id='span" + t_StrPageName + "Field' oncontextmenu='" + cObjInstance.instanceName + ".showMenu();return false;'  onclick=\"" + cObjInstance.instanceName + ".hideMenu();\"></span>";
    }
    else
    {
    	tHTML = tHTML + "<span id='span" + t_StrPageName + "Field'></span>";
    }
  //  tHTML = tHTML + "<span id='span" + t_StrPageName + "Field'></span>";//update by liujing 
   ...
   //liujing excel 
  //隐藏起来的右键菜单栏  
    tHTML = tHTML + "<div id='menuitems' class='menuitems' onMouseover='"+t_StrPageName+".highLight()' onMouseout='"+t_StrPageName+".lowLight()' onClick='"+t_StrPageName+".menuAction();'>"
    if(cObjInstance.autoParse=="1")
    { 
    	tHTML = tHTML + "<div class='menuitem' js='"+t_StrPageName+".parse();"+t_StrPageName+".hideMenu();'>粘 贴</div><hr>";
    }
    tHTML=tHTML+"</div>";
...
//        table =document.getElementById(cObjInstance.instanceName + "ExChange");
//        z=cObjInstance.arraySave.length-1;
//  	  var g=arrayHide2.length-1;
////  	  alert("g="+g);
//  	  for (; z >= 1; z--){
//          if(cObjInstance.arraySave[z][3] == 3){
//        	  cObjInstance.arraySave[z][3] = 0;
//  	        table.rows[0].childNodes[z].style.display = "";
//  	    }
//  		var flag=0;
//  		if(table.rows[0].cells[z].firstChild.value!=""&&table.rows[0].cells[z].firstChild.value!="-"){
////  			alert(table.rows[0].cells[z+1].firstChild.value+" "+cObjInstance.arraySave[z][0]);
//  		if(table.rows[0].cells[z].firstChild.value!=cObjInstance.arraySave[z][0]){
//  			flag++;
////  			alert(flag);
//  		}
////  		alert("flag="+flag);
...
////  			alert("cac");
//  			if(g%2==0){
////  				alert(arrayHide2[g]+"in change"+arrayHide2[g-1]);
////  				alert(arrayHide2[g]);
////  				alert(arrayHide2[g-1]);
//  				table.rows[0].cells[arrayHide2[g]].swapNode(table.rows[0].cells[arrayHide2[g-1]]);
//  				alert("add1one");
////  		          alert(arrayHide2[1]+" "+arrayHide2[2]+" "+arrayHide2[3]+" "+arrayHide2[4]+" "+arrayHide2[5]+" "+arrayHide2[6]);
////  				    	   var tablecols = tableField.childNodes;
////  				    	   for(var i=0;i<tablecols.length;i++){
////  				    		    var tablecol = tablecols[i].firstChild;
////  							    tablecol.rows[0].cells[arrayHide2[g]].swapNode(tablecol.rows[0].cells[arrayHide2[g-1]]);
////  			              }	
//  				    	  
//  		 }
////  			 g++;
...
//  			flag=0;
////  			 for(var e=g;e=g;e++){
////  				 arrayHide2[e]="undefined";
////  			}
//  		}
////  		alert("length"+arrayHide2.length);
//  		} 
//  	}
...
// ********************add by liujing start 20121128**********************
    
    var tableField =document.getElementById("span"+t_StrPageName + "Field");
//// alert("列数"+ tObjInstance.arraySave.length);
////	var tables = tableField.getElementsByTagName("table");
////	var count = tables.length;
////	  var z = 1;
    var z=tObjInstance.arraySave.length-1;
//	  tObjInstance.totalPre=0;
//	  if (tObjInstance.canSel == 1)
//	    {
//		  tObjInstance.totalPre = tObjInstance.totalPre + 1;
//	    }
//	  if (tObjInstance.canChk == 1)
//	    {
//	    	tObjInstance.totalPre = tObjInstance.totalPre + 1;
//	    }
	  table =document.getElementById(t_StrPageName + "ExChange");
////	  var g=1;
	  var g=arrayHide2.length;
//	  var g=arrayHide2.length-1;
////	  alert("g="+g);
////	  alert(tObjInstance.arraySave.length);
////	  for(z=1;z<9;z++){
////		 alert(z+" "+table.rows[0].cells[z].firstChild.value );
////	  }
////	  for(z=0;z<6;z++){
////			 alert(z+" vv "+oldValue[z]);
////		  }
//	  
//	  
	  for (; z >= 1; z--){
        if(tObjInstance.arraySave[z][3] == 3){
	        tObjInstance.arraySave[z][3] = 0;
	        table.rows[0].childNodes[z].style.display = "";
	    }
        if (!isInit)
...
//		alert(43);
		if(table.rows[0].cells[z].firstChild.value!=""&&table.rows[0].cells[z].firstChild.value!="-"){
//			var ab=tObjInstance.arraySave[z][0];
//			if(tObjInstance.arraySave[z][3] == 3){
//				ab=tObjInstance.arraySave[z-1][0];
//			}
//			alert(table.rows[0].cells[z+1].firstChild.value+" "+ab);
//		if(table.rows[0].cells[z].firstChild.value!=ab){
//			flag++;
//			alert(flag);
//		}
//		alert(g);
...
////				alert(arrayHide2[g-1]);
////				alert(321);
				table.rows[0].cells[arrayHide2[g-1]].swapNode(table.rows[0].cells[arrayHide2[g-2]]);
////				alert("addone");
//		        alert(arrayHide2[1]+" "+arrayHide2[2]+" "+arrayHide2[3]+" "+arrayHide2[4]+" "+arrayHide2[5]+" "+arrayHide2[6]);
				    	   var tablecols = tableField.childNodes;
				    	   for(var i=0;i<tablecols.length;i++){
				    		    var tablecol = tablecols[i].firstChild;
//				    		    alert("arrayHide2["+(g-1)+"]="+arrayHide2[g-1]);
//				    		    alert("arrayHide2["+(g-2)+"]="+arrayHide2[g-2]);
							    tablecol.rows[0].cells[arrayHide2[g-1]].swapNode(tablecol.rows[0].cells[arrayHide2[g-2]]);
//				    		    tablecol.rows[0].cells[arrayHide2[g]].swapNode(tablecol.rows[0].cells[arrayHide2[g-1]]);
			              }	
				    	   arrayHide2[g-1]=0;  
				    	   arrayHide2[g-2]=0;
...
//        alert(strOldText);
        var a="; DISPLAY: none\" class=muline";// add by liujing 20121207
        var b="\" class=muline";                 // add by liujing 20121207
        strOldText=replace(strOldText, a, b);      // add by liujing 20121207
//        alert(strOldText);
        // 添加intCount行
//        alert("行数"+intCount+t_StrPageName);
        var li=intCount-1;
//        var upLine = document.all("span" + t_StrPageName + li).innerHTML;
//        alert(("span" + t_StrPageName + li).Value=="   spanAuthorizeGrid2".trim().value);
...
//        	var upLine = document.all("span" + t_StrPageName + li).innerHTML;
//            alert(upLine.innerHTML);
//        	alert("span" + t_StrPageName + li+"   "+spanAuthorizeGrid1);
        }
...
//        var upLine = document.all("span" + t_StrPageName + li).innerHTML;
//        alert(upLine);
//        strText = "<span id='span" + t_StrPageName + spanID + "'>" + upLine + "</span>";
//        strOldText =strOldText + strText;
        // fm.all('tt').value=strOldText;
        // 注意顺序，必须是在加载变化后的文本后指定焦点
        // alert("列的数目"+cObjInstance.colCount);
...

            {
...

                {
...

                    {
                        ...
                        // 外部函数，请察看verifyInput.js parm1:位置|检验规则 parm2:
						// 要检验的值(即第N行i列的值)
...
//var a=1;
/*******************************************************************************
 * 交换表中的两列，需要遍历每一行，然后交换每一个元素。因为没有表列对象 add by wanglei 2008-08-01
 ******************************************************************************/
function ChangeColumn(table, sor, tag)
{
    try
    {
        ...
        for (var i = 0; i < table.rows.length; i++){
            table.rows[0].cells[sor].swapNode(table.rows[0].cells[tag]);// swapNode实现互换
        }
...
//完成移动到固定地方，其余列依次后移或者前移 tangyj20130913
function ChangeColumnr(table, sor, tag)
{
    ...
    try
    {
        ...
        if(sor>tag)
        {
    	for(var i=sor;i>tag;i--)
    	{
    		
            table.rows[0].cells[i].swapNode(table.rows[0].cells[i-1]);// swapNode实现互换
    	}
        }else
        {
        	for(var i=sor;i<tag;i++)
...
/*******************************************************************************
 * 将某一列拖动到固定的地方，其它一次移动 add by tangyj 2013-09-13
 ******************************************************************************/
...
function ChangeColumnR(table, sor, tag)
{
    ...
    try
    {
    	...
    	{
    		//alert();
    		//alert(table.rows[3].cells[sor]);
    		//首先交换之前将tag的值，存入一个数组
    		//var a=new Array();
    		var a=[];
    	
    		for (var j = 0; j< table.rows.length; j++){
    			a[j]=table.rows[j].cells[sor];
    		}
    		//alert(a[3]);
    		//依次往后移动
    		for(var i=tag;i>sor;i--)
    		{
    			table.rows[i].cells[sor]=table.rows[i-1].cells[sor];
    			//table.rows[i].cells[sor].swapNode(table.rows[i].cells[tag]);	
    		}
    		//将后面那个拖动的放入sor的位置
    		for (var k = 0; k< table.rows.length; k++)
    		{
    			table.rows[k].cells[sor]=a[k];
    			//table.rows[i].cells[sor].swapNode(table.rows[i].cells[tag]);	
    		}
    	}  
        return 1;
...
/*******************************************************************************
 * 拖动并交换列,动态的记录newColOrder的值 add by wanglei 2008-08-01
 ******************************************************************************/
function ChangeColOrder(sor, tag, obj)
{
    ...
    try
    {
        ...
        tObjInstance.newColOrder[tag - tObjInstance.totalPre] = temp;// 交换值
        return 1;
    }
    catch(ex)
    {
...
/*******************************************************************************
 * 拖动并交换列,及里面的数值 add by wanglei 2008-08-01
 ******************************************************************************/
function _MoveSpan(obj, t_StrPageName, obj1)
{
    ...
    function init()
    {
        ...
        dragdiv.onselectstart = function()// 禁止鼠标选中文字 onselectstart="return false"
        {
            return false
        };
        document.body.appendChild(dragdiv);
    }
    var nx = event.x,ny = event.y;// 当前鼠标位置
    var tab_data = document.getElementById(t_StrPageName);// t_StrPageName是mulline名
    ...
    function setspan()// 动态的显示一个拖动中的表“td”对象
    {
        ...
        var l = cell.offsetLeft;// offsetLeft:Html元素相对于自己的offsetParent元素的位置
        ...
        dragdiv.style.width = obj.offsetWidth;// 当前列的宽高
        dragdiv.style.height = obj.offsetHeight;// offsetHeight:获取对象相对于版面或由父坐标
    ...
    function get_Element(the_ele, the_tag)// 利用此函数，可以避免出现错误。当拖动目标移到表外时，将忽略错误。
    {
    ...
    document.onmouseup = function()
    {
        ...
        var the_end = get_Element(document.elementFromPoint(event.x, event.y), "td");// elementFromPoint找到文档中任意一点所对应的element
        ...
        var testfrist = get_Element(document.elementFromPoint(curleftX, curtopY), "td");
        var testsecond = get_Element(document.elementFromPoint(currightX, curtopY), "td");
        if (the_end != null)
        {
        ...
        try{
        if(testfrist.className.toLowerCase()=='mulinetitle')// update by liujing 20121119
        {
            ...
            for (var k = 0; k < tab_alldata.length; k++)
            {
                if (tab_alldata[k].id.indexOf(t_StrPageName + "ExChange") != -1)// t_StrPageName+"ExChange"是mulline的id
                {
                	...
                	//实现放到固定列其余后或者前移tangyj2013-09-13
                	tCflag = ChangeColumnr(tab_alldata[k], dragColStart, dragColEnd);
                }
            ...
            if (tCflag == 1)
            {
                ...
                arrayHide2[index]=dragColStart;   //add by liujing
                arrayHide2[index+1]=dragColEnd;   //add by liujing
//                alert(arrayHide2[0]+" "+arrayHide2[1]+" "+arrayHide2[2]+" "+arrayHide2[3]+" "+arrayHide2[4]+" "+arrayHide2[5]);
                index=index+2;   //add by liujing
            }
        }
        else
        {
...
function _MouseDownToResize(obj)
{
    ...
    obj.setCapture();// setCapture该函数在属于当前线程的指定窗口里设置鼠标捕获
//    alert(obj.mouseDownX+" "+obj.pareneTdW+" ");
}
var tCol = 0;
var newWidth = 0;
/*******************************************************************************
 * 按下鼠标拖动时，动态的改变列的宽度 add by wanglei 2008-08-12
 ******************************************************************************/
function _MouseMoveToResize(obj, StrPageName)
{
    tCol = getCellIndex(obj.parentElement);
    if (!obj.mouseDownX) return false;
    newWidth = obj.pareneTdW * 1 + event.clientX * 1 - obj.mouseDownX;
    if (newWidth > 0)
    {
//        obj.parentElement.childNodes[0].style.width = newWidth;// update by liujing
        for (var k = 0; k < tab_alldata.length; k++)
        {
            if (tab_alldata[k].id.indexOf(StrPageName + "ExChange") != -1)
            {
                tab_alldata[k].rows[0].childNodes[tCol].style.width = newWidth;
//                tab_alldata[k].rows[0].childNodes[tCol].childNodes[0].style.width = newWidth;// update by liujing
                tab_alldata[k].rows[0].childNodes[tCol].childNodes[0].style.width = newWidth-1;// update by liujing
            }
        }
    }
}
/*******************************************************************************
 * 鼠标抬起后，记录目前的宽度 add by wanglei 2008-08-12
 ******************************************************************************/
...
/*******************************************************************************
 * 显示需要筛选的列 add by wanglei 2008-08-13
 ******************************************************************************/
function _ShowColChoose(obj, t_StrPageName, insobj)
{ 
    try
    {
       // alert(insobj.arrayChoose);选中了几列就弹出几个check
        var t = obj.offsetTop;
        var l = obj.offsetLeft;
        while (obj = obj.offsetParent)
        {
            l += obj.offsetLeft;
            t += obj.offsetTop;// offsetTop表示元素相对于窗口上边的偏移量,但它是一个数值,例如:3
        }
        ...
        newDiv.style.top = t;// 一个块元素相对于它的原始位置在竖直方向上的偏移量,以"px"结尾的字符串
        newDiv.style.left = l;
        newDiv.style.background = "#FFFFFF";
        newDiv.style.border = "1px solid #860001";// 边框1px实线
        ...
        {
       // this.arraySave3[i][i]每一列表头的名字，宽度等等
            if (this.arraySave3[i][3] != 3 && this.arraySave3[i][1] != "0px")// this.arraySave3[i][3]是否允许输入,1表示允许，0表示不允许
            {
                var t = insobj.arrayChoose[i];// arrayChoose包含选择列信息的array
...
/*******************************************************************************
 * 记录checkbox的选中状态 add by wanglei 2008-08-21
 ******************************************************************************/
...
/*******************************************************************************
 * 排序,这个不是很好,是把DataResult的数据按照ascii码来排 DataResult可能有数量限制
 * 就目前的sql写法，我也想不出来如何从数据库层进行排序 add by wanglei 2008-08-14
 ******************************************************************************/
...
/*******************************************************************************
 * 确定所筛选列 add by wanglei 2008-08-13
 ******************************************************************************/
...
{ 
    ...
    try
    {
    // arraySave3包含变化后宽度信息的array
        for (var m = 1; m < tObjInstance.arraySave3.length; m++)
        {
     
            if (tObjInstance.arraySave3[m][3] == 3 || tObjInstance.arraySave3[m][1] == "0px")// tObjInstance.arraySave3[m][3]索引列是否允许输入,隐藏，代码选择
            {
                continue;
            }
            var oraindex = "";
            var choosebox = document.getElementById(t_StrPageName + "choose" + m);
              // t_StrPageName+"choose"+m点击序号后弹出框里每一列的checkbox名字
            var l = 0;
            for (var z = 1; z < tObjInstance.newColOrder.length; z++)  //newColOrder是列的顺序
            {
                var t = tObjInstance.newColOrder[z];// t是数字（包含序号列的列数）
                if (t - 1 == m)
                {
                    l = z;
                    if (choosebox.checked)
                    {
                        ...
                        for (var k = 0; k < tab_alldata.length; k++)  // tab_alldata是TABLE
                        {
                    ...
                    else
                    {
                        ...
                        for (var k = 0; k < tab_alldata.length; k++)
                        {
                     // t_StrPageName+"ExChange"整个mulline的id返回字符中indexof（string）中字串string在父串中首次出现的位置，从0开始！没有返回-1
                            if (tab_alldata[k].id.indexOf(t_StrPageName + "ExChange") != -1)
                            {
                                tab_alldata[k].rows[0].childNodes[l + tObjInstance.totalPre].style.display = "none";// tObjInstance.totalPre=1
                            }
                        }
                        break;
                    }
        ...
        document.body.removeChild(document.getElementById("newdiv"));// newdiv是点序号出现的框
    }
    catch(ex)
    {
        document.body.removeChild(document.getElementById("newdiv"));
...
/*******************************************************************************
 * 为Object增加Clone方法,因为有些值是需要一直保留的(在对象销毁之前) add by wanglei 2008-08-13
 ******************************************************************************/
...
/*******************************************************************************
 * 这个为IE6-7的一个bug写的方法,ff不存在这个问题 唉,真搞不懂微软为什么要这么设计. add by wanglei 2008-08-13
 ******************************************************************************/
...
//2010-4-13
function rememberSelectedRows(arr, index, selectItem, maxpage){
	try{
		var arrs = new Array(maxpage);
		if(arr == null){
			if(selectItem == ""){
				return arrs;
			}
			var splitItems = selectItem.split(',');
			var selItems = "";
			for(var i = 0;i < splitItems.length;i++){
				if(splitItems[i] != ""){
					if(selItems == ''){
						selItems = splitItems[i];
					}else{
						selItems = selItems + "," + splitItems[i];
					}
				}
			}
			arrs[index] = selItems;
			...
			for(var i = 0;i < splitItems.length;i++){
				if(splitItems[i] != ""){
					if(selItems == ''){
						...
						if(selItems.indexOf(splitItems[i]) < 0){
							selItems = selItems + "," + splitItems[i];
						}
					}
				}
...
function _parse()
{
    ...
    try
    {
        if(oInstance.autoParse!=1)
        {
        	alert(oInstance.instanceName+"中的参数autoParse设置为不可粘贴！ ");
        	return;
        }
        if (oInstance == null ||  typeof(oInstance) == "undefined" )
        {
            alert("在 MulLine.js --> _parse 函数中发生错误：实例名和控件名不允许为空！ ");
            return;
        } 
        
        //获取需要粘贴起始的行，列
        var needWrierRowNo = oInstance.lastFocusRowNo;
		var needWrierColNo = oInstance.lastFocusColNo;
		//获取剪贴板内容 并拆分成一个二维数组
		var boardDatatest=window.clipboardData.getData("text");
		if(boardDatatest==""||boardDatatest==null)
		{
			return;
		}
		var Datatestone = new Array();	
		Datatestone = boardDatatest.split("\r\n");			
		var Datatesttwo = new Array();			
		for(var i=0;i<Datatestone.length;i++)	
		{
			Datatesttwo[i]=new Array();
			Datatesttwo[i]=Datatestone[i].split("\t");				
		}
		
		//若粘贴位置非法，默认粘贴起始位置在第一个输入框处
		if(needWrierRowNo<0)
		{
			needWrierRowNo=0;
		}
		...
		//填充数据	 
		for(var i=0;i<Datatesttwo.length;i++)	
		{
			//最后一个换行符后面为空值,则不需要自动增加行数
			if((i==Datatesttwo.length-1)&&(Datatesttwo[i][0]==""))
			{					
				break;
			}
			//行数不够时，自动增加一行
			if(((needWrierRowNo+i-0)>=oInstance.mulLineCount))	
			{					
				oInstance.addOne();				
			}		
			for(var j=0,index=0;index<Datatesttwo[i].length;j++)
			{					
				//粘贴的列数大于MulLine定义的列数  后面数据将丢失
				if((needWrierColNo-0+j)>=oInstance.colCount)	
				{
					break;
				}
				//alert((needWrierColNo-0+j)+"  "+oInstance.colCount);
				//0-不允许输入; 1-允许输入; 2-代码选择; 3-隐藏; 4-密码输入; 5-需两次输入确认
				//对0，3，4，5这些特殊情况，为安全考虑，不允许粘贴  对列宽为0px的输入框不粘贴 数据依次往后面的列数填充
				if((this.arraySave[needWrierColNo-0+j][3]!='1'&&this.arraySave[needWrierColNo-0+j][3]!='2')||this.arraySave[needWrierColNo-0+j][1]=='0px')
				{					
					continue;
				}				
				oInstance.setRowColData(needWrierRowNo-0+i,needWrierColNo-0+j,Datatesttwo[i][index],oInstance);				
				//alert((needWrierRowNo-0+i)+"   "+(needWrierColNo-0+j)+"  "+ Datatesttwo[i][index]);
				index++;
			}		
		}
		//oInstance.delBlankLine();	     //删除空白行   
    }
    catch (ex)
    {
        _DisplayError("在 MulLine.js --> _parse 函数中发生异常：", oInstance);
    }
}
...
function _showMenu()
{
	...
	if (rightedge < menuitems.offsetWidth)
		menuitems.style.left = document.body.scrollLeft + event.clientX - menuitems.offsetWidth;
	else
		menuitems.style.left = document.body.scrollLeft + event.clientX;
	if (bottomedge < menuitems.offsetHeight)
		menuitems.style.top = document.body.scrollTop + event.clientY - menuitems.offsetHeight;
	else
		menuitems.style.top = document.body.scrollTop + event.clientY;
...
function _menuAction() {
	if (event.srcElement.className == "menuitem") {
		if (event.srcElement.getAttribute("js") != null)
			eval(event.srcElement.getAttribute("js"));
		if (event.srcElement.getAttribute("target") != null && event.srcElement.getAttribute("url") != null)
