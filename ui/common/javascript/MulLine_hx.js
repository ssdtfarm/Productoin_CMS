...
function _radioClick(cObj,colcount)
{
	...
	{
		_DisplayError("在MulLine.js-->_radioClick函数中发生异常:" + ex,this);
	}
}
		...
		if ( fieldCount > 0 )
		{
			...
			for ( i =1; i<fieldCount; i++)
			{
				...
				if(tempText4=='3')
				{
					...
					text = text + "<td class=mulinetitle style=\"display:'none'\"><input name=" + strPageName + i + " type=hidden value='"+tempText15+"' maxlength=" + tempText2 + "";
				}
				else
				{
					...
					{
						textTitle = textTitle + "<td class=mulinetitle width='" + tempText1 + "'>"
							+ "<input class=mulinetitle1 readonly tabindex=-1 value="
...
function _LoadPage(strPageName,cObjInstance)
{
	...
	if(cObjInstance.hiddenPlus==0)
	{
		...
		{
			innerHTML = innerHTML + "<input class=button type=button name='"+t_StrPageName + "addOne' value='+' "+status+" onclick=\" " + cObjInstance.instanceName + ".addOne('" + t_StrPageName + "'); "+cObjInstance.instanceName+".moveFocus(); "
...
function _SetPageMark(cTurnPage){
    ...
    innerHTML += "<input type='button' class=cssbutton value='->' onclick='"+tStrPageName+".SortPage.gotoPage(document.all.GotoPage"+tStrPageName+".value);'>";
    innerHTML += "</div>";
