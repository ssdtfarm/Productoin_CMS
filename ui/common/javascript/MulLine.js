...
function MulLineEnter(iFormName, iInstanceName){
    ...
    this.displayPageMark = 1;//是否显示 页码 翻页
...
function setupSelStyle(formName,instanceName,col,pageNo){
	...
	try{
		...
		if( eval(tObj+"=='code8'")||eval(tObj+"=='codeselect'"))          {     
  		...
  		else if( eval(tObj+"=='mulLineDatePicker'") || eval(tObj+"=='multiCurrency'") ){
			if( pageNo == null ){
...
function _GetChkNo(cIndex, cObj){
    ...
    if ((cIndex < 0) || (cIndex >= tObj.mulLineCount)){
        alert("MulLine.js-->getChkNo"+ I18NMsg('MutLine_FunctionSpecifiedtheWrongLine') +":" + cIndex);
        return false;
    }
    try{
...
function _CheckBoxClick(cObj, colcount){
    ...
    try{
		...
		//_DisplayError("在MulLine.js-->_checkBoxClick函数中发生异常:" + ex.message,this);
	}
}
/*******************************************************************************
 * 
...
function _CheckBoxSel(row, cObj){
    ...
    if ((rowNo > iMax) || (rowNo <= 0)){
        alert(I18NMsg('MutLine_EntertheLineNumberoutofRange'));
        return;
    }
    try{
        if (this.mulLineCount == 1){
...
//    this.arraySave3 = arrCols.Clone();// 保存原始信息
    extend(this.arraySave3,arrCols);//由于clone冲突问题，使用新的深拷贝方法
 ...
 ******************************************************************************/
function _SetFieldValue(strPageName, iArray, cObjInstance, eArray){
    var text = "";
    var textTitle = "";
    ...
    try{
        if (fieldCount > 0){
            ...
            var tempText22 = "";
            var tempText23 = "";
            var tempText24 = ""; //设置当前列点击按钮
            var tempText25 = ""; //设置单列触发onchange方法
            var tempText26 = "";
            var tempText27 = ""; 
            var tempText28 = "";
            var showTitleText = "";
            ...
            textTitle = textTitle + "<td onclick=\"" + strFocusEvent + ";\" class=mulinetitle><input class=mulinetitle readonly value='" + tempText0 + "' style='width:" + tempText1 + "'>";
            ...
            if (userWidth < cObjInstance.windowWidth){
                if (trate != 0){// add by wanglei 2008-08-12 如果是改变宽度了，就不能重新算rate了，必须用原来的了
                    rate = trate;
                }
                else{
            ...
            for (i = 1; i < fieldCount; i++){
                ...
                tempText24 = iArray[i][23]; //设置当前列点击按钮
                tempText25 = iArray[i][24]; //设置单列触发onchange方法
                tempText26 = iArray[i][25]; //设置单列自设置显示名称和方法
                tempText27 = iArray[i][26]; // 用来指定货币框是否是只读
                tempText28 = iArray[i][27];
                 try{
                    ...
                    if (tempText21 == undefined){
                     ...
                     if (tempText24 == undefined)
                    {
                        tempText24 = null;
                    }
                    if (tempText25 == undefined)
                    {
                        tempText25 = null;
                    }
                    if (tempText26 == undefined)
                    {
                        tempText26 = null;
                    }
                }
                    ...
                    if (cObjInstance.AllowSort == true){
                        if (tempTextTitle1 == 0){
                            textTitle = textTitle + "<td  class=mulinetitle>";
                           		...
                           		textTitle = textTitle + "<td  class=mulinetitle ><NOBR><input onclick=\"OrderByName(" + cObjInstance.instanceName + ",'" + tempText0+"'); \" class=mulinetitle readonly value='" + tempText0 + "' style='cursor:hand;width :" + tempTextTitle1 + "' >" + "<input style='width:1;border:none;'></NOBR></td>";
                        }
                    }
                    if (tempText4 == "10") {
                    	text = text + "<td class=muline width='" + tempText1 + "'><input name=" + strPageName + i + " value='" + tempText15 + "' datatype='"+tempText28+"' id=" + strPageName + i + "r$row$ " + isReadOnly(tempText4) + " class=" + isReadOnlyClass(tempText4) + " maxlength=" + tempText2 + " onClick=\"" + strFocusEvent + ";\"" + " onFocus=\"" + strFocusEvent + ";\" ";
			        } else {
                        ...
                        text = text + " style='text-align:left; padding-left:2px;'";
 			              ...
 			              if(tempText4 == "7"){
	                    	    ...
	                    	    if(tempText23.substring(0,3).toUpperCase() == "COL"){
						            ...
						            if((tempText27 != "") && (tempText27 != null) && (tempText27!= undefined) ){
							              text = text + " " + isReadOnly(tempText27) + " " ;	
						            }
					          }
...
//                    }
                ...
                if( tempText4 == "6" || tempText4 == "9" ){
        			...
        			if ((tempText24 == null || tempText24 == '') && (tempText26 == null || tempText26 =='')) {
	                	if (tempText25 != null && tempText25 !='') {
		                	   text = text + "  onchange=\""+ tempText25 +";\"";
		                }
	                    text = text + " style='width:" + tempText1 + "'>";
	                }
	                if ((tempText24 != null && tempText24 !='') && (tempText26 == null || tempText26 =='')) {
	                	   text = text + " style='width:100px' onchange=\"change('span$PageName$$SpanId$');\">";
	                       text = text + "<input class='cssbutton' type='button' value='选择'";
	                       text = text + "  onclick=\"return "+ tempText24 +"('span$PageName$$SpanId$');\"";
	                       text = text + " />";
	                }
	                if ((tempText24 == null || tempText24 =='') && (tempText26 != null && tempText26 !='')) {
	                	   text = text + " style='width:0px'>";
	                       text = text + " <input class='cssbutton' type='button' value=\""+tempText26.split('|')[1]+"\" ";
	                       text = text + "  onclick=\"return "+ tempText26.split('|')[0] +"('span$PageName$$SpanId$');\"";
	                       text = text + " />";
	                }
       			}
                text = text + "</td>";
            ...
            if ((eArray != null) && (eArray != '') && (eArray != "")){
                ...
                for(jindex=0;jindex<jnum;jindex++){
    ...
    try{
        ...
        for (i = 1; i <= intCount; i++){
            ...
            for (j = 0; j < cData[i - 1].length; j++){
                ...
                // }
                k = j + 1;
                if (strText.indexOf(t_StrPageName + k + " value='' datatype='money'") >= 0) {
                	strText = strText.replace(t_StrPageName + k + " value='' datatype='money'", t_StrPageName + k + " value='" + pointTwo(formatNum(cData[i - 1][j])) + "'");
                	strText = strText.replace(t_StrPageName + k + " value='.00'", t_StrPageName + k + " value=''");
                } else {
...
function isReadOnly(strReadOnly){
    var tempText;
    if (strReadOnly == "0" || strReadOnly == "10"){
        tempText = "readonly";
    }
    else{
        tempText = "";
...
function isReadOnlyClass(strReadOnly){
    ...
    if (strReadOnly == "0"){
	    ...
	    tempText = "mulLineDatePicker readonly";
	}
	else if (strReadOnly=="7"){
	    tempText = "multiCurrency";
	}
	else if (strReadOnly=="8"){
	    tempText = "mulLineDatePicker";
	}
	else if (strReadOnly=="9"){
	    tempText = "mulLineDatePickerColor readonly";
	}
    else{
        tempText = "mulcommon";
    }
...
function _SetRowColData(cRow, cCol, cData, cObjInstance){
    ...
    if ((cRow < 0) || (cRow >= tObjInstance.mulLineCount)){
        alert("MulLine.js-->setRowColData"+ I18NMsg('MutLine_FunctionSpecifiedtheWrongLine') +":"+ cRow);
        return tReturn;
    }
    if ((cCol < 0) || (cCol >= tObjInstance.colCount)){
        alert("MulLine.js-->setRowColData"+ I18NMsg('MutLine_FunctionSpecifiedtheWrongLine') +":"+ cCol);
        return tReturn;
    }
    try{
        ...
        if (tObjInstance.mulLineCount == 1){
            _ResumeState();
            try{
								// 将 fm.all('element').value给位
                                // fm.element.all的形式，解决翻页有时第一行不显示的问题 modify by
                                // wys at 2011-11-09 16:40:26
                // tStr = tObjInstance.formName + ".all('" +
                // tObjInstance.instanceName + cCol + "').value='" + newData +
                // "'";
                tStr = tObjInstance.formName + "." + tObjInstance.instanceName + cCol + ".value='" + newData + "'";
            }
            catch(ex){
...
function _SetRowColClass(cRow, cCol, cData, cObjInstance){
    ...
    try{
        ...
        // _DisplayError("在 MulLine.js --> _SetRowColData 函数中发生异常：" + ex,
        // tObjInstance);//edit by yaory
    }
    return tReturn;
}
// tongmeng 2008-10-27 add
...
function _FindSpanID(cRow, cObjInstance){
    ...
    tObjInstance.errorString = "";
    if ((cRow < 0) || (cRow >= tObjInstance.mulLineCount)){
        //alert("MulLine.js-->findSpanID"+ I18NMsg('MutLine_FunctionSpecifiedtheWrongLine') +":" + cRow);
        return tReturn;
    }
    try{
        // 注意：必须添加下面的判断条件，否则在初始化为0行时，会出现异常
            ...
            try{
                ...
                if ((tRule == null) || (tRule == "")){
                    ...
                    for (var n = 0; n < tObj.mulLineCount; n++){
                        ...
                        try{
                            rowNo = n + 1;
                            strInfo = I18NMsg("MutLine_Row") + rowNo + I18NMsg("MutLine_Row2") + tRule;	// 提示信息中确定第几行
...
/**
 * 方法：检验 MulLine 中某行某列是否得到焦点 输入：行数(从 0 开始) 输入：列数(从 1 开始) 输出：true or false
...
function AllowSortFun(obj, i){
	//20150417 tianzf 禁用界面mutline点击表头排序功能（排序后翻页按钮会丢失）
	return;
...
function _SetPageMark(cTurnPage){
    ...
    var idx = tHTML.indexOf("<DIV class=MulLinePageDiv");
    if (idx < 0) {
        idx = tHTML.indexOf("<div class=MulLinePageDiv");
    }
    if (idx > 0) {
        tHTML = tHTML.substring(0, idx);
    }
    
    var pageEvent = "";
    if (this.pageEventFuncName != "" && this.pageEventFuncName != null) {
        pageEvent = this.pageEventFuncName + "()";
    } 
    
    tHTML += "<div class=MulLinePageDiv valign=middle>";
    tHTML += "<table class = MulLinePageTable><tr hight=18px valign=middle>";
    if (tPageIndex != '1'){
        tHTML += "<td valign=bottom >&nbsp<img src='../common/images/firstPage.gif' style='cursor: hand;' onclick='" + tStrPageName + ".SortPage.gotoPage(1);" + pageEvent + "' name=btnchangepage>&nbsp</td>";
        tHTML += "<td valign=bottom ><img src='../common/images/PreviousPage.gif' style='cursor: hand;' onclick='" + tStrPageName + ".SortPage.gotoPage(" + tPageIndex + "-1);" + pageEvent + "' name=btnchangepage>&nbsp</td>";
    }
    else{
        tHTML += "<td valign=bottom >&nbsp<img src='../common/images/firstPage.gif' style='cursor: hand;' onclick='alert(\""+I18NMsg("MutLine_Thisisthetopoflist")+"!\");" + pageEvent + "' name=btnchangepage>&nbsp</td>";
        tHTML += "<td valign=bottom ><img src='../common/images/PreviousPage.gif' style='cursor: hand;' onclick='alert(\""+I18NMsg("MutLine_Thisisthetopoflist")+"!\");" + pageEvent + "' name=btnchangepage>&nbsp</td>";
    }
    tHTML += "<td valign=bottom >"+I18NMsg("MutLine_Page")+"&nbsp;" + tPageIndex + "/" + tTotalPageNum + "&nbsp;"+I18NMsg("MutLine_Page2")+"&nbsp;</td>";
    if (tPageIndex != tTotalPageNum){
        tHTML += "<td valign=bottom ><img src='../common/images/NextPage.gif' style='cursor: hand;' onclick='" + tStrPageName + ".SortPage.gotoPage(" + tPageIndex + "+1);" + pageEvent + "' name=btnchangepage>&nbsp</td>";
        tHTML += "<td valign=bottom ><img src='../common/images/lastPage.gif' style='cursor: hand;' onclick='" + tStrPageName + ".SortPage.gotoPage(" + tTotalPageNum + ");" + pageEvent + "' name=btnchangepage>&nbsp</td>";
    }
    else{
        tHTML += "<td valign=bottom ><img src='../common/images/NextPage.gif' style='cursor: hand;' onclick='alert(\""+I18NMsg("MutLine_Thisistheendoflist")+"!\");" + pageEvent + "' name=btnchangepage>&nbsp</td>";
        tHTML += "<td valign=bottom ><img src='../common/images/lastPage.gif' style='cursor: hand;' onclick='alert(\""+I18NMsg("MutLine_Thisistheendoflist")+"!\");" + pageEvent + "' name=btnchangepage>&nbsp</td>";
    }
    tHTML += "<td valign=bottom >"+I18NMsg("MutLine_TurnToPage")+"&nbsp;<input type='common' style='{border: 1px #9999CC solid;height: 18px}' name='GotoPage" + tStrPageName + "' size='3'>&nbsp;"+I18NMsg("MutLine_Page2")+"</td>";
    tHTML += "<td valign=center>&nbsp<img src='../common/images/goto.gif' style='cursor: hand;' onclick='" + tStrPageName + ".SortPage.gotoPage(document.all.GotoPage" + tStrPageName + ".value);" + pageEvent + "' name=btnchangepage></td>";
    tHTML += "</tr></table></div>";
    document.all("span" + tStrPageName).innerHTML = tHTML;
}
/**
 * 方法：返回选中的 CheckBox 总个数 输入：无 输出：整数 备注：XinYQ added on 2006-05-11
 */
...
// ==============================================================================
/**
 * 方法：使指定的 radioBox 变成选中的状态 输入：行数(从 1 开始) 输出：如果选中，返回 true，否则返回 false 备注：XinYQ
 * added on 2006-05-15
 */
...
// ==============================================================================
/**
 * 方法：使指定的 checkBox 变成选中的状态 输入：行数(从 1 开始) 输出：如果选中，返回 true，否则返回 false 备注：XinYQ
 * added on 2006-05-15
 */
...
// ==============================================================================
/**
 * 方法：当其子控件任意一个得到焦点时计算该子控件的行和列 输入：MulLine 对象 输入：子控件对象 输出：无 备注：XinYQ added on
 * 2006-10-10
 */
...
function ChangeColumn(table, sor, tag){
    try{
        for (var i = 0; i < table.rows.length; i++) {table.rows[i].cells[sor].swapNode(table.rows[i].cells[tag]);}
        return 1;
    }
    catch(ex){
        return 0;
...
//    var objClone;
//		try{
//    		if (this.constructor == Object){objClone = new this.constructor();} else {objClone = new this.constructor(this.valueOf());}
//    		for (var key in this){
...
//更换深拷贝方法，原因是使用clone与jquery使用的clone冲突引起一些问题 modify by zy 2012-11-30
function getType(o)
{
    var _t;
...
/*******************************************************************************
 * 这个为IE6-7的一个bug写的方法,ff不存在这个问题 add by wanglei 2008-08-13
 ******************************************************************************/
...
/**
 * 方法：设置指定行列的数据(外部/内部调用) 输入：cRow: 行 cCol: 列 cData: 数据 cObjInstance
 * Muline对象，外部调用时为空；内部调用时不能为空 cCode: 值为"readonly"时，改数据为只读; 为其他非空值，作为codeselect
 * code； 为null，只赋值 ""，显示输入框 cWidth:设定列宽
 * 顶级函数一般是外部调用，则cObjInstance为空，内部tObjInstance=this;
 * 则调用子函数时，将当前对象this作为cObjInstance参数传入调用子函数
 */
function _SetRowColDataCustomize(cRow, cCol, cData, cObjInstance, cCode, cWidth){
	...
	if(cData==null){cData="";}
	if((cRow<0)||(cRow>=tObjInstance.mulLineCount)){
		//alert("MulLine.js-->SetRowColDataCustomize"+ I18NMsg('MutLine_FunctionSpecifiedtheWrongLine') +":"+cRow);
		return tReturn;
	}
	if((cCol<0)||(cCol>=tObjInstance.colCount)){
	...
	try{
		...
		//_DisplayError("在 MulLine.js-->_SetRowColDataCustomize 函数中发生异常：" + ex.message + " ", tObjInstance);
	}
	return tReturn;
}
// ==============================================================================
...
function _SetRowColDataCustomize1(cRow, cCol, cData, cObjInstance, cCode, cWidth){
	...
	if(cData==null){cData="";}
	if((cRow<0)||(cRow>=tObjInstance.mulLineCount)){
		//alert("MulLine.js-->SetRowColDataCustomize1"+ I18NMsg('MutLine_FunctionSpecifiedtheWrongLine') +":"+cRow);
		return tReturn;
	}
	if((cCol<0)||(cCol>=tObjInstance.colCount)){
	...
	try{
		...
		//_DisplayError("在 MulLine.js-->_SetRowColDataCustomize1 函数中发生异常：" + ex.message + " ", tObjInstance);
	}
	return tReturn;
}
/**
