...
function easyQueryVer3(strSql, synchronization, useCache, strStart, strLargeFlag, strLimitFlag)
{
    ...
    {
        alert(I18NMsg("EasyQueryVer3 need a SQL statement"));
        return false;
    }
    else
    {
...
//        Request.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
        ...
        //by fengfei 2012-11-4 发现上面数据方式无效，改为以下方式
...
//          for(j=0; j<fieldNum; j++)
    ...
    {
        alert(I18NMsg("FailedtoSplittheData!theCauseoftheErrorI") + ex);
        return null;
    }
    //配合翻页控制进行显示数据处理
...
function decodeEasyQueryResultV2(statment,strResult,notUseEasyQuery)
{
    ...
    try
    {
        ...
        for(i=0; i<recordNum; i++)
        {
            ...
            {
                alert(I18NMsg("LengthNotMatched")+","+I18NMsg("CanNotUseThisMethod"));
                return null;
            }
            arrEasyQuery[i] = new Array();
            for(j=0; j<fieldNum; j++)
    ...
    catch(ex)
    {
    	...
    	alert(I18NMsg("FailedtoSplittheData!theCauseoftheErrorI") + ex.message);
        return null;
    }
...
function displayMultiline(arrDisplayData, multilineGrid, otherTurnPage)
{
    ...
    {
        alert( I18NMsg("displayEasyResult没有收到要显示的数据!" ));
        return false;
    }
    //multilineGrid接口参数处理
    if (typeof(multilineGrid) == "undefined")
    {
        alert( I18NMsg("displayEasyResult没有找到显示容器对象MULTILINE!" ));
        return false;
    }
    // 初始化MULTILINE表格
    try
...
//      multilineGrid.loadMulLine(multilineGrid.arraySave);
        ...
        // 2013-01-16 lanruijin 增加页面跳转显示（转到X页），在此处添加不太合适，但不想改每个调用页面
        if(cTurnPage.pageDisplayGrid.SortPage != cTurnPage){
        	cTurnPage.pageDisplayGrid.SortPage = cTurnPage;
            cTurnPage.pageDisplayGrid.setPageMark(cTurnPage);
        }
        // end 2013-01-16 lanruijin
    }
    catch(ex)
    {
        alert(I18NMsg("InitializetheMultilineObjectError"));
        return false;
    }
...
function getPageDisplayData(arrDataSet, arrDataIndex, pageRecordNum, otherTurnPage)
{
    ...
    {
        alert(I18NMsg("getPageDisplayData处理出错！"));
        return null;
    }
...
function getNextPage()
{
    ...
    {
        alert(I18NMsg("No data, please query"));
        return false;
    }
    ...
    {
        alert(I18NMsg("Thisistheendoflist"));
        return false;
    }
...
function turnPageClass()
{
    ...
    this.makeExcel=MakeExcel;//add by zhaopeng 20091030
 ...
 */
 ...
 function getWherePart(fieldName, controlName, strOperate, fieldType) {
    ...
    //hanxl modified on 2008年11月11日 09时31分06秒,修正SQL注入问题及两端空格的问题
	//OLD:value = window.document.getElementsByName(trim(controlName))[0].value;
	value = window.document.getElementsByName(trim(controlName))[0].value.replace(/'/g,'').trim();
...
function clearArrayElements(arrData)
{
    ...
    {
        alert(I18NMsg("ClearArrayElements进行数组清空失败")+ex);
        return new Array();
    }
    return arrData;
}
...
function EasyShowForm( strSQL,alterflag,paraForm)
{
    ...
    if (statment==null)
    {
        alert(I18NMsg("IncomingSqlDoesNotMeettheRequirements"));
        return false;
    }
    ...
    for( i=0;i<statment.length ;i++)
    {
        ...
        catch(ex)
        {
            if ( alterflag)
            {
                alert(I18NMsg("DidNotFindthePageElements")+ statment[i]);
                return false;
            }
        }
    }
...
function showGotoPage(tIndex){
    ...
    if (cTurnPage.strQueryResult == "" || cTurnPage.strQueryResult == false) {
//        alert("没有数据，请先查询！");
        alert(I18NMsg("Nodata,pleasequery."));
        return false;
    }
    if(tIndex==null||tIndex==""||!isInteger(tIndex))
    {
        alert(I18NMsg("Alert_Pleaseinputareasonablepagenumber")+"!");
        return;
    }
    if(tIndex<1 || tIndex>Math.ceil(cTurnPage.queryAllRecordCount/cTurnPage.pageLineNum)){
        alert(I18NMsg("Alert_Turntopageisoutofindex")+"!");
        return;
    }
...
{
	//ADD by tianzf 20140822 start
	//每次 setSQLID的时候初始化参数列表，不需要每次new一次对象
	this.sqlParaName = new Array();//参数名称数组
    this.sqlParaValue = new Array();//参数值数组
	//ADD by tianzf 20140822 end
    var cMySqlClass=this;
...
function _setInpart(paraName,controlvalue)
{
 ...
 //add by zhaopeng 20091030
 /*
function:导入函数
Created by: mojiao
Create Date: 2005-09-22	
History:
   by wellhi :增加传入参数，以对某些列的格式进行自定义
   Date :2006-07-26
   para: sFormat="1|@^2|0.00_ ^4|@";
*/
function MakeExcel(sFormat)
{ 
	try
	{
		var TurnPage = this;
		var MulLine = TurnPage.pageDisplayGrid;
		if(TurnPage.arrDataCacheSet.length==0)
		{
			alert(I18NMsg("No data, please query."));
			return;
		}
		...
		var xls;
		var xlBook;
		var rowIndex=2;//写入Execl的当前行指针
	  	var pageIndex=0;//turnPage中的arrDataCacheSet中的页的指针
		...
		if(!(sFormat==null || trim(sFormat)==""))
		{
			var arrFormat=decodeStringNoHead(sFormat);
			for(i=0;i<arrFormat.length;i++)
			{
				xls.Columns(parseInt(arrFormat[i][0])+1).NumberFormatLocal=arrFormat[i][1];
			}			
		}
		...
		for (var i=0; i<MulLine.arraySave.length; i++) {     
			arrHeader.push(MulLine.arraySave[i][0]);
		}
		f11Array(xlsheet,arrHeader,1,1,'0');
		
		if (TurnPage.pageIndex != 0)
		{
			TurnPage.firstPage();
		}
		...
		while(true)
		{
			...
			for (var i=0; i<MulLine.mulLineCount; i++) {
			  arrOrder.push(MulLine.recordNo + i + 1);
			}
			f11Array(xlsheet,arrOrder,rowIndex,1,'1');	
			
			//判断是否要从数据库检索新的数据块，如果是arrDataCacheSet中对应的pageIndex要重新从0开始计数
			if (((TurnPage.pageIndex * TurnPage.pageLineNum )% TurnPage.dataBlockNum) == 0 && TurnPage.useSimulation == 0)
			{
				pageIndex=0;		
			}
			
			//表体数组
			arrTableBody=TurnPage.getData(TurnPage.arrDataCacheSet, pageIndex*TurnPage.pageLineNum, TurnPage.pageLineNum);//数据体
			f11ArrayGroup(xlsheet,arrTableBody,rowIndex,2);
			
			pageIndex++;
			rowIndex+=MulLine.mulLineCount;
			
			//判断是否已经到了最后一页
			if (TurnPage.pageIndex == Math.ceil(TurnPage.queryAllRecordCount/TurnPage.pageLineNum) - 1)
			{
				break;
			}else
			{
	...
	{
		alert(I18NMsg("AnerroroccurredwhileprocessingExcel")+ee);	
	}
	finally
	{  
		TurnPage=null;
...
//接受一维数组
//
function f11Array(xlsheet,nArr,nRow,nCol,nFlag){
    for (var k=0; k<nArr.length; k++) {
      if(nFlag=='0') //横向
      { 
        xlsheet.Cells(nRow,nCol+k).Value =nArr[k];
      }
      if(nFlag=='1') //纵向
      {
        xlsheet.Cells(nRow+k,nCol).Value =nArr[k]; 
      }         
    }
}
...
//接受二维数组
//
function f11ArrayGroup(xlsheet,nArr,nRow,nCol){
    var i,j;
    for( i = 0; i < nArr.length; i++ ){
      for(j = 0; j < nArr[0].length; j++){	         
        xlsheet.Cells(i+nRow,j+nCol).Value=nArr[i][j];
        // add by ZhouQi 2013-05-24：自适应列宽
        xlsheet.Columns.AutoFit; 
      }
    }
}