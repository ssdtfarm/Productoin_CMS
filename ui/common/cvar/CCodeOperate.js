...
function initializeCodeEx(strCodeName,cField)
{
	...
	{
		alert(""+I18NMsg('Code data is not described in the client!')+"");
		return false;
	}
	arrRecord = strValue.split(_Code_RECORDDELIMITER);	//拆分字符串，形成返回的数组
	t_Str = getStr(arrRecord[0],1,_Code_FIELDDELIMITER);
...
//		+ ">";
	...
	{
		//tStr = arrCode[i][0]+"-"+arrCode[i][1];
		tStr = arrCode[i][1];
		tCodeL = arrCode[i][0]+"-";
		//alert(arrCode[i][0]+"-"+arrCode[i][1]);
		try{
			tCurLen = tStr.length;
			tCodeLen =tCodeL.length;
		}
		catch (ex){}
		if (tCurLen>tMaxLen) tMaxLen = tCurLen;
		if(tCodeLen>tCodeMaxLen) tCodeMaxLen = tCodeLen ;
	}
	
	tCurLen = 14 * (tMaxLen +tCodeMaxLen/2);	//XinYQ modified on 2006-06-12 : 原来的基数 25 过大,造成类似 Station 过长
	//alert(tCodeMaxLen/2);
	//alert(tCurLen);
...
function setFieldValue(Field,strCodeName,strCondition,strConditionField,changeEven)
{
	...
	try
	{
		for (i=0; i<iMax; i++)
		{
			//update by zjf 20130403 for:将undefind转换为空串，便于清空回填项
			var temp = tFldCode[tArrDisplayOrder[i]];
			if(temp == undefined){
				temp = "";
			}
			tArrDisplayObj[i].value = temp;	//根据显示顺序设置显示对象
		}
//		if (changeEven != null)
//		{
...
function goTest(obj)
{
	...
	if (arrayBmCode!=null)
	{
		...
		{
			//alert(arrayBmCode[i][0]);
			if( arrayBmCode[i][0] == trim(obj.value))
			{
				found=true;
				return;
			}	
	...
	//add by zhaopeng 20100727 如果没有匹配到，清空该下拉框
	if(found==false && i != 0)
	{
		obj.value = "";
	}
...
/**
 * 定位到指定的代码位置
 * modify by zhaopeng 20100728 如果录入错误，直接清空下拉框
 */
