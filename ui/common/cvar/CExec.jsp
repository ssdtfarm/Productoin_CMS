...
if(strMethod.equals("POST"))
{
	if( !request.getParameter( "mOperate" ).equals( "EASYQUERY" ))
	{
		...
		out.println("try{");
		out.println("  parent.document.frames('"+ESAPI.encoder().encodeForJavaScript(framename)+"').initializeCode('"+ESAPI.encoder().encodeForJavaScript(codename)+"', '" + ESAPI.encoder().encodeForJavaScript(codeCondition) + "' , '" + ESAPI.encoder().encodeForJavaScript(conditionField) + "');");
		out.println("}catch(exception){}");
		String tempStr="";
		out.println("try{");
		out.println("  parent.document.frames('"+ESAPI.encoder().encodeForJavaScript(framename)+"').showCodeList('"+ESAPI.encoder().encodeForJavaScript(codename)+"',null,null,null, '" + ESAPI.encoder().encodeForJavaScript(codeCondition) + "' , '" + ESAPI.encoder().encodeForJavaScript(conditionField) + "' , null, '" + ESAPI.encoder().encodeForJavaScript(showWidth) + "',"+ESAPI.encoder().encodeForJavaScript(changeEven)+");");
		out.println("}catch(exception){}");
		out.println("</script>");
	}
	else
	{
		...
		else
		{
			...
			tResult=ESAPI.encoder().encodeForHTML(tResult);
%>
...
<%
		...
		out.println("<script language=javascript>");
		out.println("try{");
		out.println("  parent.fraInterface.afterEasyQuery('" + ESAPI.encoder().encodeForJavaScript(tError) + "');");
		out.println("}catch(exception){}");
		out.println("</script>");
	}
}
